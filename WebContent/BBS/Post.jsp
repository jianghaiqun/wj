...
Page.onLoad(function(){
	...
	Server.sendRequest("com.sinosoft.bbs.Post.addViewCount",dc);
...
function isOperaterMember(ID, type) {
	...
	Server.sendRequest("com.sinosoft.bbs.Post.isOperaterMember",dc,function(response){
...
function save(){
	 ...
	 if(dc.get("Message").length==0){
		Dialog.alert("回复内容不能为空",function(){
		$("Message").focus();
		});
		return;
	 }
...
function del(ID){
	...
	Dialog.confirm("确定删除选中的帖子吗？",function(){
		...
		dc.add("ID",ID);
		dc.add("SiteID",$V("SiteID"));
   		Server.sendRequest("com.sinosoft.bbs.Post.del",dc,function(response){
			if(response.Status==1){
				DataList.loadData("list1");
...
function edit(ID){
	...
	if(flag){
		...
		dc.add("ID",ID);
		dc.add("SiteID",$V("SiteID"));
   		Server.sendRequest("com.sinosoft.bbs.Post.edit",dc,function(response){
   			if(response.Status==1){
   				$("edit"+ID).innerHTML="<textarea style='width:750px;height:250px' id='EditTextarea'>"+response.get("Message")+"</textarea><br><input type='button' onclick='editSave("+ID+")' value='提交'><input type='button' onclick='cancel()' value='取消'>"
   				$("EditTextarea").focus();
   				flag = false;
   			}else{
   				Dialog.alert(response.Message);
...
function editSave(ID){
	...
	Server.sendRequest("com.sinosoft.bbs.Post.editSave",dc,function(response){
...
function hide(ID){
		var dc = new DataCollection();
		dc.add("ID",ID);
   		Server.sendRequest("com.sinosoft.bbs.Post.hide",dc,function(response){
			if(response.Status==1){
				DataList.loadData("list1");
			}
		});
...
<div class="wrap">
<z:init method="com.sinosoft.bbs.Post.init">
<div id="menu" class="forumbox"><span class="fl"><a href="Index.jsp?SiteID=${SiteID}">${BBSName}</a> &raquo; <a href="Theme.jsp?ForumID=${ForumID}&SiteID=${SiteID}">${Name}</a> &raquo; ${Subject} </span> <span class="fr">${Priv} </span></div>
<div id="foruminfo">
...
<z:datalist id="list1" method="com.sinosoft.bbs.Post.getList" page="true" size="12">
	<form method="post">
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="forumTable">
	...
	<tr>
		<td ><input name="replysubmit" type="button" id="postsubmit" tabindex="3" value="发表帖子" onClick="save()"><input type="reset" value="清空内容" ></td>
	</tr>
