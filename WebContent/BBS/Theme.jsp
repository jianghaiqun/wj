 ...
 var global = null; 
	...
	function del(){
		...
		Dialog.confirm("确定删除选中的项吗？",function(){
			...
			Server.sendRequest("com.sinosoft.bbs.Theme.del",dc,function(response){
...
function moveSave(){
	...
	dc.add("IDs",arr.join());
    dc.add("SiteID",$V("SiteID"));
	Server.sendRequest("com.sinosoft.bbs.Theme.move",dc,function(response){
		if(response.Status==0){
			Dialog.alert(response.Message);
		}else{
			$D.close();
...
function brightSave(){
	...
	dc.add("IDs",arr.join());
	dc.add("SiteID",$V("SiteID"));
	Server.sendRequest("com.sinosoft.bbs.Theme.brightSave",dc,function(response){
		if(response.Status==0){
			Dialog.alert(response.Message);
		}else{
			$D.close();
...
function upOrDownSave(){
	...
	dc.add("IDs",arr.join());
	dc.add("SiteID",$V("SiteID"));
	Server.sendRequest("com.sinosoft.bbs.Theme.upOrDownSave",dc,function(response){
		if(response.Status==0){
			Dialog.alert(response.Message);
		}else{
			$D.close();
...
function topSave(){
	...
	dc.add("IDs",arr.join());
	dc.add("SiteID",$V("SiteID"));
	Server.sendRequest("com.sinosoft.bbs.Theme.topTheme",dc,function(response){
		if(response.Status==0){
			Dialog.alert(response.Message);
		}else{
			$D.close();
...
function bestSave(){
	...
	dc.add("IDs",arr.join());
	dc.add("SiteID",$V("SiteID"));
	Server.sendRequest("com.sinosoft.bbs.Theme.bestTheme",dc,function(response){
		if(response.Status==0){
			Dialog.alert(response.Message);
		}else{
			$D.close();
...
<z:init method="com.sinosoft.bbs.Theme.init">
<div class="wrap">
...
<form method="post" name="moderate" action="topicadmin.jsp?action=moderate&amp;fid=5">
    ...
    <tbody>
	<z:datalist  id="list1" method="com.sinosoft.bbs.Theme.getList" page="true" size="12" >
