...
function add(){
	...
	function addSave(){
		...
		Server.sendRequest("com.sinosoft.bbs.admin.ForumUserGroup.addSpecialGroup",dc,function(response){
	...
	function eidtColor(){
		...
		Server.sendRequest("com.sinosoft.bbs.admin.ForumUserGroup.editSpecialGroup",dc,function(response){
			Dialog.alert(response.Message);
			if(response.Status==1){
				DataList.loadData('list1');
			}
	...
	function del(ID){
		Dialog.confirm("删除后不可恢复，确认要删除？",function(){
			...
			dc.add("ID",ID);
			Server.sendRequest("com.sinosoft.bbs.admin.ForumUserGroup.delSpecailGroup",dc,function(response){
				Dialog.alert(response.Message);
				if(response.Status==1){
					DataList.loadData('list1');
				}
...
<table width="100%" border="1" cellpadding="0" cellspacing="0" bordercolor="#eeeeee" class="forumTable">
	...
	<z:datalist id="list1" method="com.sinosoft.bbs.admin.ForumUserGroup.getListSpecialGroup" page="false">
		<tr nowrap="nowrap">
