...
function add(){
	...
	var url="Article.jsp?CatalogID=" + catalogID + "&displayFlag=" + "yes";
...
function transURL(){
	var arr = DataGrid.getSelectedValue("dg1");
	if(arr == null || arr.length==0){
		Dialog.alert("请先选择要转换的一篇文章！");
		return;
	}
	if(arr.length>=2){
		Dialog.alert("请选择一篇文章进行URL转换！");
		return;
	}
	var id = arr[0];
	...
	diag.Height = 200;
	diag.Title = "选择转移栏目";
	diag.URL = "Site/transURLDialog.jsp?ArticleID="+id;
	diag.OKEvent = transURLSave;
	diag.show();
}
function transURLSave(){
	var dc = $DW.Form.getData("form2");
	if($DW.Verify.hasError()){
		return;
	}
	Server.sendRequest("com.sinosoft.cms.document.ArticleList.transURL",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				$D.close();
				DataGrid.loadData("dg1");
...
$("dg1").onContextMenu = function(tr,evt){
	...
	if(Tree.CurrentItem&&Application.getPriv("article",Tree.CurrentItem.getAttribute("innerCode"),"article_modify")&&$V("Type")!="ARCHIVE") {
		...
		menu.addItem("归档",setArchive,"Icons/icon002a11.gif");
...
function setArchive(id){	
	...
	var dc = new DataCollection();
	dc.add("IDs",arr.join(","));
	Server.sendRequest("com.sinosoft.cms.document.ArticleList.setArchive",dc,function(response){
		if(response.Status==1){
			Dialog.alert("归档成功!",function(){
				DataGrid.loadData("dg1");
			});
		}
	});
...
function search(){
	DataGrid.setParam("dg1",Constant.PageIndex,0);
	DataGrid.setParam("dg1","Prop2",$V("Prop2"));
	DataGrid.setParam("dg1","Keyword",$V("Keyword"));
...
<body>
<z:init method="com.sinosoft.cms.document.ArticleList.initDocList">
	...
	<tr valign="top">
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0"
			...
			<tr>
				...
				<div style="float: left"><z:tButtonPurview>${_Document_DocList_Button}</z:tButtonPurview></div>
				</td>
			</tr>
			<tr>
				...
				<div style="float:left;">列出: <z:select id="Type"
					...
					<!-- <option value="ONPUBLISH">发布中的文档</option> -->
		  ...
		  分类属性:<z:select id='Prop2'> ${Prop2}</z:select>
