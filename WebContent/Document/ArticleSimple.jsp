  ...
  var content = contents.join(PAGE_SPLIT);
	...
	Server.sendRequest("com.sinosoft.cms.document.Article.checkBadWord",dc,function(response){
		if(response.Status == 1){
			...
			if (isBadWordExist) {
				Dialog.wait("正在处理...");
				Server.sendRequest("com.sinosoft.cms.document.Article.save",dc,function(response){
					Dialog.closeEx();
					if(response.Status==1){
...
function del(){
	...
	Dialog.confirm("确认删除吗？点击”确认“删除，点击”取消“返回。",function(){
		...
		Server.sendRequest("com.sinosoft.cms.document.Article.del",dc,function(response){
			if(response.Status==1){
				Dialog.alert("文章删除成功");
		    	window.close();
			}else{
		...
		if(!$("CommentFlag").checked){
	  	...
	  	Server.sendRequest("com.sinosoft.cms.document.Article.checkBadWord",dc,function(response){
			if(response.Status == 1){
				...
				if (isBadWordExist) {
					Dialog.wait("正在发布...");
					Server.sendRequest("com.sinosoft.cms.document.Article.saveAndPublish",dc,function(response){
						Dialog.closeEx();
						if(response.Status==1){
							var st = response.get("SaveTime");
							$("SaveTime").innerHTML = st;
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0">
		<tr>
			...
			<div>
				...
				onClick="closeX()">
				<img src="../Icons/icon403a11.gif" />返回</z:tbutton> 
				</div>
			</td>
		</tr>
	</table>
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0"
		...
		<tr>
					...
					<table width="100%" cellpadding="2" cellspacing="0">
						...
						<tr>
							...
							<td align="left"><input name="URL" id="URL"
								size="50" type="text" value="${URL}"></td>
						 </tr>
              <tr style="display:none">
					...
					<table width="100%" cellpadding="2" cellspacing="0">
						<tr>
							<td width="8%" align="center">页面标题</td>
							<td align="left">
								<input name="PageTitle" type="text" id="PageTitle" size="50" value="${PageTitle}" />
								<input name="_PageTitles" type="hidden" id="_PageTitles" value="${PageTitles}" />
							</td>
						</tr>
					</table>
			...
			<td width="6" rowspan="3" align="right" bgcolor="#F6F9FD"
				style="border-left: 1px solid #91A9BD;border-right: 1px solid #CFE6F2"></td>
			<td width="220" valign="top" id="id_AdvanceTd"
				style="padding-top: 2px; width:220px;display:none;">
