...
function del(){
	...
	Server.sendRequest("com.sinosoft.cms.document.ArticleHistory.del",dc,function(response){
...
function restore(){
	...
	Server.sendRequest("com.sinosoft.cms.document.ArticleHistory.restore",dc,function(response){
		if(response.Status==0){
			alert(response.Message);
		}else{
			alert("成功恢复文档");
...
function add(){
	var dc = new DataCollection();
	dc.add("ArticleID",$V("ArticleID"));
	Server.sendRequest("com.sinosoft.cms.document.ArticleHistory.addVersion",dc,function(response){
		if(response.Status==0){
			alert(response.Message);
		}else{
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	...
	<tr>
		...
		<z:datagrid id="dg1"
			method="com.sinosoft.cms.document.ArticleHistory.historyDataBind"
			size="12">
