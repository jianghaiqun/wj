...
function editSave(){
	...
	Server.sendRequest("com.sinosoft.cms.document.Weekwork.save",dc,function(response){
...
function addSave(){
	...
	Server.sendRequest("com.sinosoft.cms.document.Weekwork.addSave",dc,function(response){
		Dialog.alert(response.Message);
		if(response.Status==1){
				$D.close();
				DataGrid.loadData('dg1');
...
function editNote(ID){
	...
	dc.add("ID",ID);
	Server.sendRequest("com.sinosoft.cms.document.Weekwork.edit",dc,function(response){
		if(response.Status==1){
...
function editNoteSave(ID){
	...
	dc.add("ID",ID);
	dc.add("memo",text);
	Server.sendRequest("com.sinosoft.cms.document.Weekwork.editSave",dc,function(response){
...
function cancle(ID){
	...
	<body>
<z:init method="com.sinosoft.cms.document.Weekwork.init">
...
<input type="button" value="下一周" onclick="nextorpre('${nextDay}')">
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="6"
				...
				<tr>
					...
					<z:datagrid id="dg1"
						method="com.sinosoft.cms.document.Weekwork.dg1DataBind" size="9">
