...
function topublish(){
	var arr = DataGrid.getSelectedValue("dg1");
	if(arr == null || arr.length==0){
		Dialog.alert("请先选择要转为待发布的文章！");
		return;
	}
	
	var dc = new DataCollection();
	dc.add("IDs",arr.join());
	Server.sendRequest("com.sinosoft.cms.document.ArticleList.topublish",dc,function(response){
...
function changeToPublish(){
	var arr = DataGrid.getSelectedValue("dg1");
	if(arr == null || arr.length==0){
		Dialog.alert("请先选择要转为已发布的文章！");
		return;
	}
	
	var dc = new DataCollection();
	dc.add("IDs",arr.join());
	Server.sendRequest("com.sinosoft.cms.document.ArticleList.changeToPublish",dc,function(response){
		if(response.Status==0){
			Dialog.alert(response.Message);
		}else{
			$("dg1").loadData();
...
function copySave(){
	...
	Dialog.wait("正在复制文档...");
	Server.sendRequest("com.sinosoft.cms.document.ArticleList.copy",dc,function(response){
		Dialog.closeEx();
		if(response.Status==0){
			Dialog.alert(response.Message);
		}else{
...
$("dg1").onContextMenu = function(tr,evt){
	...
	if(Tree.CurrentItem&&Application.getPriv("article",Tree.CurrentItem.getAttribute("innerCode"),"article_modify")&&$V("Type")!="ARCHIVE") {
...
// 		menu.addItem("排序",sortOrder,"Icons/icon400a4.gif");
		...
		var topFlag = true;//表示所有选中项的置顶状态都一致
...
// 		menu.addItem("处理历史",articleLog,"Icons/icon_column.gif",flag);
		var downFlag = 1;//1表示全部可以下线，-1表示全部可以取消下线，0表示混杂，不能进行操作
...
// 			}else if(first=="30" || first=="10" || first=="20"){
// 				menu.addItem("-");
...
// 	if($V("Type")=="ARCHIVE"){
// 		menu.addItem("转为已发布",changeToPublish,"Icons/icon003a13.gif");
// 	}
...
function down(){
	...
	var dc = new DataCollection();
	dc.add("ArticleIDs",arr.join(","));
	Server.sendRequest("com.sinosoft.cms.document.ArticleList.down",dc,function(response){
		if(response.Status==1){
...
function up(){
	...
	var dc = new DataCollection();
	dc.add("ArticleIDs",arr.join(","));
	Server.sendRequest("com.sinosoft.cms.document.ArticleList.up",dc,function(response){
		if(response.Status==1){
			Dialog.alert("操作成功!",function(){
				DataGrid.loadData("dg1");
			});
...
function sortOrderSave(){
	...
	Server.sendRequest("com.sinosoft.cms.document.ArticleList.sortArticle",dc,function(response){
		Dialog.closeEx();
		if(response.Status==1){
			Dialog.alert("操作成功!",function(){
				$D.close();
...
function sortOrderSave1(){	
	DataGrid.save("dg1","com.sinosoft.cms.document.ArticleList.sortArticle1",function(){			
		DataGrid.setParam("dg1",Constant.PageIndex,0);
		DataGrid.loadData("dg1");
	});
}
...
function setTopSave(){	
	var dc = $DW.Form.getData("form2");
	Server.sendRequest("com.sinosoft.cms.document.ArticleList.setTop",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				$D.close();
				DataGrid.loadData("dg1");
...
function setNotTop(id){	
	...
	var dc = new DataCollection();
	dc.add("IDs",arr.join(","));
	Server.sendRequest("com.sinosoft.cms.document.ArticleList.setNotTop",dc,function(response){
		if(response.Status==1){
			Dialog.alert("操作成功!",function(){
				DataGrid.loadData("dg1");
			});
...
<%-- 				<z:tbutton id="BtnAdd"priv="article_modify" onClick="add()"> --%>
...
<%-- 					<img src="../Icons/icon003a6.gif" width="20" height="20" />待发布</z:tbutton> <z:tbutton --%>
...
<%-- 					<img src="../Icons/icon003a3.gif" width="20" height="20" />删除</z:tbutton> --%>
					
					<z:tbutton	id="BtnOrder" priv="article_modify" onClick="sortOrderSave1()">
					<img src="../Icons/icon003a7.gif" width="20" height="20" />调整确认</z:tbutton>
					<div style="margin-top: 5px;">&nbsp;&nbsp;双击一条或多条文档依次修改顺序值，确保已做修改的文档处于选中状态时，点击【调整确认】按钮，即可调整成功！</div></div>
				</td>
			</tr>
			<tr>
				...
				<z:datagrid id="dg1" method="com.sinosoft.cms.document.ArticleList.dg1DataBind" size="15">
					<table width="100%" cellpadding="2" cellspacing="0"
						...
						<tr ztype="head" class="dataTableHead">
							...
							<td width="30%" sortfield="title" direction=""><b>标题</b></td>
							<td width="20%"><strong>顺序值</strong></td>
							<td width="17%"><strong>状态</strong></td>
							<td width="20%" sortfield="publishdate" direction=""><strong>发布时间</strong></td>
						</tr>
						<tr >
							<td>&nbsp;</td>
							<td>&nbsp;</td>	
						...
						<tr ztype="edit">
							...
							<td algin="center">${Icon}</td>					
							<td style="${TitleStyle}"><span title="${Title}">${Title}</span></td>
							<td><input name="OrderFlag" type="text"
									id="OrderFlag1" value="${OrderFlag}" size="30"></td>						 
							<td>${StatusName}</td>
							<td title="${PublishDate}">${PublishDate}</td>
						</tr>
						<tr ztype="pagebar">
