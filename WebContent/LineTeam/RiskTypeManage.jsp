...
<z:init method="com.sinosoft.platform.Code.initRiskTypeManageList">
...
function update(){
	...
	diag.Height = 150;
	diag.Title = "险种维护修改";
	diag.URL = "LineTeam/RiskTypeManageDialog.jsp?Memo="+dr.get("Memo");
...
function updateThis(){
	...
	Server.sendRequest("com.sinosoft.platform.Code.dg1update",dc,function(response){
...
function add(){
	...
	diag.Height = 150;
	diag.Title = "新建险种";
	diag.URL = "LineTeam/RiskTypeManageDialog.jsp";
	diag.onLoad = function(){
		$DW.$("CodeType").value="${CodeType}";
		$DW.$("CodeType").disabled=true;
		$DW.$("CodeValue").focus();
...
function addSave(){
	...
	Server.sendRequest("com.sinosoft.platform.Code.Riskadd",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				$D.close();
				DataGrid.loadData('dg1');
...
function del(){
	...
	dc.add("DT",dt);
	Dialog.confirm("删除险种，险种对应的结算费率也会删除，您确认要删除吗？",function(){
		Server.sendRequest("com.sinosoft.platform.Code.Riskdel",dc,function(response){
			Dialog.alert(response.Message,function(){
				if(response.Status==1){
					DataGrid.loadData('dg1');
				}		
...
function afterRowDragEnd(type,targetDr,sourceDr,rowIndex,oldIndex){
	...
	Server.sendRequest("com.sinosoft.platform.Code.sortColumn",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				DataGrid.loadData("dg1");
			}
		...
		<tr valign="top">
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="6" class="blockTable">
				...
				<tr>
					...
					<z:datagrid id="dg1"
						method="com.sinosoft.platform.Code.dg1RiskTypeManage">
						<table width="100%" cellpadding="2" cellspacing="0"
							...
							<tr ztype="head" class="dataTableHead">
								...
								<td width="15%"><b>险种名称</b></td>
								<td width="15%"><strong>险种编码</strong></td>
								<td width="15%"><strong>险种分类</strong></td>
							</tr>
