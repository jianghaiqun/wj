...
function add(){
	...
	diag.Height = 270;
	diag.Title = "添加线下出单";
	diag.URL = "LineTeam/LineTeamPolicyAcceptInfoAdd.jsp";
	diag.OKEvent = addSave;
	diag.ShowMessageRow = true;
	diag.MessageTitle = "添加承包数据";
	diag.show();
}
/**
* 保存添加数据
...
function addSave(){
	...
	Server.sendRequest("com.wangjin.lineteam.LineTeamPolicyInfo.add", dc, function(response) {
		Dialog.endWait();
...
function del(){
	...
	Dialog.confirm("确认要删除？", function() {
		...
		dc.add("IDs", arr.join());
		Server.sendRequest("com.wangjin.lineteam.LineTeamPolicyInfo.delete", dc,
				function(response) {
					Dialog.alert(response.Message, function() {
						if (response.Status == 1) {
							DataGrid.loadData("dg1");
...
function cancelSave(){
	...
	Server.sendRequest("com.wangjin.lineteam.LineTeamPolicyInfo.cancelOrder", dc, function(response) {
		Dialog.endWait();
		Dialog.alert(response.Message, function() {
			if (response.Status == 1) {
				DataGrid.loadData("dg1");
...
function batchadd(){
	...
	diag.Height = 300;
	diag.Title = "批量添加";
	diag.URL = "LineTeam/LineTeam_batchAcceptDialog.jsp";
	diag.show();
	diag.OKButton.hide();
					...
					<td>
						<z:init method="com.wangjin.lineteam.LineTeamPolicyInfo.init">
							...
							<table>
								...
								<tr>
									<td colspan="8" nowrap><z:tButtonPurview>${_LineTeam_LineTeamAcceptInfo_Button}</z:tButtonPurview></td>
								</tr>
							</table>
				        </z:init>
					</td>
				...
				<td style="padding: 0px 0px;">
				 <table width="100%" border="0" cellspacing="0" cellpadding="6" class="blockTable" style="table-layout: fixed;" >
				  <tr>
					...
					<z:datagrid id="dg1" method="com.wangjin.lineteam.LineTeamPolicyInfo.dg1DataBindAccept" size="15" scroll="true" lazy="true">
