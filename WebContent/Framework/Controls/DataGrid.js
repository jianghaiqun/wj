...
DataGrid.getAllData = function (ele) {
    ele = $(ele);
    var ds = ele.DataSource;
    var values = [];
    for (var i = 1; i < ele.rows.length; i++) {
        if (ds.Values[i - 1] != undefined && ds.Values[i - 1] != "" && ds.Values[i - 1] != null) {
            values.push(ds.Values[i - 1]);
        }
    }
    var dt = new DataTable();
...
DataGrid.getColumnInfo = function (ele) {
    ...
    for (var i = 0; i < headTR.cells.length; i++) {
        ...
        var text = td.innerText || td.textContent;//解决360极速模式下，路径层级过多导致innerText失效问题
        if (text) {//没有标题的列不要
...
DataGrid.selectedRowToExcel = function (ele) {
    ...
    DataGrid.toExcel(ele, false, arr);
    // 根据页面标志判断是否获取全局数据；由于此方法只有选中下载使用，修改为则默认只查询当前页面
...
DataGrid.showLoading = function (ele) {
    ...
    if (!bgdiv) {
        ...
        $E.hide(icondiv);
        icondiv.innerHTML = "　<img src='" + Server.ContextPath + "Framework/Images/loadingGreen15px.gif'><font color=green> 正在加载......　</font>";
...
DataGrid.loadData = function (ele, func, dc) {
    ...
    Server.sendRequest("com.sinosoft.framework.controls.DataGridPage.doWork", dc, function (response) {
        try {
...
DataGrid._onContextMenu = function (tr, evt) {
    ...
    if (dg.onContextMenu) {
        ...
        menu.addItem("创建统计计划", function () {
            DataGrid.createStatisticsPlan(id);
        });
        menu.show();
    }
    stopEvent(evt);
...
DataGrid.deleteRow = function (ele, index) {
    ...
    /*modify by cuishigang wap站需求，删除datagrid一行
     * alert(ele.rows.length);
     if(ele.isPageFlag()){
    ...
    for (var i = 0; i < headTR.cells.length; i++) {
        ...
        if (ztype && ztype.toLowerCase() == "selector") {
            ...
            if (ztype && ztype.toLowerCase() == "tree") {
                ...
                if (level < nextLevel) {
                    if (nextVisiable) {
                        arr.push("<img src='" + Server.ContextPath + "Framework/Images/butExpand.gif' onclick='DataGrid.treeClick(this)'/>&nbsp;");
                    } else {
                        arr.push("<img src='" + Server.ContextPath + "Framework/Images/butCollapse.gif' onclick='DataGrid.treeClick(this)'/>&nbsp;");
                    }
                } else {
                    arr.push("<img src='" + Server.ContextPath + "Framework/Images/butNoChild.gif'/>&nbsp;");
                }
            }
...
DataGrid.dragEnd = function (evt) {
    ...
    if (ni != si && afterDrag) {
        ...
        var sourceDr = ds.Rows[ni - 1];
        func.apply(row, [type, targetDr, sourceDr, ni, si]);
    }
}
...
DataGrid.createStatisticsPlan = function (ele) {
    ele = $(ele);
    var diag = new Dialog("选择要创建的统计计划", "Framework/Controls/DataGridCreatePlanDialog.html", 500, 150);
    diag.OKEvent = function () {
        var planName=$DW.$NV("plan_name");
        if(!planName || planName==''){
            Dialog.alert("请详细填写您的统计计划名称!");
            return;
        }
        ...
        dc.add(prefix+"columnIndexes", columnIndexes);
        dc.add(prefix+"columnWidths", columnWidths);
        dc.add(prefix+"period", $DW.$NV("period"));
        dc.add(prefix+"plan_name", $DW.$NV("plan_name"));

        var inputs = ele.Params.keys;
        for (var i = 0; i < inputs.length; i++) {
            inputId = prefix + inputs[i];
            inputValue = DataGrid.getParam(ele, inputs[i]);
            dc.add(inputId, inputValue);
        }
        inputId = prefix + "_ZVING_Widths";
        inputValue = columnWidths.join();
        dc.add(inputId, inputValue);
        inputId = prefix + "_ZVING_Indexes";
        inputValue = columnIndexes.join();
        dc.add(inputId, inputValue);

        Server.sendRequest("com.sinosoft.cms.plan.statistics.OperateStatisticsPlan.createStatisticsPlan", dc,
            function (response) {
