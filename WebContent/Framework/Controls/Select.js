...
Selector.setSelectedIndex = function(ele,index){
	...
	if(oldValue != item.$A("value")){
		...
		}catch(ex){alert("Selector.setSelectedIndex():"+ex.message);};
	}
}
...
Selector.onItemClick = function(ele,flag){//注意，此方法是从未引用Main.js的iframe中调用
	...
	if(oldValue!=ele.getAttribute("value")){
		...
		}catch(ex){alert("Selector.onItemClick():"+ex.message);};
	}
	Selector.close();
}
...
Selector.setReturn = function(t,v,scrollTop){
	...
	if(oldValue!=v){
		try{
			Selector.invokeOnChange(div);
		}catch(ex){alert("Selector.setReturn():"+ex.message);};
	}
...
Selector.loadData = function(ele,func){
	...
	Server.sendRequest("com.sinosoft.framework.controls.CodeSourcePage.getData",ele.Params,function(response){
...
Selector.loadData2 = function(ele ,MenuID ,func){
	if(!ele.Params){
		ele.Params = new DataCollection();
	}
	if(!ele.$A("code")&&!ele.$A("method")){
	...
	//ele.Params.add("ConditionValue", ConditionValue );
	ele.Params.add("ConditionValue", MenuID);
