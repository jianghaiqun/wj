...
Application.onMainMenuClick = function(ele,flag){//flag仅在回退/前进时置为true
	...
	if(!menu){
		...
		for(var i=0;i<arr.length;i++){
			...
			str.push("<div id='"+id+"' class='divtab' onClick='Application.onChildMenuClick(this)' onMouseOver='Application.onChildMenuMouseOver(this)' onMouseOut='Application.onChildMenuMouseOut(this)' url='"+arr[i][2]+"' crossorigin='"+arr[i][4]+"'><img src='"+arr[i][3]+"' /><b><span>"+arr[i][1]+"</span></b></div>");
		}
...
Application.onChildMenuClick = function(ele,flag){//flag仅在回退/前进时置为true
	...
	if(!flag){
		...
		var crossorigin = ele.getAttribute("crossorigin");
		//如果是跨域链接则拼装跨域参数
		if(crossorigin == "1"){
			if(url.substring(url.length-1) != "?"){
				url += "&";
			}
			url += "channelsn=wj&extUser="+user+"&authCode="+authCode;
		}
		$("_MainArea").src = url;
	}
	Tab.onTabClick(ele);
...
Application.onParentSiteChange = function(){//_SiteSelector
	if($("_Navigation")){
  	...
  	Cookie.set("Resource.LastAttachLib","0","2100-01-01");
		Server.sendRequest("com.sinosoft.platform.Application.changeSite",dc,function(){
...
Application.logout = function(){
	Dialog.confirm("您确认要退出系统吗？",function(){
	   Server.sendRequest("com.sinosoft.platform.Application.logout");
	});	
}
