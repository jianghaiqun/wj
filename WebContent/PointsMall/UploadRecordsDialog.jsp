...
<script type="text/javascript">
	...
	function afterUpload(fileUrl){
		   var dc = new DataCollection();	
		    ...
		    dc.add("GiftID",document.getElementById("GiftID").value);
		    var TYPE=document.getElementById("TYPE").value;
		    Server.sendRequest("com.wangjin.pointsMall.GiftManage.uploadGoodsStock0"+TYPE,dc,function(response){
		    	Dialog.closeEx();
...
<form  action="BatchUploadSave.jsp" method="post" name="fm" enctype="multipart/form-data" target="targetFrame"> <br>
	...
	<a href="../DataService/ADTemplate/<%=request.getParameter("TemplateName")%>">库存信息导入模板下载</a>
</form>
	<input name="ip" type="hidden" value="<%=request.getRemoteAddr()%>" style="width: 150px" class="inputText" id="ip" />
	<input type="hidden" value="<%=request.getParameter("GiftID")%>" id="GiftID" name="GiftID">
	<input type="hidden" value="<%=request.getParameter("TYPE")%>" id="TYPE" name="TYPE">
</div>
</body>
</html>