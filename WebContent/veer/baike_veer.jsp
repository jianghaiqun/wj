...
<%
	...
	if(StringUtil.isEmpty(url) || url.indexOf("index.shtml") != -1){
		title = PublicHead.queryIndexTitle("baike/");
		PublicHead.getListMkMd(url);
		url = "/wwwroot/kxb/baike/index.shtml";
		state = "1";
	}else if(url.indexOf("index_") != -1){
		title = PublicHead.queryListTitle("baike/"+url);
		PublicHead.getListMkMd(url);
		url = "/wwwroot/kxb/baike/"+url;
		state = "2";
	}else if(url.indexOf(".shtml") == -1){
		title = PublicHead.queryListTitle("baike/"+url);
		PublicHead.getListMkMd(url);
		url = "/wwwroot/kxb/baike/"+url+"index.shtml";
		state = "2";
	}else{
...
<!-- 百科1.0.1 -->
	...
	<%
		...
		if("1".equals(state)){
				...
				<link rel="canonical" href="<%=frontServerContextPath %>/baike/" />
		<% 	
			//百科列表页
		}else if("2".equals(state)){
			%>
				<link href="<%=frontServerContextPath %>/style/new_information.css" rel="stylesheet" type="text/css" />
				<link rel="canonical" href="<%=frontServerContextPath %>/baike/<%=oldUrl %>" />
			<%
		}else{
			%>
				<link href="<%=frontServerContextPath %>/style/new_information.css" rel="stylesheet" type="text/css" />
