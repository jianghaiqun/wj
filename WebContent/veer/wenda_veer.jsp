...
<%
	...
	String state = "";
		...
		if(StringUtil.isEmpty(url) || url.indexOf("index.shtml") != -1){
			title = PublicHead.queryIndexTitle("wenda/");
			PublicHead.getListMkMd(url);
			url = "/wwwroot/kxb/wenda/index.shtml";
			state = "1";
		}else if(url.indexOf("list") != -1){
			//提问列表页特殊处理
			title = PublicHead.queryListTitle("wenda/"+url);
			PublicHead.getListMkMd(url);
			url = "/wwwroot/kxb/wenda/"+url;
			state = "2";
			...
			title = PublicHead.queryListTitle("wenda/"+url);
			PublicHead.getListMkMd(url);
			url = "/wwwroot/kxb/wenda/"+url+"index.shtml";
			state = "2";
		}else{
	...
	<!-- 问答1.0.1 -->
		...
		<%
			...
			if("1".equals(state)){
					...
					<link rel="canonical" href="<%=frontServerContextPath %>/wenda/" />
			<% 	
				//问答列表页
			}else if("2".equals(state)){
				%>
					<link href="<%=frontServerContextPath %>/style/new_information.css" rel="stylesheet" type="text/css" />
					<link rel="canonical" href="<%=frontServerContextPath %>/wenda/<%=oldUrl %>" />
				<%
			}else{
				%>
					<link href="<%=frontServerContextPath %>/style/new_information.css" rel="stylesheet" type="text/css" />
