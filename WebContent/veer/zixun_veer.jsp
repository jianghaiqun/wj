...
<%
	...
	if(StringUtil.isEmpty(url) || url.indexOf("index.shtml") != -1){
		...
		title = PublicHead.queryIndexTitle("zixun/");
		url = "/wwwroot/kxb/zixun/index.shtml";
		state = "1";
	}else if(url.indexOf("index_") != -1){
		title = PublicHead.queryListTitle("zixun/"+url);
		PublicHead.getListMkMd(url);
		url = "/wwwroot/kxb/zixun/"+url;
		state = "2";
	}else if(url.indexOf(".shtml") == -1){
		title = PublicHead.queryListTitle("zixun/"+url);
		PublicHead.getListMkMd(url);
		url = "/wwwroot/kxb/zixun/"+url+"index.shtml";
		state = "2";
	}else{
...
<!-- 资讯1.0.1 -->
	...
	<%
		...
		if("1".equals(state)){
				...
				<link rel="canonical" href="<%=frontServerContextPath %>/zixun/" />
		<% 	
			//资讯列表页
		}else if("2".equals(state)){
			%>
				<link href="<%=frontServerContextPath %>/style/new_information.css" rel="stylesheet" type="text/css" />
				<link rel="canonical" href="<%=frontServerContextPath %>/zixun/<%=oldUrl %>" />
			<%
		}else{
			%>
				<link href="<%=frontServerContextPath %>/style/new_information.css" rel="stylesheet" type="text/css" />
