...
function Save(){
	...
	Server.sendRequest("com.sinosoft.platform.CustomColumnItem.save",dc,function(response){
...
function del(){
	...
	if(!window.confirm("确定要删除该字段吗？")){
		...
		dc.add("IDs",arr.join());
		Server.sendRequest("com.sinosoft.platform.CustomColumnItem.del",dc,function(response){
			if(response.Status==1){
				alert("删除成功");
...
function CopyToSave(){
	var dc = $DW.Form.getData("form2");
	dc.add("IDs",$DW.$V("IDs"));
	Server.sendRequest("com.sinosoft.platform.CustomColumnItem.CopyToSave",dc,function(response){
		if(response.Status==1){
			alert(response.Message);
			$D.close();
		}
...
<body scroll="no">
<z:init method="com.sinosoft.platform.CustomColumnItem.init">
...
<div id="maincontent">
<z:datagrid id="dg1" method="com.sinosoft.platform.CustomColumnItem.dg1DataBind" size="20" editAction="showEditDialog">
