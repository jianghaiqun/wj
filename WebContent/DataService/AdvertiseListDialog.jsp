...
function del(){
	...
	Dialog.confirm("确定要删除广告吗？",function(){
		...
		dc.add("IDs",arr.join());
		Server.sendRequest("com.sinosoft.cms.dataservice.Advertise.del",dc,function(response){
			if(response.Status==1){
...
function start(){
	...
	var dc = new DataCollection();
	dc.add("ID",arr[0]);
	Server.sendRequest("com.sinosoft.cms.dataservice.Advertise.start",dc,function(response){
		if(response.Status==1){
...
function afterRowDragEnd(type,targetDr,sourceDr,rowIndex,oldIndex){
	...
	Server.sendRequest("com.sinosoft.cms.dataservice.Advertise.sortAdvertise",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				DataGrid.loadData("dg1");
			}
...
<table width="100%" border="0" cellspacing="6" cellpadding="0" style="border-collapse: separate; border-spacing: 6px;">
  <tr valign="top">
    ...
    <z:init method="com.sinosoft.cms.dataservice.Advertise.init">
      <tr>
      ...
      <tr>
        ...
        <z:datagrid id="dg1" method="com.sinosoft.cms.dataservice.Advertise.dg1DataBind" size="10">
