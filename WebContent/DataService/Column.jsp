...
function addSave(){
	...
	Server.sendRequest("com.sinosoft.cms.dataservice.Column.add",dc,function(response){
...
function editSave(){
	...
	dc.add("FormID",$V("FormID"));
	Server.sendRequest("com.sinosoft.cms.dataservice.Column.edit",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				$D.close();
				DataGrid.loadData("dg1");
...
function del(){
	...
	Dialog.alert("确定要删除该字段吗？",function(){
		...
		dc.add("IDs",arr.join());
		Server.sendRequest("com.sinosoft.cms.dataservice.Column.del",dc,function(response){
			Dialog.alert(response.Message,function(){
				if(response.Status==1){
					DataGrid.loadData("dg1");
				}
...
function afterRowDragEnd(rowIndex,oldIndex){
	...
	Server.sendRequest("com.sinosoft.cms.dataservice.Column.sortColumn",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				DataGrid.loadData("dg1");
			}
...
function createTb(){
	 var dc = new DataCollection();
	 dc.add("FormID",$V("FormID"))
		Server.sendRequest("com.sinosoft.cms.dataservice.FormTable.create",dc,function(response){
			Dialog.alert(response.Message,function(){
				if(response.Status==1){
					$D.close();
				}
...
<z:datagrid id="dg1"
			method="com.sinosoft.cms.dataservice.Column.dg1DataBind">
