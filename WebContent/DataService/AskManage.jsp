...
function addSaveTeam(){
	...
	Server.sendRequest("com.sinosoft.cms.dataservice.QuestionTeam.add",dc,function(response){
...
function delTeam(param){
	...
	Dialog.confirm("确认删除 <b style='color:#F00'>"+Name+"</b> 分组吗？",function(){
		...
		Server.sendRequest("com.sinosoft.cms.dataservice.QuestionTeam.del",dc,function(response){
			Dialog.alert(response.Message,function(){
				if(response.Status==1){
					window.parent.parent.location.reload();
				}
...
function editSaveTeam(){
	...
	Server.sendRequest("com.sinosoft.cms.dataservice.QuestionTeam.edit",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				$D.close();
...
function addSave(){
	...
	Server.sendRequest("com.sinosoft.cms.dataservice.Question.add",dc,function(response){
...
function editSave(){
	...
	Server.sendRequest("com.sinosoft.cms.dataservice.Question.edit",dc,function(response){
		Dialog.alert(response.Message);
		if(response.Status==1){
				$D.close();
				var QuestionInnerCode = Tree.CurrentItem.getAttribute("cid");
...
function del(){
	...
	Dialog.confirm("此操作会删除分类下的问题信息,确定要删除吗？",function(){
		var dc = new DataCollection();	
		dc.add("IDs",arr.join());
		Server.sendRequest("com.sinosoft.cms.dataservice.Question.del",dc,function(response){
  ...
  <tr valign="top">
    <td width="180"><table oncontextmenu="return false;" width="180" border="0" cellspacing="0" cellpadding="6"
        ...
        <tr>
          <td style="padding:6px;" class="blockTd"><z:tree id="tree1"
					style="height:430px" method="com.sinosoft.cms.dataservice.QuestionTeam.treeDataBind">
    ...
    <td><table width="100%" border="0" cellspacing="0" cellpadding="6" class="blockTable">
        ...
        <tr>
		  ...
		  <z:datagrid id="dg1" method="com.sinosoft.cms.dataservice.Question.dg1DataBind" size="15">
