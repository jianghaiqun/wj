...
function add(){
	...
	diag.Height = 460;
	diag.Title = "新建广告";
	diag.URL = "DataService/AdvertiseManageAddDialog.jsp?RelaID="+$V("RelaID")+"&Type=New";
...
function addSave(){
	...
	Server.sendRequest("com.sinosoft.cms.dataservice.TopAdInformation.add",dc,function(response){
...
function editAD(){
	...
	diag.Height = 460;
	diag.Title = "编辑广告内容";
	diag.URL = "DataService/AdvertiseManageAddDialog.jsp?RelaID="+$V("RelaID")+"&ID="+arr[0];
	diag.OKEvent = addSave;
	diag.show();
}
...
function del(){
	...
	Dialog.confirm("确定要删除广告吗？",function(){
		...
		dc.add("IDs",arr.join());
		Server.sendRequest("com.sinosoft.cms.dataservice.TopAdInformation.del",dc,function(response){
			if(response.Status==1){
...
function start(){
	...
	var dc = new DataCollection();
	dc.add("ID",arr[0]);
	Server.sendRequest("com.sinosoft.cms.dataservice.TopAdInformation.start",dc,function(response){
		if(response.Status==1){
...
function afterRowDragEnd(type,targetDr,sourceDr,rowIndex,oldIndex){
	...
	Server.sendRequest("com.sinosoft.cms.dataservice.TopAdInformation.sortAdvertise",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				DataGrid.loadData("dg1");
			}
...
<table width="100%" border="0" cellspacing="6" cellpadding="0" style="border-collapse: separate; border-spacing: 6px;">
  <tr valign="top">
    ...
    <z:init method="com.sinosoft.cms.dataservice.TopAdInformation.init">
      <tr>
        <td valign="middle"><img src="../Icons/icon010a1.gif" align="absmiddle"/> 
        “<b>${RelaName}</b>” 下的广告
        <input id="RelaID" name="RelaID" type="hidden" value="${RelaID}"/>
        </td>
      </tr>
    </z:init>
      <tr>
        ...
        <z:tbutton onClick="start();"><img src="../Icons/icon010a8.gif" />启用/停用</z:tbutton>
      ...
      <tr>
        ...
        <z:datagrid id="dg1" method="com.sinosoft.cms.dataservice.TopAdInformation.dg1DataBind" size="10">
