...
function showADContent() {
  ...
  var str = "<div id='ZCMSAD_"+this.PosID+"_Box' style='width:"+this.Width+"px; height:"+this.Height+"px;border:1px solid #E1E1E1;overflow:hidden;position:relative;'>";
  var AD = eval('('+content+')');
  if (this.ADType == "image") {
	  for(var i=0;i<AD.ADImage.length;i++){
		  str += "<a href='"+AD.ADImage[i].imgADLinkUrl+"' onClick='adClick(\""+AD.ADImage[i].imgID+"\",\""+AD.ADImage[i].imgADLinkUrl+"\")' target='"+((AD.ADImage[i].imgADLinkTarget == "Old") ? "_self" : "_blank") + "'>";
	  	  str += "<img alt='"+AD.ADImage[i].imgADAlt+"' title='"+AD.ADImage[i].imgADAlt+"' src='"+this.UploadFilePath+AD.ADImage[i].ImgPath+"' width='"+this.Width+"' height='"+this.Height+"' style='position:absolute;border:0px;'/>";
	  	  str += "</a>";
	  }
  }
  str += "<div id='ZCMSAD_"+this.PosID+"_Tip' style='width:"+this.Width+"px; height:"+this.Height+"px;position:absolute;height:20px;padding:5px;font-family:Arial;color:#fff;font-weight:bold;bottom:-1;left:0;z-index:999997;filter:alpha(opacity=60);opacity:0.6;background-color:#333333'></div>"
  str += "</div>";
  document.write(str);
}
