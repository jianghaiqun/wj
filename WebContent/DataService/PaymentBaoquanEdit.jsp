...
<script type="text/javascript">
	...
	function addRemark() {
		...
		Server.sendRequest("com.wangjin.cms.payment.MemberPayment.checkRemark",dc,function(){
	...
	function add(){
		...
		var dc = Form.getData("fm_reply");
		Server.sendRequest("com.wangjin.cms.payment.MemberPayment.save",dc,function(response){
	...
	function delRemark(){
		...
		Dialog.confirm("确定要删除此记录吗？",function(){
			...
			dc.add("prop1",prop1);
			dc.add("id",id);
			Server.sendRequest("com.wangjin.cms.payment.MemberPayment.del",dc,function(response){
	...
	function saveRemark(){
		...
		var dc = Form.getData("fm_reply");
		Server.sendRequest("com.wangjin.cms.payment.MemberPayment.update",dc,function(response){
			Dialog.alert(response.Message,function(){
			if(response.Status==1){
				DataGrid.loadData("dg1");
				document.getElementById("remarkdefault").value=response.get("Group");;
...
<body class="dialogBody">
		<table width="100%" border="0" cellspacing="0" cellpadding="3"
			...
			<tr>
				<td
					...
					<z:datagrid id="dg1"
						method="com.wangjin.cms.payment.MemberPayment.baoquanInquery" size="5"
						scroll="false">
		...
		<z:init method="com.wangjin.cms.payment.MemberPayment.baoquanModi">
          <form name="fm_reply" id="fm_reply">
			<table width="100%" cellpadding="2" cellspacing="0"
				...
				<tr height="200px">
					...
					<td>
					<input type="hidden" id="prop1" value="${prop1}" /> 
