...
Page.onLoad(function(){
	$("dg1").afterEdit = function(tr,dr){
		...
		dr.set("Type",$V("Type"));
		dr.set("Name",$V("Name"));
		if($V("Type")=="ResetPassword"){
			if($V("Value").length>=8){
				if(/[a-z]/.test($V("Value"))&&/[A-Z]/.test($V("Value"))&&/\d/.test($V("Value"))){
			    	dr.set("Value",$V("Value"));
			    }else{
			    	Dialog.alert("密码必须由大小写字母和数字组成!");
			    }
			}else{
				Dialog.alert("密码至少为8位!");
			}
		}else{
			dr.set("Value",$V("Value"));
		}
...
function addSave(){
	...
	if(!/^[\w\-\.]+$/.test($DW.$V("Type"))){
		Dialog.alert("代码和名称中含有不规则字符\",\^\"");
		return;
	}
	Server.sendRequest("com.sinosoft.platform.ConfigSys.add",dc,function(response){
...
function save(){
	DataGrid.save("dg1","com.sinosoft.platform.ConfigSys.dg1Edit",function(){DataGrid.loadData('dg1');});
}
...
function del(){
	...
	Dialog.confirm("确定要删除该配置项吗？",function(){
		...
		dc.add("IDs",arr.join());
		Server.sendRequest("com.sinosoft.platform.ConfigSys.del",dc,function(response){
			Dialog.alert(response.Message,function(){
				if(response.Status==1){
					DataGrid.loadData('dg1');
				}
      ...
      <tr valign="top">
        <td><table width="100%" border="0" cellspacing="0" cellpadding="6" class="blockTable">
            ...
            <tr>
			  ...
			  <z:datagrid id="dg1" method="com.sinosoft.platform.ConfigSys.dg1DataBind" size="15">
                <table width="100%" cellpadding="2" cellspacing="0" class="dataTable">
                  <tr ztype="head" class="dataTableHead">
                    <td  width="4%" ztype="RowNo"><b>序号</b></td>
                    <td width="3%" ztype="selector" field="type">&nbsp;</td>
                    <td width="24%"><b>配置项代码</b></td>
