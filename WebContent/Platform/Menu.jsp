...
function save(){
	DataGrid.save("dg1","com.sinosoft.platform.Menu.dg1Edit",function(){DataGrid.loadData("dg1",initGrid);});
}
...
function addSave(){
	...
	Server.sendRequest("com.sinosoft.platform.Menu.add",dc,function(response){
...
function del(){
	...
	Dialog.confirm("删除后不可恢复，确认要删除？",function(){
		...
		dc.add("IDs",arr.join());
		Server.sendRequest("com.sinosoft.platform.Menu.del",dc,function(response){
			Dialog.alert(response.Message,function(){
				if(response.Status==1){
					DataGrid.loadData("dg1",initGrid);
				}
...
function sortMenu(type,targetDr,sourceDr,rowIndex,oldIndex){
	...
	Server.sendRequest("com.sinosoft.platform.Menu.sortMenu",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				DataGrid.loadData("dg1");
			}
...
<z:datagrid id="dg1" method="com.sinosoft.platform.Menu.dg1DataBind" page="false">
              <table width="100%" cellpadding="2" cellspacing="0" class="dataTable" afterdrag="sortMenu">
                <tr ztype="head" class="dataTableHead">
                  ...
                  <td width="7%" ztype="checkbox" checkedvalue="Y" field="visiable"><b>是否显示</b></td>
