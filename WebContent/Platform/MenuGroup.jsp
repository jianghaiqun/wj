...
function addSave(){
	...
	dc.add("IDs",arr.join());
	Server.sendRequest("com.sinosoft.platform.MenuGroup.add",dc,function(response){
...
function del(){
	...
	dc.add("IDs",arr.join());
	Server.sendRequest("com.sinosoft.platform.MenuGroup.del",dc,function(response){
		alert(response.Message);
		if(response.Status==1){
			window.location="MenuGroup.jsp";
		}
...
function save(){
	...
	dc.add("IDs",arr.join());
	Server.sendRequest("com.sinosoft.platform.MenuGroup.save",dc,function(response){
		alert(response.Message);
		if(response.Status==1){
			$D.close();
			window.location="MenuGroup.jsp";
...
<div id="maincontent">
<z:datagrid id="dg1" method="com.sinosoft.platform.MenuGroup.dg1DataBind"  editAction="showEditDialog" size="20">
