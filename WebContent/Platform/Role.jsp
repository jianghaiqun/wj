...
var tabMap = {
	...
	MenuButton:"RoleTabButton.jsp?4=4"
};
...
function onRoleTabClick(tabid){
	...
	// 按钮权限每次都重新刷新页面
	if(tabid == 'MenuButton' || URL.indexOf(newURL)<0){
...
function addSave(){
	...
	Server.sendRequest("com.sinosoft.platform.RoleTabBasic.add",dc,function(response){
...
function del(param){
	...
	Dialog.confirm("确认删除 <b style='color:#F00'>"+RoleCode+"</b> 角色吗？",function(){
		var dc = new DataCollection();
		dc.add("RoleCode",param);
		Server.sendRequest("com.sinosoft.platform.RoleTabBasic.del",dc,function(response){
			Dialog.alert(response.Message,function(){
				if(response.Status==1){
...
function editSave(){
	...
	Server.sendRequest("com.sinosoft.platform.RoleTabBasic.save",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				$D.close();
...
<table width="100%" border="0" cellspacing="6" cellpadding="0" style="border-collapse: separate; border-spacing: 6px;">
	<tr valign="top">
		...
		<table width="180" border="0" cellspacing="0" cellpadding="6" class="blockTable">
			<tr>
				<td style="padding:6px;" class="blockTd">
				   <z:tree id="tree1" style="height:430px;width:220px;" method="com.sinosoft.platform.Role.treeDataBind">
		...
		<td><z:tab>
			...
			<z:childtab id="MenuButton" src="" afterClick="onRoleTabClick('MenuButton')">
				<img src="../Icons/icon018a1.gif" />
				<b>按钮权限</b>
			</z:childtab>
		</z:tab></td>
