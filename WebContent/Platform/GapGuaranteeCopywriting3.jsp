...
function update(){
	...
	diag.Height = 150;
	diag.Title = "文案修改";
	diag.URL = "Platform/GapGuaranteeCopywriting3Edit.jsp?codeValue="+dr.get("codeValue");
...
function updateThis(){
	...
	Server.sendRequest("com.sinosoft.platform.GapGuarantee.update3",dc,function(response){
...
function add(){
	...
	diag.Height = 150;
	diag.Title = "新建文案";
	diag.URL = "Platform/GapGuaranteeCopywriting3Edit.jsp";
	diag.onLoad = function(){
		$DW.$("CodeType").value="${CodeType}";
		$DW.$("CodeType").disabled=true;
		$DW.$("CodeValue").focus();
...
function addSave(){
	...
	Server.sendRequest("com.sinosoft.platform.GapGuarantee.add3",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				$D.close();
				DataGrid.loadData('dg1');
...
function del(){
	...
	Dialog.confirm("您确认要删除吗？",function(){
		Server.sendRequest("com.sinosoft.platform.GapGuarantee.del3",dc,function(response){
			Dialog.alert(response.Message,function(){
				if(response.Status==1){
					DataGrid.loadData('dg1');
				}		
		...
		<tr valign="top">
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="6" class="blockTable">
				...
				<tr>
					...
					<z:datagrid id="dg1"
						method="com.sinosoft.platform.GapGuarantee.dg3HomePage3">
						<table width="100%" cellpadding="2" cellspacing="0"
							...
							<tr ztype="head" class="dataTableHead">
								...
								<td width="15%"><b>家庭类型</b></td>
								<td width="15%"><b>主人性别</b></td>
								<td width="15%"><b>年龄范围</b></td>
								<td width="15%"><strong>保障缺口测评结果页标签文案</strong></td>
							</tr>
							<tr style1="background-color:#FFFFFF"
								...
								<td><input name="prop1"  type="hidden"  id="prop1" value ='${prop1}' /></td>
								<td><input name="prop2"  type="hidden"  id="prop2" value ='${prop2}' /></td>
							</tr>
						</table></z:datagrid></td>
				</tr>
			</table>
