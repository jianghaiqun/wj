...
function update(){
	...
	diag.Height = 150;
	diag.Title = "文案修改";
	diag.URL = "Platform/GapGuaranteeCopywriting1Edit.jsp?codeValue="+dr.get("codeValue");
	diag.onLoad = function(){
		$DW.$("CodeType").value="${CodeType}";
		$DW.$("CodeName").value=dr.get("CodeName");
		$DW.$("CodeValuebefore").value=dr.get("CodeValuebefore");
		$DW.$("CodeValueafter").value=dr.get("CodeValueafter");
		
		$DW.$("CodeNameOld").value=dr.get("CodeName");
		$DW.$("CodeValuebeforeOld").value=dr.get("CodeValuebefore");
		$DW.$("CodeValueafterOld").value=dr.get("CodeValueafter");
		
		$DW.$("CodeType").disabled=true;
		$DW.$("CodeValuebefore").focus();
	};
...
function updateThis(){
	...
	Server.sendRequest("com.sinosoft.platform.GapGuarantee.update1",dc,function(response){
...
function add(){
	...
	diag.Height = 150;
	diag.Title = "新建文案";
	diag.URL = "Platform/GapGuaranteeCopywriting1Edit.jsp";
	diag.onLoad = function(){
		$DW.$("CodeType").value="${CodeType}";
		$DW.$("CodeType").disabled=true;
		$DW.$("CodeValuebefore").focus();
...
function addSave(){
	...
	Server.sendRequest("com.sinosoft.platform.GapGuarantee.add1",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				$D.close();
				DataGrid.loadData('dg1');
...
function del(){
	...
	Dialog.confirm("您确认要删除吗？",function(){
		Server.sendRequest("com.sinosoft.platform.GapGuarantee.del1",dc,function(response){
			Dialog.alert(response.Message,function(){
				if(response.Status==1){
					DataGrid.loadData('dg1');
				}		
		...
		<tr valign="top">
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="6" class="blockTable">
				...
				<tr>
					...
					<z:datagrid id="dg1"
						method="com.sinosoft.platform.GapGuarantee.dg1HomePage1">
						<table width="100%" cellpadding="2" cellspacing="0"
							...
							<tr ztype="head" class="dataTableHead">
								...
								<td width="15%"><b>收入范围</b></td>
								<td width="40%"><strong>我的十年存款能力文案</strong></td>
							</tr>
							<tr style1="background-color:#FFFFFF"
								...
								<td>${Codename1}</td>
								<td>${CodeValue}</td>
								<td><input name="CodeValuebefore"  type="hidden"  id="CodeValuebefore" value ='${CodeValuebefore}' /></td>
								<td><input name="CodeValuebefore"  type="hidden"  id="CodeValueafter" value ='${CodeValueafter}' /></td>
								<td><input name="Codename"  type="hidden"  id="Codename" value ='${Codename}' /></td>
							</tr>
						</table></z:datagrid></td>
				</tr>
			</table>
