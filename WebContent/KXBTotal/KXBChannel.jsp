...
function add(){
	...
	diag.Height = 200;
	diag.Title = "新建渠道";
	diag.URL = "KXBTotal/KXBChannelDialog.jsp";
...
function addSave(){
	...
	Server.sendRequest("com.sinosoft.platform.KXBChannel.add",dc,function(response){
...
function edit(){
	...
	diag.Height = 200;
	diag.Title = "渠道编辑" ;
	diag.URL = "KXBTotal/KXBChannelEditDialog.jsp?Id="+arr[0];
	diag.onLoad = function(){
		$DW.$("Name").focus();
	};
...
function editSave(){
	...
	Server.sendRequest("com.sinosoft.platform.KXBChannel.Edit",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				$D.close();
				DataGrid.loadData("dg1");
...
function del(){
	...
	Dialog.confirm("删除后不可恢复，确认要删除？",function(){
		...
		dc.add("IDs",arr.join());
		Server.sendRequest("com.sinosoft.platform.KXBChannel.del",dc,function(response){
			Dialog.alert(response.Message,function(){
				if(response.Status==1){
					DataGrid.loadData("dg1");
				}
    ...
    <tr valign="top">
      <td><table width="100%" border="0" cellspacing="0" cellpadding="6" class="blockTable">
          ...
          <tr>   <td style="padding:0px 5px;">
			<z:datagrid id="dg1" method="com.sinosoft.platform.KXBChannel.dg1DataBind" page="false">
