...
function sortLibSave(){
	...
	Server.sendRequest("com.sinosoft.cms.site.Catalog.sortCatalog",dc,function(response){
...
function changeSave(){
	...
	Server.sendRequest("com.sinosoft.cms.resource.AudioLib.AudioLibEdit",dc,function(response){
...
function addSave(){
	...
	Server.sendRequest("com.sinosoft.cms.resource.AudioLib.add",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
			Tree.loadData("tree1");
				$D.close();
...
function save(){
	DataGrid.save("dg1","com.sinosoft.cms.resource.AudioLib.dg1Edit",function(){
		DataGrid.loadData('dg1');
	});
}
...
function copySave(){
	...
	Server.sendRequest("com.sinosoft.cms.resource.Audio.copy",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				$D.close();
			}
...
function transferSave(){
	...
	dc.add("IDs",arr.join());
	dc.add("CatalogID",$DW.$V("CatalogID"));
	Server.sendRequest("com.sinosoft.cms.resource.Audio.transfer",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				$D.close();
				DataGrid.loadData("dg1");
...
function delLib(param){
	...
	Dialog.confirm("该分类下的音频都将被删除，你确认要删除此音频分类吗？",function(){
		...
		Server.sendRequest("com.sinosoft.cms.resource.AudioLib.delLib",dc,function(response){
		Dialog.alert(response.Message,function(){
				if(response.Status==1){
...
function del(){
	...
	Dialog.confirm("这些音频可能与文章或者音频播放器有关联，你确认要删除吗？",function(){
		var dc = new DataCollection();
		dc.add("IDs",arr.join());
		Server.sendRequest("com.sinosoft.cms.resource.Audio.del",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				DataGrid.loadData("dg1");
				}
...
function publish(){
	...
	var dc = new DataCollection();
	dc.add("IDs",arr.join());
	Server.sendRequest("com.sinosoft.cms.resource.AudioLib.publish",dc,function(response){
...
function afterRowDragEnd(type,targetDr,sourceDr,rowIndex,oldIndex){
	...
	Server.sendRequest("com.sinosoft.cms.resource.AudioLib.sortColumn",dc,function(response){
		if(response.Status==1){
			DataGrid.loadData("dg1");
		}else{
			Dialog.alert(response.Message);	
...
function DetailSave(){
	...
	Server.sendRequest("com.sinosoft.cms.resource.Audio.dialogEdit",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				$D.close();
				DataGrid.loadData("dg1");
		...
		<table width="180" border="0" cellspacing="0" cellpadding="1" class="blockTable">
			<tr>
				<td style="padding:6px;" class="blockTd">
					<z:tree id="tree1" style="height:450px;width:160px;" method="com.sinosoft.cms.resource.AudioLib.treeDataBind" level="2" lazy="true">
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="6" class="blockTable">
			<tr>
				...
				<div style="float: left"><z:tbutton id="addLibButton" priv="audio_manage" onClick="add()">
					...
					<img src="../Icons/icon051a13.gif" />重新上传</z:tbutton><z:tbutton id="editButton" priv="audio_modify" onClick="DetailEdit()">
					<img src="../Icons/icon051a4.gif" width="20" height="20" />编辑</z:tbutton>
					<z:tbutton id="editButton" priv="audio_modify" onClick="save()">
					<img src="../Icons/icon051a16.gif" width="20" height="20" />保存</z:tbutton> 
			...
			<tr>
				<td style="padding:4px 10px;">列出： 从 <input type="text" id="StartDate" style="width:100px; " ztype='date'> 至 <input type="text" id="EndDate" style="width:100px;" onblur="dateControl();"  ztype='date'> &nbsp;关键词: <input name="SearchName" type="text" id="SearchName" onFocus="delKeyWord();" style="width:110px"> <input type="button" name="Submitbutton" value="查询" onClick="doSearch()" id="Submitbutton"></td>
			</tr>
			<tr>
				<td style="padding:0px 5px;"><z:datagrid id="dg1" method="com.sinosoft.cms.resource.AudioLib.dg1DataBind" size="15">
					<table width="100%" cellpadding="2" cellspacing="0" class="dataTable" afterDrag="afterRowDragEnd">
						<tr ztype="head" class="dataTableHead">
							...
							<td width="30%"><b>音频名称</b></td>
							<td width="8%" align="right"><b>大小</b></td>
							<td width="12%"><b>标签</b></td>
							<td width="7%"><b>格式</b></td>
							<td width="8%"><b>创建者</b></td>
							<td width="14%"><b>上传时间</b></td>
							<td width="6%"><b>试听</b></td>
						</tr>
						...
						<tr ztype="edit">
							...
							<td>${AddUser}</td>
							<td>${AddTimeName}</td>
                            <td><a href="javascript:DetailEdit()">查看/编辑</a></td>
							<td>&nbsp;</td>
						</tr>
						<tr ztype="pagebar">
