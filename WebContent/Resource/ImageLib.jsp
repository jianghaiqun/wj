...
function setImageCover(imageID){
	...
	dc.add("ID",imageID);
	Server.sendRequest("com.sinosoft.cms.resource.ImageLib.setImageCover",dc,function(response){
...
function setTop(imageID){
	var dc = new DataCollection();
	dc.add("ID",imageID);
	Server.sendRequest("com.sinosoft.cms.resource.ImageLib.setTopper",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				if ($V("Name") != "请输入图片名称") {
...
function changeSave(){
	...
	Server.sendRequest("com.sinosoft.cms.resource.ImageLib.ImageLibEdit",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				Tree.loadData("tree1");
				$D.close();
...
function editSave(){
	var dc = $DW.Form.getData("form2");
	Server.sendRequest("com.sinosoft.cms.resource.Image.imagesEdit",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				$D.close();
				if ($V("Name") != "请输入图片名称") {
...
function copySave(){
	...
	Server.sendRequest("com.sinosoft.cms.resource.Image.copy",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				$D.close();
			}
...
function transferSave(){
	...
	dc.add("IDs",arr.join());
	dc.add("CatalogID",$DW.$V("CatalogID"));
	Server.sendRequest("com.sinosoft.cms.resource.Image.transfer",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				$D.close();
				DataList.setParam("dg1",Constant.PageIndex,0);
...
function delLib(param){
	...
	Dialog.confirm("该分类下的图片都将被删除，你确认要删除此图片分类吗？",function(){
		...
		Server.sendRequest("com.sinosoft.cms.resource.ImageLib.delLib",dc,function(response){
			Dialog.alert(response.Message,function(){
				if(response.Status==1){
...
function del(imageID){
	...
	Dialog.confirm("你确认删除这"+count+"张图片吗？",function(){
		var dc = new DataCollection();
		dc.add("IDs",imageID);
		Server.sendRequest("com.sinosoft.cms.resource.Image.del",dc,function(response){
			Dialog.alert(response.Message,function(){
				if(response.Status==1){
					if ($V("Name") != "请输入图片名称") {
						DataList.setParam("dg1","Name",$V("Name"));
...
function publish(){
	...
	var dc = new DataCollection();
	dc.add("IDs",arr.join());
	Server.sendRequest("com.sinosoft.cms.resource.ImageLib.publish",dc,function(response){
...
<table width="100%" border="0" cellspacing="6" cellpadding="0" style="border-collapse: separate; border-spacing: 6px;">
	<tr valign="top">
		...
		<table width="180" border="0" cellspacing="0" cellpadding="6" class="blockTable">
			<tr>
				<td style="padding:6px;" colspan="2" class="blockTd"><z:tree id="tree1" style="height:450px;width:160px;" method="com.sinosoft.cms.resource.ImageLib.treeDataBind" level="2" lazy="true" expand="false">
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0" class="blockTable">
			...
			<tr>
				<td style="padding:2px 10px;"><label> <input name="checkbox" type="checkbox" id="AllCheck" onClick="selectAll(this,'ImageID')"> 全选</label> &nbsp;&nbsp;列出： 从 <input type="text" id="StartDate" style="width:100px; " ztype='date'> 至 <input type="text" id="EndDate" style="width:100px; " ztype='date'> &nbsp;关键词: <input name="Name" type="text" id="Name" onFocus="delKeyWord();" style="width:110px"> <input type="button" name="Submitbutton" value="查询" onClick="doSearch()" id="Submitbutton"></td>
			</tr>
			<tr>
				<td style="padding:0 5px">
				<ul class="img-wrapper" style="height:380px; overflow:hidden; display:block; margin:0;">
					<z:datalist id="dg1" method="com.sinosoft.cms.resource.ImageLib.dg1DataList" size="14" page="true">
						<li>
						<dl>
