...
function onReturnBack(){
	...
	dc.add("PicID",arr[0]);
	Server.sendRequest("com.sinosoft.cms.resource.Image.getPicSrc",dc,function(response){
		path = response.get("ImagePath");
...
function editSave(){
	...
	Server.sendRequest("com.sinosoft.cms.resource.Image.edit",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				$D.close();
				DataList.setParam("dg1","Name",$V("Name"));
...
<table width="760" align="center" cellpadding="0" cellspacing="0">
	<tr>
		<td>
		<table width="100%" cellpadding="3" cellspacing="0" id="tbSearch">
			<tr>
				<td width="148" align="left">所属分类: <z:select id="CatalogID"
					style="width:80px" listWidth="200" listHeight="300"
					listURL="Resource/ImageLibSelectList.jsp"> </z:select></td>
				<td width="42">名称：</td>
				<td width="172"><input name="Name" id="Name" type="text" value=""></td>
				<td width="44">描述：</td>
				<td width="163"><input name="Info" id="Info" type="text" value=""></td>
				<td width="171">
                <z:tbutton id="addButton" onClick="doSearch()"><img src="../Icons/icon030a15.gif" />查询</z:tbutton>
                <z:tbutton id="returnButton" onClick="onReturnBack()"><img src="../Icons/icon030a15.gif" />确定</z:tbutton>
                </td>
			</tr>
		</table>
		</td>
	...
	<tr>
		...
		<ul class="img-wrapper"
			...
			<z:datalist id="dg1"
				method="com.sinosoft.cms.resource.ImageLib.dg1DataListBrowse" size="10"
				page="true">
