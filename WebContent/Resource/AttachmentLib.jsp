...
function changeSave(){
	...
	Server.sendRequest("com.sinosoft.cms.resource.AttachmentLib.AttachmentLibEdit",dc,function(response){
...
function addSave(){
	...
	Server.sendRequest("com.sinosoft.cms.resource.AttachmentLib.add",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				Tree.loadData("tree1");
				$D.close();
...
function save(){
	DataGrid.save("dg1","com.sinosoft.cms.resource.AttachmentLib.dg1Edit",function(){
...
function copySave(){
	...
	Server.sendRequest("com.sinosoft.cms.resource.Attachment.copy",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				$D.close();
			}
...
function transferSave(){
	...
	dc.add("IDs",arr.join());
	dc.add("CatalogID",$DW.$V("CatalogID"));
	Server.sendRequest("com.sinosoft.cms.resource.Attachment.transfer",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				$D.close();
...
function delLib(param){
	...
	Dialog.confirm("该分类下的附件都将被删除，你确认要删除此附件分类吗？",function(){
		...
		Server.sendRequest("com.sinosoft.cms.resource.AttachmentLib.delLib",dc,function(response){
		Dialog.alert(response.Message,function(){
				if(response.Status==1){
...
function del(){
	...
	Dialog.confirm("这些附件可能与文章或者图片有关联，你确认要删除吗？",function(){
		var dc = new DataCollection();
		dc.add("IDs",arr.join());
		Server.sendRequest("com.sinosoft.cms.resource.Attachment.del",dc,function(response){
			Dialog.alert(response.Message,function(){
				if(response.Status==1){
					if ($V("SearchName") != "请输入附件名称") {
						DataGrid.setParam("dg1","Name",$V("SearchName"));
...
function publish(){
	...
	var dc = new DataCollection();
	dc.add("IDs",arr.join());
	Server.sendRequest("com.sinosoft.cms.resource.AttachmentLib.publish",dc,function(response){
...
function afterRowDragEnd(type,targetDr,sourceDr,rowIndex,oldIndex){
	...
	Server.sendRequest("com.sinosoft.cms.resource.AttachmentLib.sortColumn",dc,function(response){
		if(response.Status==1){
			DataGrid.loadData("dg1");
		}else{
			Dialog.alert(response.Message);	
...
function DetailEditSave(){
	...
	Server.sendRequest("com.sinosoft.cms.resource.Attachment.dialogEdit",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				$D.close();
				DataGrid.loadData("dg1");
...
<table width="100%" border="0" cellspacing="6" cellpadding="0" style="border-collapse: separate; border-spacing: 6px;">
	<tr valign="top">
		...
		<table width="180" border="0" cellspacing="0" cellpadding="6" class="blockTable">
			<tr>
				<td style="padding:6px;" class="blockTd"><z:tree id="tree1" style="height:450px;width:160px;" method="com.sinosoft.cms.resource.AttachmentLib.treeDataBind" lazy="true" expand="false">
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="6" class="blockTable">
			...
			<tr>
				...
				<z:datagrid id="dg1" method="com.sinosoft.cms.resource.AttachmentLib.dg1DataBind" size="15">
					<table width="100%" cellpadding="2" cellspacing="0" class="dataTable" afterDrag="afterRowDragEnd">
						<tr ztype="head" class="dataTableHead">
							...
							<td width="5%" align="center"><b>大小</b></td>
							<td width="5%"><b>格式</b></td>
							<td width="7%"><b>创建者</b></td>
							<td width="8%"><b>上传时间</b></td>
							<td width="7%"><b>操作</b></td>
							<td width="4%"><b>下载</b></td>
							<td width="18%"><b>链接</b></td>
						</tr>
