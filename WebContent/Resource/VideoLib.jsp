...
function setVideoCover(videoID){
	...
	dc.add("ID",videoID);
	Server.sendRequest("com.sinosoft.cms.resource.VideoLib.setVideoCover",dc,function(response){
...
function setTop(videoID){
	var dc = new DataCollection();
	dc.add("ID",videoID);
	Server.sendRequest("com.sinosoft.cms.resource.VideoLib.setTopper",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				if ($V("Name") != "请输入视频名称") {
...
function changeSave(){
	...
	Server.sendRequest("com.sinosoft.cms.resource.VideoLib.VideoLibEdit",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				Tree.loadData("tree1");
				$D.close();
...
function addSave(){
	...
	Server.sendRequest("com.sinosoft.cms.resource.VideoLib.add",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				Tree.loadData("tree1");
				$D.close();
...
function copySave(){
	...
	Server.sendRequest("com.sinosoft.cms.resource.Video.copy",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
			$D.close();
			}
...
function transferSave(){
	...
	dc.add("IDs",arr.join());
	dc.add("CatalogID",$DW.$V("CatalogID"));
	Server.sendRequest("com.sinosoft.cms.resource.Video.transfer",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				$D.close();
				if ($V("Name") != "请输入视频名称") {
...
function editSave(){
	...
	Server.sendRequest("com.sinosoft.cms.resource.Video.edit",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				$D.close();
				if ($V("Name") != "请输入视频名称") {
...
function delLib(param){
	...
	Dialog.confirm("该分类下的视频都将被删除，你确认要删除此视频分类吗？",function(){
		...
		Server.sendRequest("com.sinosoft.cms.resource.VideoLib.delLib",dc,function(response){
			Dialog.alert(response.Message,function(){
				if(response.Status==1){
...
function del(videoID){	
	...
	Dialog.confirm("这些视频可能与文章或者视频播放器有关联，你确认要删除吗？",function(){
		var dc = new DataCollection();
		dc.add("IDs",videoID);
		Server.sendRequest("com.sinosoft.cms.resource.Video.del",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				if ($V("Name") != "请输入视频名称") {
					DataList.setParam("dg1","Name",$V("Name"));
...
function publish(){
	...
	var dc = new DataCollection();
	dc.add("IDs",arr.join());
	Server.sendRequest("com.sinosoft.cms.resource.VideoLib.publish",dc,function(response){
...
<table width="100%" border="0" cellspacing="6" cellpadding="0" style="border-collapse: separate; border-spacing: 6px;">
	<tr valign="top">
		...
		<table width="180" border="0" cellspacing="0" cellpadding="6" class="blockTable">
			<tr>
				<td style="padding:6px;" class="blockTd"><z:tree id="tree1" style="height:450px;width:160px;" method="com.sinosoft.cms.resource.VideoLib.treeDataBind" lazy="true" expand="false">
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0" class="blockTable">
			...
			<tr>
				<td style="padding:2px 10px;"><label><input type="checkbox" id="AllCheck" onClick="selectAll(this,'VideoID')">全选</label> &nbsp;&nbsp;列出： 从 <input type="text" id="StartDate" style="width:100px; " ztype='date'> 至 <input type="text" id="EndDate" style="width:100px; " ztype='date'> &nbsp;关键词: <input name="Name" type="text" id="Name" onFocus="delKeyWord();" style="width:110px"> <input type="button" name="Submitbutton" value="查询" onClick="doSearch()" id="Submitbutton"></td>
			</tr>
			<tr>
				<td style="padding:0 5px;">
				<ul class="img-wrapper" style="height:380px; overflow:hidden; display:block; margin:0;">
					<z:datalist id="dg1" method="com.sinosoft.cms.resource.VideoLib.dg1DataList" size="14" page="true">
						<li>
						<dl>
