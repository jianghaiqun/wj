...
function jsWidth(){
	var ratio;
	var hratio;
	var jsHeight = $("Height1").value;
	//alert("jsHeight:"+jsHeight);
	if(jsHeight==0){
		$("Width1").value=jsHeight;
		return;
	} else {
		hratio = jsHeight / oralHeight;
	}
	ratio = hratio >= 1 ? 1 : hratio;
	//alert("ratio   :   "+ratio);
	var resHeight = Math.round(oralHeight * hratio);
	var resWidth = Math.round(oralWidth * hratio);
	$("Width1").value = resWidth;
	$("Height1").value = resHeight;
	//alert("jsWidth  :   resHeight   "+resHeight+"  resWidth   "+resWidth );
}
--></script>
...
<table width="760" cellpadding="3" cellspacing="0">
	<tr>
        ...
        <!-- <img id="previewFile5" src="../Framework/Images/blank.gif"  onload="onPreviewLoad(this)"/> -->
