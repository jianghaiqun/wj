...
function binding(bindingId) {
	...
	if(bindingId == "phone"){
		...
		var phoneContent = '<span style="padding:0 4px; color:#7B7B7B;">手机号:</span>'
			+ '<input onfocus="clearJY()" id="inputPhone" disabled="disabled" style="width:8em; padding:4px; margin-top:5px;" value="'+phone+'" /> <input  " class="member_yanzheng"  type="button" value="发送验证码" onclick="SVCToPhone()" />'
			+ '<p style="color:#666; padding-left:50px;">绑定手机后，您即可使用手机号进行登录，<br>在忘记密码时也可以通过绑定手机找回</p>'
			+ '<span style="padding:0 4px; color:#7B7B7B;">验证码:</span>'
			+ '<input id="inputYZM" style="width:4em; padding:4px; margin-top:5px;" /><p style="color:#ff0000; padding-left:50px;"><span id="status2"></span></p>';
		//没有录入手机。
		if(phone == ""){
...
jQuery.ajax({
		url: "member!SVCToPhones.action?ways="+mobileno,
		type: "post",
		dataType: "json",
		success: function(data){
			if(data==true){
			jQuery("#status2").html("验证码已发送,请查看您的手机短信<br>尽快完成操作！");
			}
			else{
			jQuery("#status2").html("验证码已发送失败<br>请重新发送或者联系客服！");
			}
  ...
  if((news-old)<(1000*60)){
  document.getElementById("status2").innerHTML="您的验证码已发送,请耐心等待!";
  }
  else{
   var olddate2 =new Date().getTime();
