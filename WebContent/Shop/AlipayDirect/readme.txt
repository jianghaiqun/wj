接口应用说明:
1. jsp_direct_gbk : 支付宝支付提供的样例代码.
   商户只需要配置完成信息，将其部署到web应用服务器的发布目录下即可.
...
1. jsp_direct_gbk----
  ...
  WebRoot-------|
		|----images:放置需要的图片信息
              	|----index.jsp:提供给商户测试的支付功能的信息录入界面
              	|----alipay_pay.jsp:接受商户输入信息的提交后页面,商户点击支付后提交给com.alipay.util包中的           
...
2.备注：
  ...
  B.在测试异步或者同步返回时如果想让测试结果记录日志可以在src下com.alipay.util包下找到SignatureHelper_return(同步日

志)和SignatureHelper(异步日志)两个类中有一个sign方法中配置日志记录的路径
