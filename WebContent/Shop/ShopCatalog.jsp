...
Page.onLoad(function(){
	if(Cookie.get("DocList.LastShopCatalog")=="0"){
		Tree.CurrentItem = $("tree1").$T("p")[0];
		Tree.CurrentItem.onclick.apply(Tree.CurrentItem);
	}else{
		var node = Tree.getNode("tree1","cid",Cookie.get("DocList.LastShopCatalog"));
		//如果没有展开则载入
		if(!node){
			var code = Cookie.get("DocList.LastShopCatalogCode");
			while(code&&code.length>6){
				...
				if(node){
					Tree.lazyLoad(node,function(){
						...
						node = Tree.getNode("tree1","cid",Cookie.get("DocList.LastShopCatalog"));
		...
		$S("CatalogID",Cookie.get("DocList.LastShopCatalog"));
		$S("CatalogInnerCode",'<%=CatalogUtil.getInnerCode(ServletUtil.getCookieValue(request,"DocList.LastShopCatalog"))%>');
		Tree.CurrentItem.onclick.apply(Tree.CurrentItem);
	}
});
...
function onTreeClick(ele){
	...
	if(!Tree.isRoot(ele)){
		Cookie.set("DocList.LastShopCatalog",cid,"2100-01-01");
		Cookie.set("DocList.LastShopCatalogCode",code,"2100-01-01");
	}
	$S("CatalogID",cid);
...
function addSave(){
	...
	Server.sendRequest("com.sinosoft.cms.site.Catalog.add",dc,function(response){
		if(response.Status==0){
			...
			Dialog.alert("新建栏目成功",function(){
				...
				Tree.loadData("tree1",function(){
					Tree.select("tree1","cid",Cookie.get("DocList.LastShopCatalog"));
		      $S("CatalogID",Cookie.get("DocList.LastShopCatalog"));
		      $S("CatalogInnerCode",'<%=CatalogUtil.getInnerCode(ServletUtil.getCookieValue(request,"DocList.LastShopCatalog"))%>');
				});
			});
...
function move(){
	...
	diag.Height = 420;
	diag.Title = "选择转移栏目";
	diag.URL = "Site/CatalogListDialog.jsp?Type=3&CatalogType=9";
...
function moveSave(){
	...
	Server.sendRequest("com.sinosoft.cms.site.Catalog.move",dc,function(response){
		if(response.Status==0){
			Dialog.alert(response.Message);
		}else{
			$D.close();
...
function del(){
	...
	Dialog.confirm('警告：您确认要删除"'+$V("Name")+'"这个栏目吗？\n这样会把栏目下的所有相关文章都删除掉！',function(){
		dc.add("CatalogID",$V("CatalogID"));
		Server.sendRequest("com.sinosoft.cms.site.Catalog.del",dc,function(response){
			if(response.Status==0){
				Dialog.alert(response.Message);
			}else{
				Dialog.alert("删除栏目成功",function(){
					...
					$S("CatalogID","");
					onTabChange("Basic");
					Cookie.set("DocList.LastShopCatalog","","2100-01-01");
				});
			}
		});
	});
...
function clean(){
	...
	Dialog.confirm('警告：您确认要清空"'+$V("Name")+'"的所有文档吗？',function(){
		dc.add("CatalogID",$V("CatalogID"));
		Server.sendRequest("com.sinosoft.cms.site.Catalog.clean",dc,function(response){
			if(response.Status==0){
				Dialog.alert(response.Message);
			}else{
...
function batchDelete(){
	...
	diag.Height = 400;
	diag.Title = "选择要删除的栏目";
	diag.URL = "Site/CatalogListDialog.jsp?CatalogType=9&Action=Delete";
...
function batchDeleteSave(){
	...
	Dialog.wait("正在删除栏目，请等待...");
	Server.sendRequest("com.sinosoft.cms.site.Catalog.batchDel",dc,function(response){
		Dialog.endWait();
		Dialog.alert(response.Message);
...
<input type="hidden" id="CatalogType" value="9">
		...
		<table width="180" border="0" cellspacing="0" cellpadding="6"
			...
			<tr>
				<td style="padding:6px;" class="blockTd"><z:tree id="tree1"
					...
					method="com.sinosoft.cms.site.Catalog.treeDataBind" level="3"
					lazy="true">
