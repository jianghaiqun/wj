...
function save(){
	...
	DataGrid.save("dg1","com.sinosoft.cms.site.Shop.dg1Edit",function(){window.location.reload();});
}
...
function addSave(){
	...
	Server.sendRequest("com.sinosoft.cms.site.Shop.add",dc,function(response){
...
function moveSave(){
  ...
  if(arrDest == null || arrDest.length==0){
	...
	Server.sendRequest("com.sinosoft.cms.site.Shop.move",dc,function(response){
		if(response.Status==0){
			Dialog.alert(response.Message);
		}else{
			$D.close();
...
function del(){
	...
	Dialog.confirm('警告：您确认要删除"'+$V("Name")+'"这个栏目吗？\n这样会把栏目下的所有相关文章都删除掉！',function(){
			dc.add("CatalogID",$V("CatalogID"));
			Server.sendRequest("com.sinosoft.cms.site.Shop.del",dc,function(response){
				if(response.Status==0){
					Dialog.alert(response.Message);
				}else{
...
function editSave(){
	...
	Server.sendRequest("com.sinosoft.cms.site.Shop.save",dc,function(response){
		if(response.Status==0){
			Dialog.alert(response.Message);
		}else{
...
function publishSave(){
	...
	dc.add("CatalogID",$V("CatalogID"));
	Server.sendRequest("com.sinosoft.cms.site.Shop.publish",dc,function(response){
		if(response.Status==0){
			Dialog.alert(response.Message);
			$D.close();
		}else{
...
function batchDelete(){
	...
	diag.Height = 400;
	diag.Title = "选择要删除的栏目";
	diag.URL = "Site/CatalogListDialog.jsp?Type=9$Action=Delete";
		...
		<table width="160" border="0" cellspacing="0" cellpadding="6"
			...
			<tr>
				<td style="padding:6px;"><z:tree id="tree1"
					...
					method="com.sinosoft.cms.site.Shop.treeDataBind" level="2" lazy="true">
