...
function addSave(){
  ...
  dc.add("UseFlag",$DW.$NV("UseFlag"));
	Server.sendRequest("com.sinosoft.cms.datachannel.DeployConfig.add",dc,function(response){
...
function editSave(){
  ...
  dc.add("Method",$DW.$NV('Method'));
  dc.add("UseFlag",$DW.$NV("UseFlag"));
	Server.sendRequest("com.sinosoft.cms.datachannel.DeployConfig.edit",dc,function(response){
		if(response.Status==1){
...
function del(){
	...
	Dialog.confirm("确定要删除该配置项吗？",function(){
		...
		dc.add("IDs",arr.join());
		Server.sendRequest("com.sinosoft.cms.datachannel.DeployConfig.del",dc,function(response){
...
function deploy(){
	...
	Dialog.confirm("分发任务将扫描目录，确定要立即执行选中的任务吗？",function(){
			var dc = new DataCollection();	
			dc.add("IDs",arr.join());
			Server.sendRequest("com.sinosoft.cms.datachannel.DeployConfig.deploy",dc,function(response){
				if(response.Status==0){
					Dialog.alert(response.Message);
				}else{
...
<table width="100%" border="0" cellspacing="0" cellpadding="1">
	...
	<tr>
		<td>
		<z:datagrid id="dg1" method="com.sinosoft.cms.datachannel.DeployConfig.dg1DataBind" size="14">
