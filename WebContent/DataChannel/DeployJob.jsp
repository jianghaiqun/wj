...
function del(){
	...
	Dialog.confirm("确定要删除该任务项吗？",function(){
		...
		dc.add("IDs",arr.join());
		Server.sendRequest("com.sinosoft.cms.datachannel.DeployJob.del",dc,function(response){
...
function reExecuteJob(){
	...
	if(!arr||arr.length==0){
		...
		var dc = new DataCollection();	
		dc.add("IDs",arr.join());
		Server.sendRequest("com.sinosoft.cms.datachannel.DeployJob.reExecuteJob",dc,function(response){
			if(response.Status==0){
				Dialog.alert(response.Message);
			}else{
				Dialog.alert("分发成功");
...
function executeFailJob(){
	var arr = DataGrid.getSelectedValue("dg1");
		var dc = new DataCollection();	
		Server.sendRequest("com.sinosoft.cms.datachannel.DeployJob.executeFailJob",dc,function(response){
			if(response.Status==0){
				Dialog.alert(response.Message);
			}else{
				Dialog.alert("分发成功");
...
function delAll(){
	  Dialog.confirm("确定要清空所有任务？",function(){
	  		var dc = new DataCollection();	
				Server.sendRequest("com.sinosoft.cms.datachannel.DeployJob.delAll",dc,function(response){
					if(response.Status==0){
						Dialog.alert(response.Message);
					}else{
						Dialog.alert("删除成功");
...
<z:init method="com.sinosoft.cms.datachannel.DeployJob.init">
	<body>
	<table width="100%" border="0" cellspacing="0" cellpadding="1">
		...
		<tr>
			<td>
			<z:datagrid id="dg1" method="com.sinosoft.cms.datachannel.DeployJob.dg1DataBind" size="14">
