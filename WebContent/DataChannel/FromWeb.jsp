...
function addSave(){
	...
	*/
	...
	Server.sendRequest("com.sinosoft.datachannel.FromWeb.add",dc,function(response){
...
function del(){
	...
	Dialog.confirm("确定要删除该任务吗？",function(){
		Server.sendRequest("com.sinosoft.datachannel.FromWeb.del",dc,function(response){
...
function delResult(){
	...
	Dialog.confirm("采集数据是指从指定URL下载的文本文件和<br>图片文件,<font color=red>不包括已经转入栏目的文章</font>。<br><br>确定要清空该任务的采集数据吗？",function(){
		var dc = new DataCollection();	
		dc.add("IDs",arr.join());
		Server.sendRequest("com.sinosoft.datachannel.FromWeb.delResult",dc,function(response){
			Dialog.alert(response.Message);
		});											  
	},null,350);
} 
...
function execute(){
	...
	Dialog.confirm("确定要执行该任务吗？",function(){
		var dc = new DataCollection();	
		dc.add("ID",arr[0]);
		Server.sendRequest("com.sinosoft.datachannel.FromWeb.execute",dc,function(response){
			if(response.Status==1){
...
function cancel(){
	...
	Dialog.confirm("确定要中止执行该任务吗？",function(){
		var dc = new DataCollection();	
		dc.add("ID",arr[0]);
		Server.sendRequest("com.sinosoft.datachannel.FromWeb.cancel",dc,function(response){
			Dialog.alert(response.Message);
		});
	});
}
...
function dealData(){
	...
	var dc = new DataCollection();	
	dc.add("ID",arr[0]);
	Server.sendRequest("com.sinosoft.datachannel.FromWeb.dealData",dc,function(response){
		if(response.Status==1){
			taskID = response.get("TaskID");
			var p = new Progress(taskID,"正在处理采集到的Web文件...",700,150);
			p.show();
		}else{
			Dialog.alert(response.Message);	
		}
	...
	<tr valign="top">
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="6" class="blockTable">
			...
			<tr>
				...
				<z:tbutton onClick="dealData()"><img src="../Icons/icon026a8.gif" width="20" height="20" />处理数据</z:tbutton> 
			...
			<tr>
				...
				<z:datagrid id="dg1" method="com.sinosoft.datachannel.FromWeb.dg1DataBind" page="false">
					<table width="100%" cellpadding="2" cellspacing="0" class="dataTable">
						<tr ztype="head" class="dataTableHead">
							...
							<td width="9%"><b>采集类别</b></td>
							<td width="35%"><b>起始地址</b></td>
							<td width="12%"><b>线程数</b></td>
						</tr>
