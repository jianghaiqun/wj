...
<z:init method="com.sinosoft.cms.document.ProductBaseMsgManage.init">
	<html>
	<head>
...
var contents; //页面内容数组
var contents1; //页面内容数组
var contents2; //页面内容数组
...
function save(){
    ...
    if(content.trim()=="" && checkContentNull){
     Dialog.alert("基本信息不能为空");
 	   return;
    }
    content = contents.join(PAGE_SPLIT);
	dc.add("Content",content);
	
	editor = FCKeditorAPI.GetInstance('FrontInfo');
	contents[currentPage-1]=editor.GetXHTML(false);
	content = contents.join();
    content = content.replace(/<br \/>/g,"");
	dc.add("FrontInfo",content);
	
	editor = FCKeditorAPI.GetInstance('TailInfo');
	contents[currentPage-1]=editor.GetXHTML(false);
	content = contents.join();
    content = content.replace(/<br \/>/g,"");
	dc.add("TailInfo",content);
...
function saveArticle(saveMethod,dc){
	...
	Dialog.wait("正在处理...");
	Server.sendRequest("com.sinosoft.cms.document.ProductBaseMsgManage."+saveMethod,dc,function(response){
	  Dialog.closeEx();
...
function uploadOne(colsName){
	customActionName = colsName;
	$S("ImageIDs",getImagesFromContent());
	var CatalogID = $V("CatalogID");
	var diag = new Dialog("Diag1");
	diag.Width = 800;
...
function onReturnBack(returnID){
	if(customPicName!=null){
		...
		Server.sendRequest("com.sinosoft.cms.document.Article.getPicSrc",dc,function(response){
			...
			if(customActionName=="FrontInfo")
			{
				editor = FCKeditorAPI.GetInstance('FrontInfo');
				editor.InsertHtml( response.get("picSrc") || "" ) ;
				FCKeditorAPI.GetInstance("FrontInfo").Focus();
			}else if(customActionName=="TailInfo"){
				editor = FCKeditorAPI.GetInstance('TailInfo');
				editor.InsertHtml( response.get("picSrc") || "" ) ;
				FCKeditorAPI.GetInstance("TailInfo").Focus();
			}else if(customActionName=="Content"){
				editor = FCKeditorAPI.GetInstance('Content');
				editor.InsertHtml( response.get("picSrc") || "" ) ;
				FCKeditorAPI.GetInstance("Content").Focus();
			}
		...
		<tr>
			...
			<div><z:tbutton onClick="save()">
				...
				<form method="post" id="form1">
					...
					<input type="hidden" id="ProductHiddenFlag" value="true">
													...
													<tr id="trSubTitle" style="display: none">
														...
														<td align="center" >
													     ...
													     <td colspan="3" align="left">
															...
															<div><z:tbutton onClick="uploadOne('Content')">
																<img src="../Icons/icon018a4.gif" />基本信息图片上传</z:tbutton></div>
								...
								oFCKeditor.Create() ;
													...
													<tr style="height:50px">
							                          <td align="center" >
													     ...
													     <td colspan="3" align="left">
																是否允许使用品牌信息
											                	<z:select id="IsFlag" style="width:100px;"  value="${IsFlag}">${@IsFlag1}</z:select>
											                	&nbsp;
											                	是否关联品牌
											                	<z:select id="ComFlag" style="width:100px;"  value="${ComFlag}">${@ComFlag1}</z:select>
													     </td>
													</tr>
												    <tr>
							                          <td align="center" >
													     ...
													     <td colspan="3" align="left">
																<textarea name="textarea" id="_FrontInfo"
																	style="display: none">${frontInfo}</textarea>
																<textarea name="textarea" id="_FrontInfos"
																	style="display: none">${frontInfo}</textarea>
																	<div><z:tbutton onClick="uploadOne('FrontInfo')">
																<img src="../Icons/icon018a4.gif" />头部信息图片上传</z:tbutton></div>
																<script type="text/javascript">
								var sBasePath = Server.ContextPath+"Editor/" ;
								var oFCKeditor = new FCKeditor( 'FrontInfo' ) ;
								oFCKeditor.BasePath	= sBasePath ;
								oFCKeditor.Width = 750 ;
								oFCKeditor.Height = 200 ;
								oFCKeditor.Config['EditorAreaCSS'] = '${CssPath}';
								oFCKeditor.Config[ 'ToolbarLocation' ] = 'Out:xToolbar' ;
								oFCKeditor.Value = $V("_FrontInfo");
								oFCKeditor.Create() ;
							</script>
															</td>
														</tr>
														...
														<td align="center" >
													     ...
													     <td colspan="3" align="left">
																<textarea name="textarea" id="_TailInfo"
																	style="display: none">${TailInfo}</textarea>
																<textarea name="textarea" id="_TailInfos"
																	style="display: none">${TailInfo}</textarea>
																	<div><z:tbutton onClick="uploadOne('TailInfo')">
																<img src="../Icons/icon018a4.gif" />尾部信息图片上传</z:tbutton></div>
																<script type="text/javascript">
								var sBasePath = Server.ContextPath+"Editor/" ;
								var oFCKeditor = new FCKeditor( 'TailInfo' ) ;
								oFCKeditor.BasePath	= sBasePath ;
								oFCKeditor.Width = 750 ;
								oFCKeditor.Height = 200 ;
								oFCKeditor.Config['EditorAreaCSS'] = '${CssPath}';
								oFCKeditor.Config[ 'ToolbarLocation' ] = 'Out:xToolbar' ;
								oFCKeditor.Value = $V("_TailInfo");
								oFCKeditor.Create() ;
								oFCKeditor._Upload() ;
							</script>
															</td>
