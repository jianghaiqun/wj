	...
	<table cellspacing="0" cellpadding="0" border="0" align="center"
		...
		<tbody>
			<tr class="m_order_th">
				...
				<th width="100">投保金额（元）</th>
				<th width="78">起息日</th>
				<th width="78">当日收益</th>
				<th width="100">账户预期收益</th>
				<th width="114">操作</th>
			</tr>
			...
			<s:iterator id="list" value="#request.listRecord" status="status">
				<tr class="m_order_num2">
					<!--订单号-->
					<td>
					<a target="_blank" class="order_links_s"
					...
					<!-- 理财险产品信息 -->
					<td><span class="memberOrder-name"><s:property
								value="#list.productname" /></span></td>
					<!--投保金额（元）-->
					<td><s:property value="#list.principal" /></td>
					<!--起息日-->
					<td><s:if test="#list.insstatus == ''">——</s:if>
						<s:else>
					...
					<!--操作-->
					<td><s:if test="#list.insstatus == ''">
												暂未生效
											</s:if> <s:else>
							<div class="lc_action">
								<a
									href="order_config_new!linkOrderDetails.action?orderSn=<s:property value="#list.ordersn"/>&KID=<s:property value="#list.KID"/>"
									target="_blank" class="lc_des">查看</a>
								<s:if test="#list.insstatus == 1">
									<a
										href="order_query!financingSurrender.action?orderSn=<s:property value="#list.ordersn"/>"
										class="lc_des lc_tb">退保</a>
								</s:if>
								<s:if test="#list.insstatus == 2">
									<span class="lc_tbing">退保中</span>
								</s:if>
							</div>
						</s:else></td>
				</tr>
