	...
	<div class="body">
		...
		<div class="cartItemListDetail">
			<div class="top">
				...
				<div class="topMiddle">已放入购物车的产品:</div>
			...
			<div class="middle">
				<table class="listTable">
					...
					<#if (cartItemList == null || cartItemList?size == 0)!>
						<tr>
							<td class="noRecord" colspan="<#if (loginMember.memberRank.preferentialScale != 100)!>6<#else>5</#if>">购物车目前没有加入任何产品!</td>
						</tr>
					<#else>
						<tr>
							<td class="info" colspan="<#if (loginMember.memberRank.preferentialScale != 100)!>6<#else>5</#if>">
								产品共计：<span id="totalQuantity" class="red">${totalQuantity}</span> 件    
								<#if systemConfig.pointType != "disable">
									积分：<span id="totalPoint" class="red">${totalPoint}</span>    
								</#if>
								产品总金额(不含运费)：<span id="totalPrice" class="red">${totalPrice?string(orderUnitCurrencyFormat)}</span>
							</td>
						</tr>
					</#if>
				</table>
				<div class="blank"></div>
				<a class="continueShopping" href="${base}/"><span class="icon"> </span>继续投保</a>
				<#if (cartItemList != null && cartItemList?size > 0)!>
