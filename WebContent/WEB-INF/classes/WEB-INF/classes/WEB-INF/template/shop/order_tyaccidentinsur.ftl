...
function loadRelation(){
	 ...
	 var userType=document.getElementById("mulPeople");
	 if((userType!=null) && (typeof(userType)!="undefined")){
	   	 insuredVIP(userType.value);
	 }
}
...
function changeInformation(objForm,aa){
   ...
   function insuredVIP(userType){
	   	if(userType=="太平洋车险用户专享优惠价格"){
	   		document.getElementById("contNo").className = "required";
	   		document.getElementById("carCont").style.display="block";
	   	}else{
	   		document.getElementById("contNo").removeAttributeNode(document.getElementById("contNo").getAttributeNode("class"));
	   		document.getElementById("contNo").value="";
	   		document.getElementById("carCont").style.display="none";
	   	}
   }
	 ...
	 <#else>
       ...
       <div class="line_a">
            ...
            <div class="form">
                 ...
                 <#if (mulPeopleList.size()>0)!>
                    <p>
                        <span>保险责任多类人群列表</span> 
                        <#if (mulPeopleList?size>1)>
                        <select name="mulPeople"  id="mulPeople" onchange="insuredVIP(this.value);">
								<#list mulPeopleList as list>
								   <option value="${list.factorValue}" <#if mulPeople==list.factorValue>selected="selected"</#if> >
									${list.factorDisplayValue}
								   </option>
								</#list>
						 </select>
						 <#else>
						   		<#list mulPeopleList as list>
						   		        <input value="${list.factorValue}" name="mulPeople" id="mulPeople" type="hidden">
										<span>${list.factorDisplayValue}</span>
								</#list>
						 </#if>
                    </p>
                 </#if>
	             <p id ="carCont" style="display:none">
	                <span>车险保单号:</span> 
	                <input type="text" id="contNo" name="informationInsureds1.contNo" value="${(informationInsureds1.contNo)!}" /><label class="requireField">*</label>
	             </p>
       ...
       <input type ="hidden" id="countPrice" name="countPrice" value="${countPrice}">
       ...
       <p>
        ...
        <p>
        <span>联系地址:</span> 
        <input type="text" id="applicantAddress" name="informationAppnt.applicantAddress" maxlength=200 class="required" value="${(informationAppnt.applicantAddress)!}"/>
        <label class="requireField">*</label>
     </p>
     <p>
         <span>邮编:</span> 
     ...
     <div class="form">
       <p>
        ...
        <div id="insurancedOne"  style="display:none;">
             <p>
                <span>姓名:</span> 
                <input type="text" id="recognizeeName1" name="informationInsureds1.recognizeeName" onkeyup="value=value.replace(/[^\a-\z\A-\Z\u4E00-\u9FA5]/g,'')" value="${(informationInsureds1.recognizeeName)!}" maxlength=50/>
            	 <label class="requireField">*</label>
             </p>
            <p>
               <span>证件类型:</span> 
