...
function limitImg(img){
    var ss = img.split(".");
    if(ss.length == 2){
        if (ss[1] == "jpg"||ss[1] == "gif") {
            if(img.width=100){
               alert("图片宽度应为100像素");  
               return false;  
            }  
...
<body class="input">
	<div class="body">
		...
		<form id="inputForm" class="validate" action="<#if isAdd??>payment_config!save.action<#else>payment_config!update.action</#if>" enctype="multipart/form-data" method="post">
			...
			<table class="inputTable">
				...
				<tr>
					...
					<td>
						<input type="file" name="logo" onchange="limitImg(this.value);"/><#if (paymentConfig.logo != null)!>&nbsp;&nbsp;&nbsp;<a href="${base}${paymentConfig.logo}" class="imagePreview" target="_blank">查看</a></#if>
						<label class="requireField">*</label>
					</td>
				</tr>								
				<tr>
