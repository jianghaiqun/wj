...
<body class="list">
	<div class="body">
		<div class="listBar">
			<h1><span class="icon">&nbsp;</span>产品类型列表&nbsp;<span class="pageInfo">总记录数: ${pager.totalCount}(共${pager.pageCount}页)</span></h1>
		</div>
		<form id="listForm" action="product_ins_type!list.action" method="post">
			<div class="operateBar">
				<input type="button" class="addButton" onclick="location.href='product_ins_type!add.action'" value="添加类型" />
				<label>查找:</label>
			...
			<table class="listTable">
				...
				<#list pager.list as list>
					<tr>
						...
						<td>
							<a href="product_ins_attribute!list.action?productInsTypeId=${list.id}" title="查看属性列表">
								[属性列表]
							</a>
						</td>
						<td>
							<a href="product_ins_type!edit.action?id=${list.id}" title="编辑">[编辑]</a>
						</td>
					</tr>
				</#list>
			</table>
			<#if (pager.list?size > 0)>
				<div class="pagerBar">
					<input type="button" class="deleteButton" url="product_ins_type!delete.action" value="删 除" disabled hidefocus="true" />
