...
$().ready( function() {
	...
	$completedProcessButton.click( function() {
		var $this = $(this);
		if (confirm("投保完成后将不允许对此投保进行任何操作，确认执行？") == true) {
			$.ajax({
	...
	$invalidProcessButton.click( function() {
		var $this = $(this);
		if (confirm("投保作废后将不允许对此投保进行任何操作，确认执行？") == true) {
			$.ajax({
...
<body class="input">
	<div class="body">
		<div class="inputBar">
			<h1><span class="icon">&nbsp;</span>处理投保</h1>
		</div>
		<div class="blank"></div>
		<ul class="tab">
			<li>
			...
			<li>
				<input type="button" id="paymentTabButton" value="投保支付"<#if order.orderStatus == "completed" || order.orderStatus == "invalid" || order.paymentStatus == "paid" || order.paymentStatus == "partRefund" || order.paymentStatus == "refunded"> disabled</#if> hidefocus="true" />
			</li>
			<li>
				<input type="button" id="shippingTabButton" value="投保发货"<#if order.orderStatus == "completed" || order.orderStatus == "invalid" || order.shippingStatus == "shipped"> disabled</#if> hidefocus="true" />
			</li>
			<li>
		...
		<div class="tabContent">
			<table class="inputTable">
				<tr>
					...
					<td>
						<input type="button" name="paymentProcess" class="formButton" value="投保支付"<#if order.orderStatus == "completed" || order.orderStatus == "invalid" || order.paymentStatus == "paid" || order.paymentStatus == "partRefund" || order.paymentStatus == "refunded"> disabled</#if> hidefocus="true" />
						<input type="button" name="shippingProcess" class="formButton" value="投保发货"<#if order.orderStatus == "completed" || order.orderStatus == "invalid" || order.shippingStatus == "shipped"> disabled</#if> hidefocus="true" />
						<input type="button" name="completedProcess" class="formButton" value="投保完成"<#if order.orderStatus == "completed" || order.orderStatus == "invalid"> disabled</#if> hidefocus="true" />
					</td>
					<td colspan="2">
						...
						<tr>
							<td colspan="6">
								<div class="buttonArea">
									...
									<input type="submit" class="formButton" value="确  定" hidefocus="true" />&nbsp;&nbsp;&nbsp;&nbsp;
									-->
									<font color="red">[在“支付宝”付款信息返回后提供处理功能]</font>
									<input type="button" class="formButton" onclick="window.history.back(); return false;" value="返  回" hidefocus="true" />
								</div>
							</td>
						</tr>
