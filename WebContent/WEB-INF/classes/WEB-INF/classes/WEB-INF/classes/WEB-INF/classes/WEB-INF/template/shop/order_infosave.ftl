...
$().ready( function() {
	...
	var $deleteCartItem = $(".deleteCartItem");//购物车
...
<!---=========增加填写保单信息的功能======------>
	...
	<div class="body">
		...
		<div class="orderInfoDetail">
			<form id="orderInfoForm" action="order!result.action?orderId=${order.id}&productId=${product.id}" method="post">
				<@s.token />
				<div class="blank"></div>
				<table class="cartItemTable">
					...
					<#list cartItemSet as list>
						<tr>
							...
							<td>
							<#if (list.valid == false)>
									<input type="button" onclick="location.href='${base}/shop/order!bbxinfo.action?orderId=${order.id}&productId=${list.product.id}'" value="填写保单信息" class="red" hidefocus="true" />
								<#else>
							...
							<#if (list.valid == false)>
								...
								<#else>
									<a class="deleteCartItem {id: '${list.product.id}',orderId: '${order.id}'}" href="javascript: void(0);">删除</a>
							</#if>
							</td>
						</tr>
					</#list>
