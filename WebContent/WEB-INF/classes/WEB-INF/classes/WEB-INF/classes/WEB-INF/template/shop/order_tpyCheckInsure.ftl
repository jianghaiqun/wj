...
<style type="text/css">
   body, div, dl, dt, dd, ul, ol, li, h1, h2, h3, h4, h5, h6, pre, code, form, fieldset, legend, input, textarea, p, blockquote, th, td, hr, button, article, aside, details, figcaption, figure, footer, header, hgroup, menu, nav, section{margin:0;padding:0}
	...
	.loading-indicator-bars {
			background-image: url('${base}/template/shop/images/loading-bars.gif');
			width: 150px;
}
	</style>
...
alert(assd);
	   var orderSn = document.getElementById("orderSn").value;
				jQuery.ajax({
					url: sinosoft.base+"/shop/order!checkInsurePay.action?orderSn="+orderSn,
					dataType: "json",
					async: false,
	...
	<div class="w900">
		...
		<div class="lay-ins-con pro-tab">
			...
			<table border=0 cellspacing=0 cellpadding=0>
					...
					<tr>
					<td id="divc2" ><img width="170" height="15" title="正在核保，请稍等" src="${base}/template/shop/images/loading-bars.gif"/></td>
					</tr>
					<tr id="divc3" style="display:none;">
					<#if (order.insuranceCompanySn!"") == "2011">
						<#assign href=base + "/shop/order!buyNowUpdate.action?orderId=" + order.id + "&productId=" + productId +"&orderSn=" + order.orderSn />
					<#else>
						<#assign href=base + "/shop/order_config!buyNowUpdate.action?orderId=" + order.id + "&productId=" + productId +"&orderSn=" + order.orderSn />
					</#if>
