  ...
  <div class="line_a">
     ...
     <div class="form">
       <p>
        ...
        <p>
           <span>旅游目的地:</span>
           <select id="destinationCountry" name="informationInsureds.destinationCountry">
               <#if (listCountryCode!=null && listCountryCode.size()>0)>
                    <#list listCountryCode as list>
                         <option value="${list.codeValue}" <#if (informationInsureds.destinationCountry==list.codeValue)>selected="selected"</#if>>${list.codeName}</option>
                    </#list>
               </#if>
           </select>
        </p>
