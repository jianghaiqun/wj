...
<body class="input">
	<div class="body">
		...
		<form id="inputForm" class="validate" action="car_rateinformation!save.action" method="post" enctype="multipart/form-data">
		    <input type="hidden" name="carRateIF.id" value="${carRateIF.id}"/>
						   ...
						   <#if (coms != null && coms?size > 0)>
							<#list coms as list>
					            <option value="${list.supplierCode}"
					               <#if (list.supplierCode==carRateIF.companyCode)!>
					                  selected
					   ...
					   <option value="-1">---请选择---</option>
					      <#if (citys != null && citys?size > 0)>
							<#list citys as list>
					            ...
					            <#if (list.placeCode==carRateIF.regionCode)!>
					                  selected
					            </#if> 
					            >${list.placeName}</option>
					        </#list>
					      </#if>
					   </select>
					</td>
				...
				<tr>
					...
					<td>
					   <select id="modelType" name="carRateIF.modelType" >
						  <option value="-1">----请选择----</option>
                          <option value="A" <#if (carRateIF.modelType == "A")> selected</#if>>商业险A条款模板</option>
                          <option value="B" <#if (carRateIF.modelType == "B")> selected</#if>>商业险B条款模板</option>
                          <option value="C" <#if (carRateIF.modelType == "C")> selected</#if>>商业险C条款模板</option>
                          <option value="D" <#if (carRateIF.modelType == "D")> selected</#if>>商业险其他模板</option>
					   </select>
					</td>
				</tr>
				<tr>
					...
					<td>
					   <input type="text" name="carRateIF.dicount" id= "dicount" value="${carRateIF.dicount}"/>
					</td>
				</tr>
				<tr>
					<th>
					...
					<td>
					   <select id="flag" name ="carRateIF.flag">
					          <option value="Y"<#if (carRateIF.flag == "Y")> selected</#if> >是</option>
					          <option value="N"<#if (carRateIF.flag == "N")> selected</#if>>否</option>
					   </select>
					</td>
				</tr>
			</table>
