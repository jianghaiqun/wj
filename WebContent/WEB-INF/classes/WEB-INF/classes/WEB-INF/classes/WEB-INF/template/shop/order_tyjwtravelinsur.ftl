...
function loadRelation(){
	 ...
	 /* zhangjinquan 11180 bug933 下一步重新填写自动带出目的地  2012-10-20 */
	 setOutGoingParpose();
}
 ...
 <p>
  ...
  <div class="line_a">
     ...
     <div class="line_a" style="height:200px;">
		       ...
		       <input type="hidden" id="checkOutPice" name="checkOutPice" value="<#list detisnateList as list>${list}</#list>"  class="required" /></span>
	       </div>
  </div>
 <div class="line_a">
...
法定继承人
 ...
 <div class="windowTop_">
	...
	<div class="wraplogin_" >
     <table>
 		<#assign index = 0 />
 		<#assign countlist=informationInsureds1.destinationCountry>
	     <#list listCountryCode  as list>
	     	<#assign countCode=list.get('codeValue')>	     	
	        <#if (index%5==0)>
	            <tr height="20px">
	        </#if>
	             <td style="padding-left: 10px">
	             	...
	             	<input type="checkbox" name="informationInsureds1.destinationCountry" value="${list.get('codeValue')}" <#if checkflag="1">checked</#if>> <label for="${list.get('codeValue')}"> ${list.get("codeName")}</label>
	             </td>
	     	<#if (index%5==4)>
	            </tr>
	        </#if>
 ...
 <div style="text-align:center;height:40px;font-size:18px; margin-top:40px">
        <span >
        	  <span>
