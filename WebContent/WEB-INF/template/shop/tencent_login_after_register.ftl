         	...
         	<div class="reg-box up_ragbox set_user_box">
        		...
        		<div id="reg-wrap">
                	...
                	<div class="lh_title cf">
		                <div class="lh_tou"> <img id="lh_img" src="${Avatar}" height="60"  alt=""/></div>
		                <div class="lh_tit_p">您好,${nickName}</br >欢迎使用${comName}登录开心保网站！<br>已有会员账号绑定可享专属优惠！<br>跳过绑定此单不享受会员优惠哦！</div>
	                </div>
...
function logqueryRemarkError(){
	jQuery.ajax({
					url: '${base}/shop/member!queryLoginRemark.action',
					type: "post",
					dataType: "json",
					success: function(data){
						if (data == false) {
