...
<div class="form tb-up-from"><div class="favoree_select">
	  		...
	  		<label for="recognizeetg_zd"><input type="radio" value="1" class="favoree_tag" id="recognizeetg_zd" name="benefitOperate"checked="checked">指定受益人</label>  
  		</div>
	...
	<span class="span_t">*性别：</span>
		...
		<label>
			...
			<#if (sdinformationbnfList[x-1].bnfSex==list.codeValue)!>checked="checked"<#elseif (list.codeName=="男" && sdinformationbnfList[x-1]==null ) !>checked="checked"</#if>>${list.codeName}</label>
	             ...
	             <input type="text" class="shop_day"  verify="出生日期|notnull" onBlur="verifyElement('出生日期|NOTNULL&AGE',this.value,this.id);"
	             value="${(sdinformationbnfList[x-1].bnfBirthday)!}"  onclick="WdatePicker({skin:'whyGreen',minDate:'%y-%M-{%d-18000}',maxDate:'%y-%M-{%d-1}'})"  name="sdinformationbnfList[${x-1}].bnfBirthday" id="favoreeBirthday_${x-1}">
	        	 <label class="requireField"></label>
	         </p><p>
