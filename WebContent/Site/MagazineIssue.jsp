...
function del(){
	...
	Dialog.confirm("确认删除选中的期号吗？",function(){
			...
			Server.sendRequest("com.sinosoft.cms.site.MagazineIssue.del",dc,function(response){
				if(response.Status==0){
...
function editSave(){
  ...
  if($DW.Verify.hasError()){
	...
	Server.sendRequest("com.sinosoft.cms.site.MagazineIssue.edit",dc,function(response){
		if(response.Status==0){
			Dialog.alert(response.Message);
		}else{
...
function generate(){
	...
	Server.sendRequest("com.sinosoft.cms.site.MagazineIssue.generate",dc,function(response){
		if(response.Status==0){
			Dialog.alert(response.Message);
		}else{
...
function publishSave(){
	...
	Server.sendRequest("com.sinosoft.cms.site.Magazine.publish",dc,function(response){
		if(response.Status==0){
			Dialog.alert(response.Message);
			$D.close();
		}else{
...
<body>
<z:init method="com.sinosoft.cms.site.MagazineIssue.init">
	...
	<table width="100%" cellpadding="4" cellspacing="0">
		<tr>
			<td><z:tbutton onClick="add();">
				...
				onClick="copy();">
				<img src="../Platform/Images/tab1_tri.gif" />复制</z:tbutton>--></td>
		</tr>
	</table>
		...
		method="com.sinosoft.cms.site.MagazineIssue.dg1DataBind" size="15">
