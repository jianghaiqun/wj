...
function addSave(){
	...
	Server.sendRequest("com.sinosoft.cms.site.CatalogExtend.add",dc,function(response){
...
function editSave(){
	...
	dc.add("CatalogID",$V("CatalogID"));
	Server.sendRequest("com.sinosoft.cms.site.CatalogExtend.save",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				$D.close();
				DataGrid.loadData("dg1");
...
function del(){
	...
	Dialog.confirm("您确认要删除吗？",function(){
		...
		dc.add("CatalogID",$V("CatalogID"));
		dc.add("IDs",arr.join());
		Server.sendRequest("com.sinosoft.cms.site.CatalogExtend.del",dc,function(response){
			Dialog.alert(response.Message,function(){
				if(response.Status==1){
					DataGrid.loadData("dg1");
				}
...
function importBatch(){
	...
	diag.Height = 150;
	diag.Title = "扩展属性批量导入";
	diag.URL = "Site/Import_extendField.jsp?CatalogID="+CatalogID;
	diag.show();
	diag.OKButton.hide();
	...
	<table width="100%" cellpadding="0" cellspacing="0"
		...
		<tr>
			...
			<z:tbutton id="BtnAdd" priv="article_manage"
				...
				<img src="../Icons/icon024a3.gif" />自定义字段批量导入
				</z:tbutton></td>
		</tr>
	</table>
	<z:datagrid id="dg1"
		method="com.sinosoft.cms.site.CatalogExtend.dg1DataBind" page="false"
		autoFill="true">
