...
function addSave(){
	...
	Server.sendRequest("com.sinosoft.cms.site.Site.add",dc,function(response){
...
function del(){
	...
	Dialog.confirm("您确认要删除站点吗，删除后部分数据和文件将无法恢复？",function(){
		...
		Server.sendRequest("com.sinosoft.cms.site.Site.del",dc,function(response){
			Dialog.endWait();
			Dialog.alert(response.Message,function(){
				if(response.Status==1){
					window.parent.location.reload();
...
function editSave(){
	...
	Server.sendRequest("com.sinosoft.cms.site.Site.save",dc,function(response){
...
function executeImportSite(){
	...
	Server.sendRequest("com.sinosoft.cms.site.Site.checkImportSite",dc,function(response){
...
function afterRowDragEnd(type,targetDr,sourceDr,rowIndex,oldIndex){
	...
	Server.sendRequest("com.sinosoft.cms.site.Site.sortColumn",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				DataGrid.loadData("dg1");
			}
...
<body scroll="no">
<z:init method="com.sinosoft.cms.site.Site.init">
		...
		<tr valign="top">
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="6" class="blockTable">
				...
				<tr>
					...
					<z:datagrid id="dg1" method="com.sinosoft.cms.site.Site.dg1DataBind" size="15">
