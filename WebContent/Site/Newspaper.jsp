...
function addIssueSave(){
  ...
  if($DW.Verify.hasError()){
	...
	Server.sendRequest("com.sinosoft.cms.site.NewspaperIssue.add",dc,function(response){
		if(response.Status==0){
			Dialog.alert(response.Message);
		}else{
...
function addNewspaperSave(){
	...
	Server.sendRequest("com.sinosoft.cms.site.Newspaper.add",dc,function(response){
		if(response.Status==0){
			Dialog.alert(response.Message);
		}else{
			Dialog.alert("新建成功",function(){
...
function editNewspaperSave(){
	...
	Server.sendRequest("com.sinosoft.cms.site.Newspaper.edit",dc,function(response){
		if(response.Status==0){
			Dialog.alert(response.Message);
		}else{
...
function delNewspaper(){
	...
	Dialog.confirm('警告：您确认要删除"'+$V("Name")+'"这个期刊吗？\n这样会把期刊下的所有相关文章都删除掉！',function(){
			dc.add("CatalogID",$V("CatalogID"));
			dc.add("CatalogType",$V("CatalogType"));
			Server.sendRequest("com.sinosoft.cms.site.Newspaper.del",dc,function(response){
				if(response.Status==0){
					Dialog.alert(response.Message);
				}else{
		...
		<table width="180" border="0" cellspacing="0" cellpadding="6"
			...
			<tr>
				<td style="padding:6px;" class="blockTd"><z:tree id="tree1"
					style="height:440px"
					method="com.sinosoft.cms.site.Newspaper.treeDataBind" level="2"
					lazy="true">
