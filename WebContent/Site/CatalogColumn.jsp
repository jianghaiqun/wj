...
function SysAdd(){
	...
	diag.Height = 350;
	diag.Title = "添加系统栏目字段";
	diag.URL = "Site/CatalogColumnSysDialog.jsp";
	diag.onLoad = function(){
		//$DW.$("Name").focus();
	};
	diag.OKEvent = sysAddSave;
	diag.show();
}
...
function addSave(){
	...
	Server.sendRequest("com.sinosoft.cms.site.CatalogColumn.add",dc,function(response){
...
function sysAddSave(){
	var dc = $DW.Form.getData("form2");
	dc.add("CatalogID",$V("CatalogID"));
	Server.sendRequest("com.sinosoft.cms.site.CatalogColumn.sysAdd",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				$D.close();
				DataGrid.loadData("dg1");
...
function editSave(){
	...
	dc.add("CatalogID",$V("CatalogID"));
	Server.sendRequest("com.sinosoft.cms.site.CatalogColumn.save",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				$D.close();
				DataGrid.loadData("dg1");
...
function delSave(){
	...
	dc.add("CatalogID",$V("CatalogID"));
	dc.add("IDs",arr.join());
	Server.sendRequest("com.sinosoft.cms.site.CatalogColumn.del",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				DataGrid.loadData("dg1");
				$D.close();
...
function importBatch(){
	...
	diag.Width = 400;
	diag.Height = 150;
	diag.Title = "自定义字段批量导入";
	diag.URL = "Site/Import_customField.jsp?CatalogID="+CatalogID;
	diag.show();
	diag.OKButton.hide();
	...
	<table width="100%" cellpadding="0" cellspacing="0"
		...
		<tr>
			...
			<z:tbutton id="BtnAdd" priv="article_manage"
				...
				<z:tbutton id="SysAdd" priv="article_manage"
				onClick="SysAdd()">
				<img src="../Icons/icon024a2.gif" />添加系统内容模型</z:tbutton>
				<z:tbutton id="import"
				priv="article_manage" onClick="importBatch()">
				<img src="../Icons/icon024a3.gif" />自定义字段批量导入</z:tbutton>
				</td>
		</tr>
	</table>
	<z:datagrid id="dg1"
		method="com.sinosoft.cms.site.CatalogColumn.dg1DataBind" page="false">
