define(["jquery", "jquery_cookie", "jquery_form", "art_dialog", "art_iftools", "calendar", "header", "footer", "login", "jqupdate", "productcompare"], function() {
    return {
        ...
        showBenefit: function() {
            // 加载默认值
            var def = sessionStorage.getItem('SHOW_DENEFIT_DEF');
            if(def && def != null){
                ...
                var birthday = sessionStorage.getItem('SHOW_DENEFIT_BIRTHDAY');
                jQuery("#inpRiskAppFactor_TextAge").val(birthday);
            }

            var dutyVal = sessionStorage.getItem('SHOW_DENEFIT_DUTY');
            if(dutyVal){
            ...
            var mult = sessionStorage.getItem('SHOW_DENEFIT_MULT'),
                jBnrInput = jQuery(".bnr_box .add_mult input"),
                jInput = jQuery(".add_mult input"),
                jPrice = jQuery(".price .priceC span"),
                jMoney = jQuery("#txt_price"),
                oThis = this;
            ...
            jPrice.html("<em>￥</em>" + 1000*mult + ".00");
            jMoney.html("<em>￥</em>" + 1000*mult + ".00");

            oThis.prem = 1000*mult;
                ...
                jQuery.ajax({
                    ...
                    success: function(data){
                        ...
                        if(data && data.status == '1'){
                            ...
                            var feeYearText;
                            selectedLi.each(function(i, v) {
                                _val = jQuery(v).children().html();
                                _key = jQuery(v).children().attr("id");

                                if (_key.indexOf("Sex_") != -1) {
                                    sexText = _val;
                                } else if (_key.indexOf("TextAge_") != -1) {
                                    ageText = _val;
                                } else if (_key.indexOf("Period_") != -1) {
                                    preiodText = _val;
                                } else if (_key.indexOf("FeeYear_") != -1) {
                                    feeYearText = _val;
                                }
                            })

                            jQuery("#li_sexAndAge").html(sexText + ageText);
                            jQuery("#li_period").html("保险期限" + preiodText);
                            jQuery("#li_feeYear").html("缴费年限" + feeYearText);
                            jQuery("#li_prem").html("年缴保费" + oThis.prem + "元");

                            jQuery(".priceM").find("span").html("￥" + data.prem);
                            jQuery(".money").find("em").html("￥" + data.prem);

                            var benefitData = data.data;
                            var appendStr = "";
                            for(var i=0;i<benefitData.length;i++){
                                appendStr += "<tr>"+
                                    "<td>"+benefitData[i].policyYear+"</td>"+
                                    "<td>"+benefitData[i].age+"</td>"+
                                    "<td>"+benefitData[i].totalPremiums+"</td>"+
                                    "<td>"+benefitData[i].livePremium+"</td>"+
                                    "<td>"+benefitData[i].diePremium+"</td>"+
                                    "<td>"+benefitData[i].cashValue+"</td>"+
                                    "<td>"+benefitData[i].accumulatedValue+"</td>"+
                                    "<td>"+benefitData[i].liveTotalBenefit+"</td>"+
                                    "</tr>";
                            }
        ...
        productEventShow: function() {
            ...
            jQuery.ajax({
                type: 'post',
                url: sinosoft.base+'/shop/sales_volume!showActivity.action?productId='+riskCode+'&channelSn='+channelsn,
                dataType: "json",
                async: true,
                success: function(data) {
