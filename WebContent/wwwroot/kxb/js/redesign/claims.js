jQuery(document).ready(function() {
    ...
    jQuery("#add_bbr").click(function() {
        ...
        lieq.find(".mess").html("");
        lieq.addClass("no-first").find("td").eq(1).addClass("cutting").end()
                                            .eq(2).addClass("error");
    ...
    jQuery("#sjms").blur(function() {
     ...
     jQuery("#j_captcha").blur(function() {
        new ZMCommon().NotEmpty("j_captcha", "j_captchaErr", "验证码")
    }
    ).focus(function() {
        new ZMCommon().SetHTML("j_captchaErr", "")
    }
    )
}
);
function Remove(cxr) {
	art.dialog({
        ...
        ok: function(){
            ...
            jQuery("#lipei_ul li").eq(0).removeClass("no-first");
...
valObj1.ValAll = function() {
    var retVal = (this.NotEmpty("orderSn", "OrderSnErr", "订单号") && this.NotEmpty("contactName", "LinkNameErr", "联系人") && this.NotEmpty("ConEmail", "EmailErr", "联系人邮箱") && this.NotEmpty("ConTel", "TelErr", "联系人电话") && this.NotEmpty("ConTime", "TimeErr", "事故发生时间") && this.NotEmpty("ConDi", "DiErr", "事故发生地点") && this.NotEmpty("sjms", "sjmsErr", "事故描述") && this.NotEmpty("j_captcha", "j_captchaErr", "验证码"));
    if (retVal) {
        var dangerNameObjs = this.GetObjByName("name");
        var dangerNoObjs = this.GetObjByName("identityId");
        if (dangerNameObjs.length != dangerNoObjs.length) {
            retVal = false
        }
        if (retVal) {
            for (var i = 0; i <= dangerNameObjs.length - 1; i++) {
                if (!this.NotEmptyByObj($(dangerNameObjs[i]).get(0), $(dangerNameObjs[i]).parent().next().find(".mess").get(0), "姓名")) {
                    ...
                    if (!this.NotEmptyByObj($(dangerNoObjs[i]).get(0), $(dangerNoObjs[i]).parent().next().find(".mess").get(0), "证件号")) {
                        retVal = false;
                        break
                    }
                }
        ...
        success: function(data) {
            if (data.status == "success") {
                var dialog = art.dialog({
                    title: '提交成功',
                    icon:"succeed",
                    content: '您的报案已成功记录，我们会尽快与您取得联系，<br>请耐心等待审核，谢谢',
                    ok: function(){
                        location.href = sinosoft.front + "/lipei/";
                        return false;
                    }
                });
            } else {
            	$("#regWindowCaptchaImage").click();
