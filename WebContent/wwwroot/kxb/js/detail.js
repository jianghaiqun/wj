...
jQuery(document).ready(function() {
	...
	if(jQuery("#pointproducttype").length==0){
		...
		if (jQuery("#error_div").css("display") == undefined || jQuery("#error_div").css("display") == 'none') {
			...
			document.getElementById("productLashNum_" + riskCode).innerHTML =  0 + "个";
		}
	}
...
function doPoints(){
	jQuery.ajax({
		url:  sinosoft.base+'/shop/points!queryPayPrice.action?',
		type: 'post',
		dataType: 'json',
		success: function(data){
			var obj = eval(data);
...
jQuery.fn.autoShowList = function(opt) {
	...
	var elmBox, elmLi, tdList, jCpPriceUl, jBnrPrice, jPlanCon, jAutoSelDay, jDateBtn, jSelLi, nTemp;
