...
jQuery.fn.fixService = function(opt) {
    ...
    function _monitor() {
        if (_opts.direction == "right") {
            ...
            if (_opts.fix_positon < 0) {
                _opts.fix_positon = 0
            }
            Othis.css({
                right: _opts.fix_positon
            })
        } else {
            _opts.fix_positon = (Wid.width() - DOM.width()) / 2 - Othis.width() - _opts.MRight;
            if (_opts.fix_positon < Othis.width()) {
                _opts.fix_positon = 0
            }
            Othis.css({
                left: _opts.fix_positon
            })
        }
        function _MaxPosTop() {
...
(function() {
    ...
    if ("undefined" != typeof nonInsureFlag && nonInsureFlag == "1") {
        document.write('<div id="Weixi" style="display:none;"><script type="text/javascript" src="' + sinosoft.jsPath + '/nonInsure_weixitrack.js"><\/script></div>')
    } else {
...
if (jQuery("div[id='ajaxad']").find("p:eq(1)")) {
    jQuery("div[id='ajaxad']").find("p:eq(1)").each(function() {
        jQuery.getJSON(sinosoft.base + "/shop/ajax_ad!getAdContent.action?articleID=" + jQuery("#ArticleID").val() + "&callback=?", function(data) {
...
jQuery(document).ready(function() {
    ...
    jQuery(".m-service").taghoverbind({
        showClass: ".m-service_con"
    });
    jQuery(".m-WeChat").taghoverbind({
        showClass: ".m-WeChatImg"
    });
    jQuery(".m-WeChat a").taghoverbind({
        showClass: ".m-WeChatImg"
    });
    jQuery(".g-screen-tagul li").tagclickbind();
    jQuery(".m-travel-txt").inputfocusstyle();
    jQuery(".m-select_tag span").tagclickbind({
        addstyle: "select"
    });
    jQuery("#m-hotCityTit li").tagclickbind({
        con: "m-hotCity",
        addstyle: "select",
        mode: "p",
        nav: "m-hotCityTit"
...
timeCountDown.prototype.addEvent = function() {
    ...
    if (_this.isCounting) {
        ...
        switch (_this._opt.whichPage) {
            case "sign":
                _this.wrap.removeClass(_this.startClass).html(_this.endHtml).bind("click", function(e) {
                    e.preventDefault();
