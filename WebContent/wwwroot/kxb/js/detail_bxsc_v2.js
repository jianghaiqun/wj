...
jQuery.fn.selectDropDown = function(opt) {
	var _opt, jBody, jCp, jCpDiv, jCpUl, jCpPriceUl, jCpLi, jCpChild, jDaySelect, jDateAll, jDateBox, jDateStart, jDateEnd, jDateBtn, liArr, jBnrBox, jSel, jUl, jBnrPrice, nTemp, oTemp;
		...
		jCpChild.bind("click", function(e) {
			...
			if(complicatedFlag == "Y") { 
				...
				nTemp = jCpPriceUl.find("li > span")
		   				  ...
		   				  .text()
		   				  .slice(1);
		   	jCpChild.parent().find(":hidden").parent().removeClass("li_selected"); //删除隐藏选项的.li_selected
			}
...
jQuery.fn.complexProduct = function(opt) {
	...
	var _opts, elmBox, elmLi, tdList, jCpPriceUl, jBnrPrice, jPlanCon, jAutoSelDay, jDateBtn, jSelLi, nTemp, oInputDate, oSelList, oBnrBox;
		...
		if(jQuery(_this).parent().attr("id") == "birthday_li_id") {
			...
			if(jQuery(_this).children().attr("id").indexOf("Sex_") != -1 && jQuery("li#birthday_li_id").length > 0) {
				...
				if ((jQuery(_this).children().attr("id").indexOf("Period_") != -1 || jQuery(_this).children().attr("id").indexOf("FeeYear_") != -1) 
						&& jQuery("li#birthday_li_id").length > 0) {
