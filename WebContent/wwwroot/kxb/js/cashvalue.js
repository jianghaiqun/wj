	...
	if(def && def != null){
		...
		for(var i=0;i<defArray.length;i++){
			...
			if(_key.indexOf("TextAge") > 0){
				$("#TextAge_0").text(_value.substring(0,_value.length-1)+"周岁").attr("name",_value);
			}
			var selected = $("[name='"+_value+"']");
		...
		$("#inpRiskAppFactor_TextAge").val(birthday);
	}

	// 查询现金价值
	compRecalTemp("224801001");
