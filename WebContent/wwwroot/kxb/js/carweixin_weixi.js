...
// BEGIN
["sitetop","^(^/(index\.shtml)?$)?$"],
["pingan_cx_group","^/chexian/pingan\.shtml$"],
["taipingyang_cx_group","^/chexian/taipingyang\.shtml$"],
["yangguang_cx_group","^/chexian/yangguang\.shtml$"],
["dadi_cx_group","^/chexian/dadi\.shtml$"],
["tianping_cx_group","^/chexian/tianping\.shtml$"],
["total_cx_group","^(^/(chexian)?$)?$|^/chexian/.*$|^.*/cheliangbaoxian/.*$|^.*/50000100/.*$|^.*/50000101/.*$|^.*/50000102/.*$|^.*/40000002/.*$|^.*/50100000/.*$"],
["product_search","^/Search/Search.jsp\?query.*$"],
["product_lvyou","^/lvyou-baoxian/$"],
["product_guoneilvyou","^/lvyou-baoxian/guonei/$"],
["product_jingwailvyou","^/lvyou-baoxian/jingwai/$"],
["product_shengenqianzheng","^/lvyou-baoxian/shengenqianzheng/$"],
["product_yiwai","^/yiwai-baoxian/$"],
["product_jiaotongyiwai","^/yiwai-baoxian/jiaotong/$"],
["product_renshenyiwai","^/yiwai-baoxian/renshen/$"],
["product_hangkongyiwai","^/yiwai-baoxian/hangkong/$"],
["product_jiankang","^/jiankang-baoxian/$"],
["product_zhongdajibing","^/jiankang-baoxian/zhongdajibing/$"],
["product_tedingjibing","^/jiankang-baoxian/tedingjibing/$"],
["product_zhuyuanzhiliao","^/jiankang-baoxian/zhuyuanyiliao/$"],
["product_renshou","^/renshou-baoxian/$"],
["product_yanglao","^/renshou-baoxian/yanglaoxian/$"],
["product_dingqi","^/renshou-baoxian/dingqi/$"],
["product_licai","^/renshou-baoxian/licai/$"],
["product_jiacai","^/jiacai-baoxian/$"],
["product_chexian","^/chexian/$"],
["product_taipingyangchexian","^/chexian/taipingyang.shtml$"],
["product_pinganchexian","^/chexian/pingan.shtml$"],
["product_yangguangchexian","^/chexian/yangguang.shtml$"],
["product_dadichexian","^/chexian/dadi.shtml$"],
["product_tx","^/tx/$"],
["enterinfoform","^/wj/shop/order_config_new!buyNow.action\?.*$|^/wj/shop/order_config_new!keepInput.action\?.*$|^/wj/shop/order_config_new!buyNowUpdate.action\?.*$"],
["enterinfoshow","^/wj/shop/order_config_new!sendDirectUrl.action\?.*"],
["payment","^/wj/shop/member_shopcart!getShopCartINF.action\?.*$|^/wj/shop/order_config_new!pay.action\?.*$"],
["paysuccess","^/wj/shop/pay!doCallBack.action\?.*$|^/wj/shop/alipay!returnUrl.action\?.*$"],
["registerpage","^wj/shop/member_register!home.action.*"],
["loginpage","^/wj/shop/member!newLogin.action?.*"],
["zhishicontent","^/zhishi.*"],
["zixuncontent","^/zixun.*"],
["wendacontent","^/wenda.*"],
["baikecontent","^/baike.*"],
["zhuanticontent","^/zhuanti.*"],
["gongsicontent","^/gongsi.*"],
["jifencontent","^/jifen.*"],
["member_center","^/wj/shop/member_center!index\.action$|^/wj/shop/order_query!queryOrder\.action$|^/wj/shop/stow!queryScan.action$|^/wj/shop/my_compare!show.action$|/wj/shop/point!newList.action|^/wj/shop/question!questionList.action$|^/wj/shop/profile!edit.action$|^/wj/shop/coupon!queryCoupon.action$"],
["help_center","^/about/.*"]
// END
]
...
{	
	this.getparam='c=1003'+ addUnitFilterURLRule(_TheRuleArray);

	//var trace_protocol = location.protocol + '//'; 

	this.click_server = 'http://121.199.20.156';

	this.path  = 'VL/Trace';

	this.javascript_server = '';
...
{		
	...
	for (prop in Obj_VLTrace_ClassObj_GlobalValues) {	
		document.write(prop + " = '" + Obj_VLTrace_ClassObj_GlobalValues[prop] + "'<br>");
	}	

	var i;	
	for (i = 0 ; i < 10 ; i++) {	
		document.write("trace_uid[" + i + "] = '" + Obj_VLTrace_ClassObj_GlobalValues.trace_uid[i] + "'<br>");
	}	
	for (i = 0 ; i < 10 ; i++) {	
		document.write("trace_euid[" + i + "] = '" + Obj_VLTrace_ClassObj_GlobalValues.trace_euid[i] + "'<br>");
	}	

	document.write("VLTrace_Global_Var_Plugin = '" + VLTrace_Global_Var_Plugin + "'<br>");	



	if (Obj_VLTrace_ClassObj_GlobalValues.inquiry_call_param != '') {	
		document.write('[SCRIPT type="text/javascript" ');
		document.write('src="' + Obj_VLTrace_ClassObj_GlobalValues.trace_serverurl);
		document.write('?' + Obj_VLTrace_ClassObj_GlobalValues.inquiry_call_param);
...
function V5reqImg(ImageUrl, WaitTime, JsCodeClickLink, ClickLinkIsNewWindow){			
	...
	if ((isSafari || isFirefox) &&  !ClickLinkIsNewWindow) {	
		isWinXpSp2 = false;
	}	
...
function V5getTraceUrlFileDLExtLink( Obj, UrlType ){			
	...
	function getLinkUrl( Obj ){		
		var url = getProperty2(Obj, 'exturl');
		
		if(url == ""){
			url = getProperty2(Obj, 'href');
		}
...
{			
	...
	{		
		...
		if (objGV.trace_v == true) {	
			document.write('<img src="' + url +'" width="0" height="0" border="0" alt="">');
		}	
	}	
      ...
      if(typeof(this[p]) == 'string' || typeof(this[p]) == 'number'){
        ...
        if(index >= 0){
          ...
          if(this.param[index] == ''){
            ...
            //--kaixinbao special OrderID START
            if(p=='OrderID'){
            this.param[index] = 'W' + this.param[index];
            }
            //--kaixinbao special OrderID END
          }
          this[p] = '';
        }
      }
...
function addUnitFilterURLRule(unitTypeRealID){
	var curUri  = window.location.pathname;
	var upr = "";
	var length = unitTypeRealID.length;
    for(var i = 0; i<length; i++){
    	var urg = new RegExp(unitTypeRealID[i][1], "i");
    	if(urg.test(curUri)){
    		upr += "&u1=" + unitTypeRealID[i][0];
    		break;
    	}
    }
	return upr;
