...
<script type="text/javascript">
	function sear() {
		...
		DataGrid.setParam("dg1", "payStartDate", $V("payStartDate"));
		DataGrid.setParam("dg1", "payStartTime", $V("payStartTime"));
		DataGrid.setParam("dg1", "payEndDate", $V("payEndDate"));
		DataGrid.setParam("dg1", "payEndTime", $V("payEndTime"));
		...
		DataGrid.setParam("dg1", "realProvideUserSearch", $V("realProvideUserSearch"));
	...
	function batchsear(){
		DataGrid.setParam("dg1", Constant.PageIndex, 0);
		DataGrid.setParam("dg1", "batchflag","1");
		DataGrid.setParam("dg1", "couponSn", $V("couponSn"));
		DataGrid.setParam("dg1", "orderSn", $V("orderSn"));
		DataGrid.setParam("dg1", "status", $V("status"));
		DataGrid.setParam("dg1", "payStartDate", $V("payStartDate"));
	...
	function addCpsCoupon() {
		...
		diag.Height = 500;
		diag.Title = "新建优惠劵";
		diag.URL = "Coupon/CouponAdd.jsp?ActivityChannel=cps";
		diag.onLoad = function() {
			$DW.$('productTb').hide();
			createWindowD = diag;
			createWindowW = diag.Window;
	...
	function provide() {
		var arr = DataGrid.getSelectedValue("dg1");
		if (!arr || arr.length == 0) {
			Dialog.alert("请先选择要发放的优惠券！");
			return;
		}
		if (arr.length > 1) {
			Dialog.alert("请选择一个要发放的优惠券,如果需要选择发放多个优惠券,请使用批量上传发放功能！");
			return;
		}
		var provideuser=DataGrid.getSelectedValueByColumn("dg1","provideuser");
		var realname=$V("realname");
		if(provideuser.indexOf(realname)==-1){
			Dialog.alert("只有优惠券发放人包括自己的才可以发放！");
			return;
		}
		//用途为活动发放的不允许手动发放
		var purpose=DataGrid.getSelectedValueByColumn("dg1","purpose");
		if(purpose!="手动发放"){
			Dialog.alert("只有用途为\"手动发放\"的优惠券才可以手动发放！");
			return;
		}
		//获得勾选的固定列的值
		var valueByColumn=DataGrid.getSelectedValueByColumn("dg1","status");
		if(valueByColumn!="未使用"){
			Dialog.alert("只有\"未使用\"状态的优惠券才可以发放！");
			return;
		}
		Dialog.confirm("发放后不可恢复，确认发放？", function() {
			var diag = new Dialog("Diag1");
			diag.Width = 450;
			diag.Height = 120;
			diag.Title = "优惠劵发放";
			diag.URL = "Coupon/ProvideDialog.jsp";
			diag.onLoad = function() {
			};
			diag.OKEvent = provideSave;
			diag.ShowMessageRow = true;
			diag.MessageTitle = "优惠劵发放";
			diag.show();
		});
	}
	function provideSave() {
		...
		var dc = new DataCollection();
		dc.add("IDs", arr.join());
		dc.add("mailMobile", $DW.$V('mailMobile'));
		Server.sendRequest("com.wangjin.coupon.CouponInfo.provide", dc,
				function(response) {
					Dialog.alert(response.Message, function() {
						if (response.Status == 1) {
							DataGrid.loadData("dg1");
	...
	function frozen() {
		var arr = DataGrid.getSelectedValue("dg1");
		if (!arr || arr.length == 0) {
			Dialog.alert("请先选择要冻结的行！");
			return;
		}
		//获得勾选的固定列的值
		var valueByColumn=DataGrid.getSelectedValueByColumn("dg1","status");
		if(valueByColumn!="未使用"){
			Dialog.alert("只有\"未使用\"状态的优惠券才可以冻结！");
			return;
		}
		//已关联活动的优惠券不允许更改
		var activitysn=DataGrid.getSelectedValueByColumn("dg1","activitysn");
		if(!(activitysn==""||activitysn==null)){
			Dialog.alert("已关联活动的优惠券不可以冻结！");
			return;
		}
		Dialog.confirm("确认要冻结么？", function() {
			var dc = new DataCollection();
			dc.add("IDs", arr.join());
			Server.sendRequest("com.wangjin.coupon.CouponInfo.frozen", dc, function(
					response) {
				Dialog.alert(response.Message, function() {
					if (response.Status == 1) {
						DataGrid.loadData("dg1");
	...
	function unFrozen() {
		var arr = DataGrid.getSelectedValue("dg1");
		if (!arr || arr.length == 0) {
			Dialog.alert("请先选择要解冻的行！");
			return;
		}
		//获得勾选的固定列的值
		var valueByColumn=DataGrid.getSelectedValueByColumn("dg1","status");
		if(valueByColumn!="冻结"){
			Dialog.alert("只有\"冻结\"状态的优惠券才可以解冻！");
			return;
		}
		Dialog.confirm("确认要解冻么？", function() {
			var dc = new DataCollection();
			dc.add("IDs", arr.join());
			Server.sendRequest("com.wangjin.coupon.CouponInfo.unFrozen", dc, function(
					response) {
				Dialog.alert(response.Message, function() {
					if (response.Status == 1) {
						DataGrid.loadData("dg1");
	...
	function provideBatch() {
		var diag = new Dialog("Diag1");
		diag.Width = 400;
		diag.Height = 150;
		diag.Title = "优惠券批量发放导入";
		diag.URL = "Coupon/Provide_batchDialog.jsp";
		diag.show();
		diag.OKButton.hide();
	...
	//线下发放并导出
	function provideExcel(){
		var arr = DataGrid.getSelectedValue("dg1");
		var purpose=DataGrid.getSelectedValueByColumn("dg1","purpose");
		var row =  DataGrid.getSelectedData("dg1").Rows;
		for(var i=0;i<row.length;i++){
			var dr=row[i];
			var r = dr.get("purpose");
			if(r!="手动发放"){
				Dialog.alert("选择要线下发放的优惠券用途必须全部为\"手动发放\"！");
				return;
			}
		}
	...
	//优惠券批次导出
	function CouponBatchExcel(){
		var Batch = $V("batch");
		if (Batch == null || Batch == '') {
			Dialog.alert("请先选择要发放并导出的优惠券！");
			return;
		}
		Dialog.confirm("发放后不可恢复，确认发放？", function() {
			DataGrid.selectedRowToExcel("dg1");
			...
			<tr>
				...
				<fieldset><legend> <label>优惠劵</label> </legend> <z:init
					...
					<table>
						<tr>
							...
							<td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
							<td height="35" align="right" bordercolor="#eeeeee"
								class="tdgrey1">使用单号：</td>
							<td><input value="" type="text" id="orderSn" name="orderSn"
					...
					<table>
						<tr>
							...
							<td><input value="" type="text" id="provideUserSearch" name="provideUserSearch"
								ztype="String" class="inputText" size="20"></td>
							<td height="35" align="right" bordercolor="#eeeeee"
								class="tdgrey1">&nbsp;&nbsp;&nbsp;实际发放人：</td>
							<td><input value="" type="text" id="realProvideUserSearch" name="realProvideUserSearch"
								ztype="String" class="inputText" size="20"></td>
							<td height="35" align="right" bordercolor="#eeeeee"
					...
					<table>
						<tr>
							<td height="35" align="right" bordercolor="#eeeeee"
								class="tdgrey1">所属邮箱：</td>
							<td><input value="" type="text" id="email" name="email" 
								ztype="String" class="inputText" size="20"></td>
							<td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
							<td height="35" align="right" bordercolor="#eeeeee"
								class="tdgrey1">所属手机：</td>
							<td><input value="" type="text" id="mobile" name="mobile"
								ztype="String" class="inputText" size="20"></td>
							<td height="35" align="right" bordercolor="#eeeeee"
								class="tdgrey1">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
							<td height="35" align="right" bordercolor="#eeeeee"
					...
					<table>
						<tr>
							<td height="35" align="right" bordercolor="#eeeeee"
								class="tdgrey1">使用开始时间：</td>
							<td><input name="payStartDate" id="payStartDate" value=""
								type="text" size="14" ztype="Date" /></td>
							<td><input name="payStartTime" id="payStartTime" value=""
								type="text" size="14" ztype="Time" /></td>
							<td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
							<td height="35" align="right" bordercolor="#eeeeee"
								class="tdgrey1">使用结束时间：</td>
							<td><input name="payEndDate" id="payEndDate" value=""
								type="text" size="14" ztype="Date" /></td>
							<td><input name="payEndTime" id="payEndTime" value=""
								type="text" size="14" ztype="Time" /></td>
						</tr>
					</table>
					<table>
				...
				<table>
					<tr>
					 ...
					 <z:tButtonPurview>${_Coupon_CouponInfo_Button}</z:tButtonPurview>
			<tr>
				<td>&nbsp;</td>
			</tr>
			<tr>
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="6"
					...
					<tr>
						...
						<z:datagrid id="dg1"
							method="com.wangjin.coupon.CouponInfo.dg1DataBind" size="12"
