 ...
 *  
 *
 * KEY:SINOSOFT25805FB3BE09EA71895EAB4E18BEB912
 * ============================================================================
 */
...
public class CartItemAction extends BaseShopAction {
	...
	private String sky;// 测试公司简介
	private String orderId;// 测试公司简介
...
//		if (quantity > 1) {
//			return ajaxJsonErrorMessage("添加购物车失败,本次购物同一种产品只能购买一次!");
//		}
		...
		if (loginMember == null) {
						...
						if (StringUtils.isNotEmpty(cookie.getValue())) {
							...
							for (CartItemCookie previousCartItemCookie : previousCartItemCookieList) {
								...
								if (StringUtils.equals(previousCartItemCookie.getI(), id)) {
									...
									if (product.getStore() != null && (previousCartItemCookie.getQ()) > 1) {
										return ajaxJsonErrorMessage("添加购物车失败,本次购物同一种产品只能购买一次!");
									}
									if (product.getStore() != null && (product.getFreezeStore() + previousCartItemCookie.getQ()) > product.getStore()) {
										return ajaxJsonErrorMessage("添加购物车失败，商品库存不足1!");
									}
								}
			...
			if (!isExist) {
				...
				if (product.getStore() != null && (cartItemCookie.getQ()) > 1) {
					return ajaxJsonErrorMessage("添加购物车失败,本次购物同一种产品只能购买一次!");
				}
				if (product.getStore() != null && (product.getFreezeStore() + cartItemCookie.getQ()) > product.getStore()) {
					return ajaxJsonErrorMessage("添加购物车失败，商品库存不足2!");
				}
			}
			for (CartItemCookie cartItemCookie : cartItemCookieList) {
				if (StringUtils.equals(cartItemCookie.getI(), id)) {
					...
					if (product.getStore() != null && (cartItemCookieProduct.getFreezeStore() + cartItemCookie.getQ()) > cartItemCookieProduct.getStore()) {
						return ajaxJsonErrorMessage("添加购物车失败，商品库存不足3!");
					}
				}
			}
			...
			if (previousCartItemList != null) {
				for (CartItem previousCartItem : previousCartItemList) {
					if (StringUtils.equals(previousCartItem.getProduct().getId(), id)) {
						...
						if (product.getStore() != null && (previousCartItem.getQuantity()) > 1) {
							return ajaxJsonErrorMessage("添加购物车失败,本次购物同一种产品只能购买一次!");
						}
						if (product.getStore() != null && (product.getFreezeStore() + previousCartItem.getQuantity()) > product.getStore()) {
							return ajaxJsonErrorMessage("添加购物车失败，商品库存不足4!");
						}
			...
			if (!isExist) {
				...
				if (product.getStore() != null && (cartItem.getQuantity()) > 1) {
					return ajaxJsonErrorMessage("添加购物车失败,本次购物同一种产品只能购买一次!");
				}
				if (product.getStore() != null && (cartItem.getQuantity()) > product.getStore()) {
					return ajaxJsonErrorMessage("添加购物车失败，商品库存不足5!");
				}
		...
		if (loginMember == null) {
			...
			if (cartItemSet != null) {
				for (CartItem cartItem : cartItemSet) {
					...
					if(cartItem.getValid()==true){
						OrderItem orderItem = orderItemService.getByOrP(id, orderId);
