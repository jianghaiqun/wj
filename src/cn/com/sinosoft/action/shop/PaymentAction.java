...
import cn.com.sinosoft.bean.TenpayConfig.TenpayType;
import cn.com.sinosoft.entity.Deposit;
import cn.com.sinosoft.entity.Deposit.DepositType;
 ...
 *  
 *
 * KEY:SINOSOFT0D8A50887A9F4C77410AE65149244875
 * ============================================================================
 */

@ParentPackage("member")
@InterceptorRefs({
	@InterceptorRef(value = "token", params = {"excludeMethods", "confirm,tenpayReturn,alipayto"}),
...
public class PaymentAction extends BaseShopAction {
	...
	private List<PaymentConfig> paymentConfigs = new ArrayList<PaymentConfig>();
	...
	public String alipayto() throws Exception {
		return "alipayto";
	}
...
//			paymentConfig = order.getPaymentConfig();
		...
		paymentConfigs = paymentConfigService.getAll();
		setResponseNoCache();
		return "confirm";
	}
...
//			System.out.println("order编号是1："+order.getOrderSn());
//			System.out.println("order应付金额1："+order.getProductTotalPrice());
//			System.out.println("order备注描述1："+order.getMemo());
			if(order.getOrderItemSet().iterator().hasNext()){
//				System.out.println("order订单名称1："+order.getOrderItemSet().iterator().next().getProductName());
			}
//			paymentConfig = order.getPaymentConfig();
...
//				return ERROR;
//			}
			if (order.getPaymentStatus() == cn.com.sinosoft.entity.Order.PaymentStatus.paid) {
...
//			}
			paymentFee = order.getPaymentFee();
			amountPayable = order.getProductTotalPrice().subtract(order.getPaidAmount());
...
//			System.out.println("order编号是2："+order.getOrderSn());
