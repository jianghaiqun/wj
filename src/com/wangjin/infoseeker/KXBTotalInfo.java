...
public class KXBTotalInfo extends Page {
	...
	public void dg1DataBind(DataGridAction dga) {
		...
		if (StringUtil.isNotEmpty(channelsn)) {
			String channel = QueryUtil.getKXBChannelInfo(channelsn,"");
			parameter = parameter + " and EXISTS (SELECT 1 FROM KXBChannelInfo WHERE ChannelCode = a.channelsn and ChannelCode IN ("+channel+"))";
	...
	public DataTable dg1DataBindB2B(DataGridAction dga,String channelSn) {
		...
		if(StringUtil.isNotEmpty(channelsn)){
			String channel = QueryUtil.getKXBChannelInfo(channelsn, "b2b");
			if(StringUtil.isEmpty(channel)){
				return null;
			}
			//parameter = parameter + " and s.ChannelSn in ("+channel+")";
			//惠租车作为渠道查询，sql调整
			if (channelsn.indexOf("huizuche") >= 0) {
				parameter = parameter + " and (s.ChannelSn in ("+channel+") or s.memberid in ('huizuche','huizuche001')) ";
			}
			else {
				parameter = parameter + " and s.ChannelSn in ("+channel+") and s.memberid not in ('huizuche','huizuche001') ";
			}
		}
		StringBuffer sql = new StringBuffer();
		QueryBuilder qb = null;
	...
	public void dg3DataBind(DataGridAction dga) {
		...
		if (StringUtil.isNotEmpty(channelsn)) {
			String channel = QueryUtil.getKXBChannelInfo(channelsn,"");
			parameter = parameter + " and EXISTS (SELECT 1 FROM KXBChannelInfo WHERE ChannelCode = orders.channelsn and ChannelCode IN ("+channel+"))";
