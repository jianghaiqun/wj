...
public class QueryServlet extends HttpServlet {
	...
	public void service(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			//modify by wangchangyang bug0000424: CPA和CPS获取订单信息变更 start
			String sql_query = "select C.cid,C.wi,C.on,C.ta,C.pna,C.dt,C.pp,C.sd,O.modifyDate as ud,O.orderStatus as os,O.payStatus as ps,T.payType as pw from cps C left join SDorders O on  C.on=O.orderSn left join tradeinformation T on C.on=T.ordID  where (C.openid IS NULL or C.openid='null' or C.openid='')  ";
			//modify by wangchangyang bug0000424: CPA和CPS获取订单信息变更 end
			if (StringUtils.isNotBlank(cid)) {
				sql_query = sql_query + " and C.cid='" + cid + "'";
				if (StringUtils.isNotBlank(date)) {
					sql_query = sql_query + " and DATE_FORMAT(C.sd,'%Y%m%d')='"
							+ date + "'";
				} else if (StringUtils.isNotBlank(ud)) {
					sql_query = sql_query
							+ " and DATE_FORMAT(O.modifyDate,'%Y%m%d')='" + ud
							+ "'";
				}
			} else {
				if (StringUtils.isNotBlank(date)) {
					sql_query = sql_query
							+ " and DATE_FORMAT(C.sd,'%Y%m%d')='" + date
							+ "'";
				} else if (StringUtils.isNotBlank(ud)) {
					sql_query = sql_query
							+ " and DATE_FORMAT(O.modifyDate,'%Y%m%d')='"
							+ ud + "'";
				}
			}
