 ...
 * Function: 保存百年订单的信息. <br/>
 * Date:2016年5月31日 下午5:45:38 <br/>
 *
 * @author:chouweigao
 */
public class AeonlifeService extends BaseServiceImpl<Object, String> {
	public static final String BILL_PAY_TYPE = "bill_qb";
	public static final String ZJZF_PAY_TYPE = "zjzf";
	...
	public void executeSave(Map<String, Object> cInputData) {
		...
		for (String key : set) {
			...
			// 导入功能使用，存储订单支付时 下单时间
			aeonCancelDate = aeon.getCancelDate();
			...
			try {
				...
				logger.error("百年稳赢保终身寿险订单保存失败:{}", e.getMessage());
	...
	public Map<String, Object> executeUpdate(Map<String, Object> cInputData) {
		...
		if (!cInputData.isEmpty()) {
			...
			String querySql = "SELECT channelSn, bgCancelResultUrl,partnerKey,partnerId FROM PartnerInfo WHERE type = 'syn' AND bgCancelResultUrl <> '' AND bgCancelResultUrl IS NOT NULL";
			DataTable piQb = new QueryBuilder(querySql).executeDataTable();
			for (int i = 0; i < piQb.getRowCount(); i++) {
				DataRow row = piQb.get(i);
				...
				// 推送数据
				if (StringUtil.isNotEmpty(row.getString("bgCancelResultUrl"))) {

					// 对应 合作方渠道获取数据
					QueryBuilder qb = new QueryBuilder(sql, row.getString("channelSn"));
					DataTable dt = qb.executeDataTable();
					if (dt.getRowCount() > 0) {
						// 出单回调-发到到第三方消息队列
						try {
							//通过订单号查询订单信息，回调第三方 退保信息
							logger.info("sendLicaiCancalToReceiver-partnerId：{}", row.getString("partnerId"));
							PayBaseService payService = new PayBaseServiceImpl();
							payService.sendLicaiCancalToReceiver(row.getString("partnerId"));
						} catch (Exception e) {
							logger.error("退保结果回调-发到到第三方消息队列出现异常=======payUrlService.sendLicaiCancalToReceiver" + e.getMessage(), e);
						}
					} 
				}
			}
		...
		//新增理财险退保
				String lcx_b2b = "SELECT d.channelsn,d.memberId,d.orderSn,fi.cancelDate,a.productId,fi.principal,fi.policyno,a.startDate,a.endDate FROM financinginfo AS fi ,SDInformation a, sdorders d"
											+" WHERE fi.OrderSn = a.orderSn AND fi.OrderSn = d.orderSn AND (d.channelsn ='b2b_app' or d.channelsn ='b2b_html5')"
											+" AND DATE_FORMAT(DATE_SUB(CURDATE(),INTERVAL 1 DAY),'%Y-%m-%d') = DATE_FORMAT(fi.CancelDate,'%Y-%m-%d')"
											+" AND DATE_FORMAT(a.startDate,'%Y-%m-%d') > DATE_FORMAT(DATE_SUB(fi.CancelDate,INTERVAL 10 DAY),'%Y-%m-%d') AND fi.InsStatus ='3'";
				
				QueryBuilder qb_lcx = new QueryBuilder(lcx_b2b);
				DataTable dt_lcx = qb_lcx.executeDataTable();
				if (dt_lcx.getRowCount() > 0) {
					...
					for (int i = 0; i < dt_lcx.getRowCount(); i++) {
						...
						String orderSn = dt_lcx.getString(i, "orderSn");
						String createDate = dt_lcx.getString(i, "cancelDate");
						String productId = dt_lcx.getString(i, "productId");
						String principal = dt_lcx.getString(i, "principal");
						String policyno = dt_lcx.getString(i, "policyno");
						String startDate = "";
						String format = "yyyy-MM-dd HH:mm:ss";
						String endDate = "";
						Date date = DateUtil.strToDate(createDate, format);
						cc.dealB2bBalanceInfo(channelsn, userName, orderSn, date, productId, "-"+principal, policyno, startDate, endDate);
					}
				}
	 ...
	 * @return
	 */
	public Map<String, Object> genResaultMap(Map<String, Object> resMap, Map<String, Object> headerMap){
		Map<String, Object> rMap = new HashMap<String, Object>();
		rMap.put("header", headerMap);
		logger.error("body:{}", JSONObject.fromObject(resMap).toString());
		rMap.put("body", AesUtilQN.encrypt(JSONObject.fromObject(resMap).toString(), (String) headerMap.get("secretkey")));
		// 对body体进行加密处理
		return rMap;
	}
	...
	private void saveOrUpdateFinancinginfo(Aeon aeon) {
		...
		try {
			...
			sql = "INSERT INTO financinginfo (PolicyNo, OrderSn,  Total, Principal, Income, InsStatus, Prop2, CreateDate, ModifyDate) "
			...
			logger.error("资产同步异常" + e.getMessage(), e);
		}
		...
		if ((stauts.equals("1") && row == 0) || (stauts.equals("2") && row == 1)) {
			...
			if (dtab.getRowCount() > 0) {
				...
				if (dtab.getRowCount() > 0) {
					...
					logger.error("会员号：{}|订单号：{}|剩余总金额：{}", argArr);
				}
				if (row == 0) {// 承保
				...
				logger.error("资产同步Record—sql语句========={}", ins_sql);
	...
	private String[] concatString(Aeon aeon) {
		...
		String todayincome = aeon.getTodayincome();
		...
		sb.append(str).append(pno).append(str).append(SftpCommon.SPLIT_STR)
				...
				.append(str).append(todayincome).append(str).append(SftpCommon.SPLIT_STR);
		arr[0] = sb.toString();
		// 拼装update语句
		sb = new StringBuffer();
		sb.append("Total=").append(str).append(total).append(str).append(SftpCommon.SPLIT_STR)
				...
				.append("Income=").append(str).append(income).append(str).append(SftpCommon.SPLIT_STR)
				.append("Prop2=").append(str).append(todayincome).append(str);
	...
	private void update(Aeon aeon) throws Exception {
		...
		// 保全号
		String remarkNo = aeon.getAeonPolicyNo();
		String insStatus = "4";

		// 退保状态1：成功 0失败
		if (StringUtil.isNotEmpty(aeonPolicyResult) && ("1".equals(aeonPolicyResult) || "2".equals(aeonPolicyResult))) {
			/**
			 * 更新状态
			 */
			String update_orders = "UPDATE sdorders SET OrderStatus = '9',MODIFYDATE = NOW() WHERE paysn ='" + aeonOrderSn
					+ "' or ordersn ='" + aeonOrderSn + "'";
			logger.error("update_orders=========={}", update_orders);
			...
			String update_riskType = "UPDATE sdinformationrisktype SET appStatus ='2',cancelDate ='" + cancelDate_str
					+ "' WHERE ordersn = (SELECT ordersn FROM sdorders WHERE paysn ='" + aeonOrderSn + "' or ordersn ='"
					+ aeonOrderSn + "')";
			logger.error("update_riskType========={}", update_riskType);
		 ...
		 * 更新自己的
		 */
		String update_financinginfo = "UPDATE financinginfo SET InsStatus = '" + insStatus + "',Total='" + total
				+ "',principal='"
				+ principal + "',income='" + income + "' ,fee='" + fee + "' ,returnType='"
				+ returnType + "' ,Prop1='" + remarkNo + "' ,cancelResult='" + aeonPolicyResult + "' ,CancelMsg='" + aeonMsg
				+ "',MODIFYDATE = NOW() WHERE OrderSn ='"
				+ aeonOrderSn + "' and InsStatus <> '3'";
		logger.error("update_financinginfo========={}", update_financinginfo);
	...
	private void executeSave() throws Exception {
		...
		if (paysn.startsWith("LCBX")) {
			createDate = aeonCancelDate;
		} else {
			createDate = new Date();
		}
		...
		if (StringUtil.isEmpty(productId)) {
			logger.error("未在zdconfig的value中查询到百年稳赢保终身寿险");
			return;
		}
		/**
		...
		if (fms != null) {
			...
			TradeInfoSchema tradeInfoSchema = getTradeInfoSchema();
			...
			tr.add(tradeInfoSchema,Transaction.INSERT);
			tr.commit();
		} else {
	...
	private SDOrdersSchema getSDorder() {
		...
		// 支付类型
		sdorders.setpayType(ZJZF_PAY_TYPE);
		// 待付状态为已支付
	...
	private SDInformationRiskTypeSchema getSDinformationrisktype(SDInformationSchema sdinformation) {
		...
		logger.info("获得SDInformationRiskType");
		...
		if (paysn.startsWith("LCBX")) {
			sdinformationRiskType.setinsureDate(DateUtil.toDateTimeString(aeonCancelDate));
		} else {
	...
	private SDInformationAppntSchema getSDinformationappnt() {
		...
		logger.info("获得SDInformationAppnt");
	...
	private SDInformationInsuredSchema getSDinformationinsured(SDInformationAppntSchema sdinformationappnt) {
		...
		logger.info("获得SDInformationInsured");
	...
	private TradeSummaryInfoSchema getTradeSummaryInfoSchema() {
		...
		tss.setPayType(ZJZF_PAY_TYPE);
		tss.setPayTypeName("银行卡直接支付");
		tss.setTradeAmount(aeonAmount);
		tss.setTotalAmount(aeonAmount);
		tss.setActivitySumAmount("0.00");
		tss.setCouponSumAmount("0.00");
		tss.setPointSumAmount("0.00");
		return tss;
	}
	
	private TradeInfoSchema getTradeInfoSchema(){
		TradeInfoSchema tis = new TradeInfoSchema();
		
		tis.setpaySn(paysn);
		tis.setorderSn(orderSn);
		tis.setpayType(ZJZF_PAY_TYPE);
		tis.settotalAmnout(aeonAmount);
		tis.setremark1("已支付");
		tis.setintegral("0");
	...
	private TradeInformationSchema getTradeInformationSchema() {
		...
		if (paysn.startsWith("LCBX")) {
			tis.setreceiveDate(DateUtil.toDateTimeString(aeonCancelDate));
			tis.setsendDate(DateUtil.toDateTimeString(aeonCancelDate));
		} else {
	...
	/**
	 * 执行百年爆品的退保逻辑
	 * @param dataCol 读取百年传送的退保文件内容集合（以行为单位）
	 */
	public void executeSurrend4BaoPin(List<String> dataCol){
		for (int i = 2; i < dataCol.size(); i++) {
			String channelsn = "";
			String memberId = "";
			String productId = "";
			String comCode = "";
			...
			String[] infos = dataCol.get(i).split("\\|",-1);
			String sql = "select a.insuranceCompany , c.appStatus ,a.ordersn ,c.id,c.balanceStatus ,d.channelsn ,d.memberId ,a.productId,a.startdate,a.enddate,b.insuredSn "
					+ "from SDInformation a, sdorders d, sdinformationinsured b ,sdinformationrisktype c "
					+ "where a.informationSn=b.informationSn and b.recognizeeSn = c.recognizeeSn and a.orderSn = d.orderSn and c.policyNo=?";
			DataTable dt = new QueryBuilder(sql, infos[0]).executeDataTable();
			if (dt.getRowCount() == 1) {
				...
				insuredSn = dt.getString(0, 10);
			}
			if (!"0".equals(balanceStatus) && "1".equals(appStatus)) {
				TBContInsureServlet tb = new TBContInsureServlet();
				tb.callFCContService(ordersn);
				balanceStatus = new QueryBuilder("select t.balanceStatus from sdinformationrisktype t where t.id= ?", riskTypeId).executeString();
				if(!"0".equals(balanceStatus)){
					continue;
				}
			}
			...
			if (("1".equals(appStatus) || "3".equals(appStatus)) && StringUtil.isNotEmpty(insuredSn)
				...
				if (sdRiskTypeSchema.fill()) {
					...
					Date date = DateUtil.parse(infos[2], DateUtil.Format_Date2);
					sdRiskTypeSchema.setcancelDate(date);
			...
			} else if ("2".equals(appStatus)) {//已经撤单
				continue;
				...
				if (!sdRiskTypeSet.isEmpty() && sdRiskTypeSet.size() == 1) {
					...
					if ("4".equals(sdRiskTypeSchema.getappStatus()) && !"2".equals(sdRiskTypeSchema.getbalanceStatus())) {
						...
						if (tCancelContService.callProductInterface(ordersn, "", PubFun.getCurrentDate(), "02",
							...
							try {
								cancelCont.cancelCpsOrder(ordersn);
								if (Constant.B2B_APP.equals(channelsn)  || Constant.B2B_HTML5.equals(channelsn)) {
									CancelCont cc = new CancelCont();
									cc.dealB2bBalanceInfo(channelsn, memberId, ordersn,
											sdRiskTypeSchema.getcancelDate(), productId,
											"-"+sdRiskTypeSchema.getpayPrice(),sdRiskTypeSchema.getrecognizeeSn(),sdRiskTypeSchema.getsvaliDate().toString(),sdRiskTypeSchema.getevaliDate().toString());
								}
