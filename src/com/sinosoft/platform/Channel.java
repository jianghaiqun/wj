...
public class Channel extends Page {
	...
	public static void dg1DataBind(DataGridAction dga) {
		String sql = "select InnerChannelCode ID,ParentInnerChanelCode ParentID,CONCAT(ChannelCode,'-',ChannelName) Name,ChannelLevel as TreeLevel,"
					+"CASE IsCoupon when 'Y' THEN '是' WHEN 'N' THEN '否' ELSE '' END IsCoupon,CASE IsActivity when 'Y' THEN '是' WHEN 'N' THEN '否' ELSE '' END IsActivity from ChannelInfo order by CreateDate";
		DataTable dt = new QueryBuilder(sql).executeDataTable();
		dga.bindData(dt);
	}
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public static Mapx dg2DataBind(Mapx params) {
		String sql = "select ChannelCode ID,InnerChannelCode,ParentInnerChanelCode ParentID,ChannelName Name,ChannelLevel as TreeLevel,Prop1,IsCoupon,IsActivity,orderflag from ChannelInfo where InnerChannelCode = "+params.get("Id");
		DataTable dt = new QueryBuilder(sql).executeDataTable();
		...
		DataTable dt1 = new QueryBuilder("select ChannelName name,InnerChannelCode id from ChannelInfo order by CreateDate").executeDataTable();
		params.put("ParentMenu", HtmlUtil.dataTableToOptions(dt1,dt.get(0, "ParentID")));
		params.put("OrderFlag", dt.get(0, "orderflag"));
		return params;
	}
	public void Edit() {
		...
		for (int i = 0; i < orderFlag.length(); i++) {
			if (!Character.isDigit(orderFlag.charAt(i))) {
				Response.setStatus(0);
				Response.setMessage("排序等级请填入有效数字");
				return;
			}
		}
		...
		DataTable dt1 = new QueryBuilder("select InnerChannelCode,ChannelLevel from ChannelInfo where ChannelCode =?",code).executeDataTable();
		...
		String insertSQL = " UPDATE ChannelInfo set ChannelName=?,ParentInnerChanelCode=?,ChannelLevel=?,ModifyDate=?,Prop1=?,IsCoupon=?,IsActivity=?,orderflag=? where ChannelCode=? ";
		...
		if(channelDiff!=0){
			for(int i=0;i<arr.length;i++){
				if(!"$".equals(arr[i]) && !innerCode.equals(arr[i])){
					QueryBuilder qb2 = new QueryBuilder(" UPDATE ChannelInfo SET ChannelLevel=ChannelLevel+? WHERE InnerChannelCode=? ");
					qb2.add(channelDiff);
					qb2.add(arr[i]);
					trans.add(qb2);
				}
	...
	public static Mapx init(Mapx params) {
		Mapx map = new Mapx();
		DataTable dt = new QueryBuilder("select ChannelName name,InnerChannelCode id from ChannelInfo order by CreateDate").executeDataTable();
	...
	public void add() {
		...
		int k = new QueryBuilder(" select count(1) from ChannelInfo where ChannelCode=? ",code).executeInt();
		if(k>=1){
			Response.setStatus(0);
		...
		Transaction trans = new Transaction();
		String insertSQL = " INSERT INTO ChannelInfo(ChannelCode,InnerChannelCode,ChannelName,ParentInnerChanelCode,ChannelLevel,CreateDate,ModifyDate,Prop1,IsCoupon,IsActivity,orderflag) VALUE(?,?,?,?,?,?,?,?,?,?,?) ";
		if(StringUtil.isEmpty(parentID)){
			//一级渠道
			parentID = "0";
		}
	...
	public static String getInnerChannelCode(String parentId){
		...
		if(StringUtil.isEmpty(parentId) || "0".equals(parentId)){
			innerChannelCode = StringUtil.rightPad(NoUtil.getMaxNo("INNERCHANNELCODE"), '0', 5);
		}else{
			innerChannelCode = parentId+StringUtil.rightPad(NoUtil.getMaxNo("INNERCHANNELCODE"), '0', 5);
		}
	...
	public static String getChannelLevel(String cChannelCode){
		...
		DataTable dt = new QueryBuilder(" SELECT ChannelLevel FROM ChannelInfo WHERE innerChannelCode=? ",cChannelCode).executeDataTable();
	...
	public void del() {
		...
		QueryBuilder qb1 = new QueryBuilder(" SELECT InnerChannelCode,ChannelName FROM ChannelInfo where InnerChannelCode in (" + ids + ")");
		DataTable dt1 = qb1.executeDataTable();
		int tLen = dt1.getRowCount();
		if(dt1!=null && tLen>=1){
			for(int j=0;j<tLen;j++){
				long count = new QueryBuilder("select count(*) from ChannelInfo where ParentInnerChanelCode=" + dt1.getString(j, "InnerChannelCode") + " and InnerChannelCode not in (" + ids + ")").executeLong();
				if (count > 0) {
					Response.setStatus(0);
		...
		QueryBuilder qb2 = new QueryBuilder(" DELETE FROM ChannelInfo where InnerChannelCode in (" + ids + ")");
		Transaction trans = new Transaction();
		trans.add(qb2);
		if (trans.commit()) {
			Response.setStatus(1);
	...
	public static void treeDataBindForVip(TreeAction ta) {
		...
		if(dt.getRowCount() > 0){
			for(int i=0;i<dt.getRowCount();i++){
				DataRow dr = dt.get(i);
				if("tm".equals(dr.getString("ChannelCode"))){
					String parentId = dr.getString("ID");
					dt.insertRow(new Object[]{ "tb_ht_wx", getInnerChannelCode(parentId), parentId, "淘宝（微信已激活）", 1});
					dt.insertRow(new Object[]{ "tb_my_wx", getInnerChannelCode(parentId), parentId, "支付宝（微信已激活）", 1});
					dt.insertRow(new Object[]{ "tb_ht_other", getInnerChannelCode(parentId), parentId, "淘宝（其他已激活）", 1});
					dt.insertRow(new Object[]{ "tb_my_other", getInnerChannelCode(parentId), parentId, "支付宝（其他已激活）", 1});
				}
			}
		}
		ta.bindData(dt);
	...
	public static void treeDataBind(TreeAction ta) {
		...
		sql.append(" ORDER BY orderflag ASC ");
		dt = new QueryBuilder(sql.toString()).executeDataTable();
		ta.setRootText("订单渠道");
		dt.setWebMode(false);
		ta.bindData(dt);
	...
	public static void treeAllDataBind(TreeAction ta) {
		...
		//int parentLevel = params.getInt("ParentLevel");
		String parentID = (String) params.get("ParentID");
		StringBuffer sql = new StringBuffer("select ChannelCode,InnerChannelCode ID,ParentInnerChanelCode ParentID,ChannelName Name,ChannelLevel as TreeLevel from ChannelInfo ");
		if(StringUtil.isNotEmpty(parentID)){
			String innercode = new QueryBuilder("select InnerChannelCode from ChannelInfo where ChannelCode='"+parentID+"'").executeString();
			if("Member".equals(parentID)){
				innercode = new QueryBuilder("select InnerChannelCode from ChannelInfo where ChannelCode='b2b'").executeString();
	...
	public static void treeDataBindForCoupon(TreeAction ta) {
		...
		StringBuffer sql = new StringBuffer("select ChannelCode,InnerChannelCode ID,ParentInnerChanelCode ParentID,ChannelName Name,ChannelLevel as TreeLevel,'' as Checked,'' as disabled from ChannelInfo where Prop1='Y' and IsCoupon='Y' ");
		sql.append(" order by CreateDate ");
		dt = new QueryBuilder(sql.toString()).executeDataTable();
	...
	public static void treeDataBindForActivity(TreeAction ta) {
		...
		StringBuffer sql = new StringBuffer("select ChannelCode,InnerChannelCode ID,ParentInnerChanelCode ParentID,ChannelName Name,ChannelLevel as TreeLevel,'' as Checked,'' as disabled from ChannelInfo where Prop1='Y' and IsActivity = 'Y' ");
		sql.append(" order by CreateDate ");
		dt = new QueryBuilder(sql.toString()).executeDataTable();
		if (StringUtil.isNotEmpty(csnid)) {
