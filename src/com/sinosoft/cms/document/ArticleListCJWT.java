...
public class ArticleListCJWT extends Page {
	...
	public static void dg1DataBind(DataGridAction dga) {
		...
		QueryBuilder qb = new QueryBuilder(
				"select ID,Attribute,Title,URL,Author,PublishDate,Addtime,Status,WorkFlowID,Type,"
						+ "TopFlag,OrderFlag,TitleStyle,TopDate,ReferTarget,ReferType,ReferSourceID from "
						+ Table + " where CatalogID=?");
		qb.add(catalogID);
	...
	public void create() {
		...
		if (tSCFAQSet.size() > 0) {
			...
			for (int i = 0; i < tSCFAQSet.size(); i++) {
				...
				FaqHtml += "<li>";
				FaqHtml += "<p class=\"q_cont\"><span class=\"icon\"></span>";
				FaqHtml += tSCFAQSchema1.getFAQName();
				FaqHtml += "</p>";
				FaqHtml += "<p class=\"a_cont\"><span class=\"icon\"></span>";
			...
			if(tZCArticleSchema.getStatus()!=0){
				...
				articleLog.setAction("REDIT");
				articleLog.setActionDetail("常见问题-重新编辑");
