...
public class CatalogYXCP extends Page {
	...
	public static Mapx init(Mapx params) {
		...
		if (o1 != null) {
			...
			if (catalog.fill()) {
				...
				map.put("ProductType", HtmlUtil.codeToOptions("ProductType", catalog.getProductType(),true));
				return map;
			}
		}
		return null;
	...
	public static void treeDataBind(TreeAction ta) {
		...
		if (ta.isLazyLoad()) {
			...
			QueryBuilder qb = new QueryBuilder(
					"select ID,ParentID,TreeLevel,Name,SingleFlag,InnerCode,OrderFlag from ZCCatalog"
							+ " Where Type = ? and SiteID = ? and TreeLevel-1 <=? and innercode like '002313%' order by orderflag,innercode");
			qb.add(catalogType);
			qb.add(siteID);
			qb.add(ta.getLevel());
			dt = qb.executeDataTable();
	...
	public static void treeDataBind1(TreeAction ta) {
		...
		if (ta.isLazyLoad()) {
			...
			QueryBuilder qb = new QueryBuilder(
					"select ID,ParentID,TreeLevel,Name,SingleFlag,InnerCode,OrderFlag from ZCCatalog"
							+ " Where Type = ? and SiteID = ? and TreeLevel-1 <=? and innercode like '002398%' order by orderflag,innercode");
			qb.add(catalogType);
			qb.add(siteID);
			qb.add(ta.getLevel());
			dt = qb.executeDataTable();
	...
	public static void treeDiagDataBind(TreeAction ta) {
		...
		if (catalogType == CatalogCJWT.SHOP_GOODS_BRAND) {
			siteName = "商品品牌库";
		}
		String inputType = (String) ta.getParams().get("Type");
		if ("3".equals(inputType)) {// 单选
	...
	public static void treeResourceDataBind(TreeAction ta) {
		...
		int catalogType = (catalogTypeStr != null) ? Integer.parseInt(catalogTypeStr.toString()) : CatalogCJWT.IMAGELIB;
		if (CatalogCJWT.IMAGELIB == catalogType) {
			ImageLib.treeDataBind(ta);
		} else if (CatalogCJWT.VIDEOLIB == catalogType) {
			VideoLib.treeDataBind(ta);
		} else if (CatalogCJWT.AUDIOLIB == catalogType) {
			AudioLib.treeDataBind(ta);
		} else if (CatalogCJWT.ATTACHMENTLIB == catalogType) {
	...
	public void importCatalog() {
		...
		for (int i = 0; i < dt.getRowCount(); i++) {
			...
			if (StringUtil.isNotEmpty(type) && !"null".equals(type)) {
				...
				catalog.setType(CatalogCJWT.CATALOG);
			}
	...
	public void move() {
		...
		if (parentID == 0) {
			...
			if (type == CatalogCJWT.CATALOG) {
				site.setChannelCount(site.getChannelCount() + 1);
			} else if (type == CatalogCJWT.SUBJECT) {
				site.setSpecialCount(site.getSpecialCount() + 1);
			} else if (type == CatalogCJWT.MAGAZINE) {
				site.setMagzineCount(site.getMagzineCount() + 1);
			}
	...
	public void del() {
		...
		CatalogCJWT.deleteCatalog(trans, Long.parseLong(ID));
		if (trans.commit()) {
	...
	public void batchDel() {
		...
		for (int i = 0; i < catalogs.length; i++) {
			CatalogCJWT.deleteCatalog(trans, Long.parseLong(String.valueOf(catalogs[i])));
		}
		if (trans.commit()) {
			for (int i = 0; i < catalogs.length; i++) {
	...
	public static void deleteCatalog(Transaction trans, long ID) {
		...
		if (catalog.getType() == CatalogCJWT.MAGAZINE) {
