...
public class ThreadPool extends ThreadGroup {
    ...
    private LinkedList<Object> workQueue;      //工作队列  
    ...
    public ThreadPool(int poolSize) {  //poolSize 表示线程池中的工作线程的数量  
        ...
        workQueue = new LinkedList<Object>();  //创建工作队列  
    ...
    private synchronized Runnable getTask(int threadid) throws InterruptedException {  
        while(workQueue.size() == 0) {  
            ...
            logger.info("工作线程{}等待任务...", threadid);
            wait();             //如果工作队列中没有任务,就等待任务  
        }  
        logger.info("工作线程{}开始执行任务...", threadid);
        return (Runnable) workQueue.removeLast(); //反回队列中第一个元素,并从队列中删除  
    }  
