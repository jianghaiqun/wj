...
/**
 * 表名称：广告表备份<br>
 * 表代码：BZCAdvertisement<br>
 * 表主键：ID, BackupNo<br>
 */
public class BZCAdvertisementSchema extends Schema {
	private Long ID;
	...
	public static final String _TableCode = "BZCAdvertisement";

	public static final String _NameSpace = "com.sinosoft.schema";

	protected static final String _InsertAllSQL = "insert into BZCAdvertisement values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";

	protected static final String _UpdateAllSQL = "update BZCAdvertisement set ID=?,PositionID=?,PositionCode=?,SiteID=?,AdName=?,AdType=?,AdContent=?,OrderFlag=?,IsHitCount=?,HitCount=?,StickTime=?,IsOpen=?,StartTime=?,EndTime=?,Prop1=?,Prop2=?,Prop3=?,Prop4=?,AddUser=?,AddTime=?,ModifyUser=?,ModifyTime=?,BackupNo=?,BackupOperator=?,BackupTime=?,BackupMemo=? where ID=? and BackupNo=?";

	protected static final String _DeleteSQL = "delete from BZCAdvertisement  where ID=? and BackupNo=?";

	protected static final String _FillAllSQL = "select * from BZCAdvertisement  where ID=? and BackupNo=?";

	public BZCAdvertisementSchema(){
	...
	protected Schema newInstance(){
		return new BZCAdvertisementSchema();
	}
	...
	public BZCAdvertisementSet query() {
		return query(null, -1, -1);
	}

	public BZCAdvertisementSet query(QueryBuilder qb) {
		return query(qb, -1, -1);
	}

	public BZCAdvertisementSet query(int pageSize, int pageIndex) {
		return query(null, pageSize, pageIndex);
	}

	public BZCAdvertisementSet query(QueryBuilder qb , int pageSize, int pageIndex){
		return (BZCAdvertisementSet)querySet(qb , pageSize , pageIndex);
	}
