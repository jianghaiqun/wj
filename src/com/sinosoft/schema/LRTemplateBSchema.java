 ...
 * 表主键：Id, Version<br>
 */
public class LRTemplateBSchema extends Schema {
	private String Id;
	...
	public static final SchemaColumn[] _Columns = new SchemaColumn[] {
		...
		new SchemaColumn("Version", DataColumn.INTEGER, 1, 0 , 0 , true , true),
	...
	public static final String _TableCode = "LRTemplateB";

	public static final String _NameSpace = "com.sinosoft.schema";

	protected static final String _InsertAllSQL = "insert into LRTemplateB values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";

	protected static final String _UpdateAllSQL = "update LRTemplateB set Id=?,Version=?,RuleName=?,RuleCh=?,SQLStatement=?,BOMs=?,SQLParameter=?,ViewParameter=?,Valid=?,State=?,TemplateLevel=?,Business=?,Type=?,TableName=?,StartDate=?,EndDate=?,Description=?,Creator=?,Modifier=?,Approver=?,Deployer=?,MakeTime=?,MakeDate=?,ModifyTime=?,ModifyDate=?,AuthorTime=?,AuthorDate=?,DeclTime=?,DeclDate=?,MarketingNum=?,DRLPath=? where Id=? and Version=?";

	protected static final String _DeleteSQL = "delete from LRTemplateB  where Id=? and Version=?";

	protected static final String _FillAllSQL = "select * from LRTemplateB  where Id=? and Version=?";
	...
	public LRTemplateBSet query() {
		return query(null, -1, -1);
	}

	public LRTemplateBSet query(QueryBuilder qb) {
		return query(qb, -1, -1);
	}

	public LRTemplateBSet query(int pageSize, int pageIndex) {
		return query(null, pageSize, pageIndex);
	}

	public LRTemplateBSet query(QueryBuilder qb , int pageSize, int pageIndex){
		return (LRTemplateBSet)querySet(qb , pageSize , pageIndex);
	}
