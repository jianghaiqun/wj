 ...
 * 表主键：ID, BackupNo<br>
 */
public class BZCLinkSchema extends Schema {
	private Long ID;
	...
	public static final SchemaColumn[] _Columns = new SchemaColumn[] {
		...
		new SchemaColumn("LinkGroupID", DataColumn.LONG, 2, 0 , 0 , true , false),
		new SchemaColumn("Name", DataColumn.STRING, 3, 100 , 0 , true , false),
		new SchemaColumn("URL", DataColumn.STRING, 4, 100 , 0 , true , false),
		new SchemaColumn("ImagePath", DataColumn.STRING, 5, 100 , 0 , false , false),
		new SchemaColumn("OrderFlag", DataColumn.LONG, 6, 0 , 0 , false , false),
	...
	public static final String _TableCode = "BZCLink";

	public static final String _NameSpace = "com.sinosoft.schema";

	protected static final String _InsertAllSQL = "insert into BZCLink values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";

	protected static final String _UpdateAllSQL = "update BZCLink set ID=?,SiteID=?,LinkGroupID=?,Name=?,URL=?,ImagePath=?,OrderFlag=?,Prop1=?,Prop2=?,Prop3=?,Prop4=?,AddUser=?,AddTime=?,ModifyUser=?,ModifyTime=?,BackupNo=?,BackupOperator=?,BackupTime=?,BackupMemo=? where ID=? and BackupNo=?";

	protected static final String _DeleteSQL = "delete from BZCLink  where ID=? and BackupNo=?";

	protected static final String _FillAllSQL = "select * from BZCLink  where ID=? and BackupNo=?";

	public BZCLinkSchema(){
	...
	public BZCLinkSet query() {
		return query(null, -1, -1);
	}

	public BZCLinkSet query(QueryBuilder qb) {
		return query(qb, -1, -1);
	}

	public BZCLinkSet query(int pageSize, int pageIndex) {
		return query(null, pageSize, pageIndex);
	}

	public BZCLinkSet query(QueryBuilder qb , int pageSize, int pageIndex){
		return (BZCLinkSet)querySet(qb , pageSize , pageIndex);
	}

	public void setV(int i, Object v) {
		if (i == 0){if(v==null){ID = null;}else{ID = new Long(v.toString());}return;}
		if (i == 1){if(v==null){SiteID = null;}else{SiteID = new Long(v.toString());}return;}
		if (i == 2){if(v==null){LinkGroupID = null;}else{LinkGroupID = new Long(v.toString());}return;}
	...
	*/
	public long getLinkGroupID() {
		if(LinkGroupID==null){return 0;}
		return LinkGroupID.longValue();
	}

	/**
	* 设置字段LinkGroupID的值，该字段的<br>
	* 字段名称 :链接分类ID<br>
	* 数据类型 :bigint<br>
	* 是否主键 :false<br>
	* 是否必填 :true<br>
