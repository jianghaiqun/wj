 ...
 * 表名称：分项统计表<br>
 * 表代码：ZCStatItem<br>
 * 表主键：SiteID, Period, Type, SubType, Item<br>
 */
public class ZCStatItemSchema extends Schema {
	private Long SiteID;
	...
	public static final String _TableCode = "ZCStatItem";

	public static final String _NameSpace = "com.sinosoft.schema";

	protected static final String _InsertAllSQL = "insert into ZCStatItem values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";

	protected static final String _UpdateAllSQL = "update ZCStatItem set SiteID=?,Period=?,Type=?,SubType=?,Item=?,Count1=?,Count2=?,Count3=?,Count4=?,Count5=?,Count6=?,Count7=?,Count8=?,Count9=?,Count10=?,Count11=?,Count12=?,Count13=?,Count14=?,Count15=?,Count16=?,Count17=?,Count18=?,Count19=?,Count20=?,Count21=?,Count22=?,Count23=?,Count24=?,Count25=?,Count26=?,Count27=?,Count28=?,Count29=?,Count30=?,Count31=? where SiteID=? and Period=? and Type=? and SubType=? and Item=?";

	protected static final String _DeleteSQL = "delete from ZCStatItem  where SiteID=? and Period=? and Type=? and SubType=? and Item=?";

	protected static final String _FillAllSQL = "select * from ZCStatItem  where SiteID=? and Period=? and Type=? and SubType=? and Item=?";

	public ZCStatItemSchema(){
	...
	public ZCStatItemSet query() {
		return query(null, -1, -1);
	}

	public ZCStatItemSet query(QueryBuilder qb) {
		return query(qb, -1, -1);
	}

	public ZCStatItemSet query(int pageSize, int pageIndex) {
		return query(null, pageSize, pageIndex);
	}

	public ZCStatItemSet query(QueryBuilder qb , int pageSize, int pageIndex){
		return (ZCStatItemSet)querySet(qb , pageSize , pageIndex);
	}
	...
	* 备注信息 :<br>
	描述时段的字符串，全部是数字。<br>
	如果时段类别是全部，则是0<br>
	如果时段类别是周，则是7位数字如2008901<br>
	如果时段类别是月，则是6位数字如200811<br>
	如果时段类别是日，则是8位数字如20081120<br>
	如果时段类别是时，则是10位数字如2008112015<br>
	*/
