...
public class BZCCommentSchema extends Schema {
	...
	protected static final String _UpdateAllSQL = "update BZCComment set ID=?,RelaID=?,CatalogID=?,CatalogType=?,SiteID=?,Title=?,Content=?,AddUser=?,AddUserIP=?,AddTime=?,VerifyFlag=?,VerifyUser=?,VerifyTime=?,Prop1=?,Prop2=?,SupporterCount=?,AntiCount=?,SupportAntiIP=?,ReplyContent=?,Score=?,IsBuy=?,DescribeScore=?,PolicyScore=?,ClientScore=?,CoverageScore=?,sortNum=?,praisedNum=?,Purpose=?,memGrade=?,BackupNo=?,BackupOperator=?,BackupTime=?,BackupMemo=? where ID=? and BackupNo=?";
