...
/**
 * 表名称：分支机构保单信息备份<br>
 * 表代码：BLineTeamPolicyInfo<br>
 * 表主键：id, BackupNo<br>
 */
public class BLineTeamPolicyInfoSchema extends Schema {
	private String id;
	...
	public static final String _TableCode = "BLineTeamPolicyInfo";

	public static final String _NameSpace = "com.sinosoft.schema";

	protected static final String _InsertAllSQL = "insert into BLineTeamPolicyInfo values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";

	protected static final String _UpdateAllSQL = "update BLineTeamPolicyInfo set id=?,branchInnercode=?,companyCode=?,riskType=?,prem=?,policyNo=?,agentRate=?,startDate=?,insureDate=?,customName=?,customMobileNo=?,customIdentityType=?,customIdentityId=?,agentName=?,agentMobileNo=?,agentPrice=?,cancelTime=?,plateNumber=?,status=?,remark1=?,remark2=?,remark3=?,userBranchInnercode=?,createDate=?,createUser=?,modifyDate=?,modifyUser=?,BackupNo=?,BackupOperator=?,BackupTime=?,BackupMemo=? where id=? and BackupNo=?";

	protected static final String _DeleteSQL = "delete from BLineTeamPolicyInfo  where id=? and BackupNo=?";

	protected static final String _FillAllSQL = "select * from BLineTeamPolicyInfo  where id=? and BackupNo=?";

	public BLineTeamPolicyInfoSchema(){
	...
	protected Schema newInstance(){
		return new BLineTeamPolicyInfoSchema();
	}
	...
	public BLineTeamPolicyInfoSet query() {
		return query(null, -1, -1);
	}

	public BLineTeamPolicyInfoSet query(QueryBuilder qb) {
		return query(qb, -1, -1);
	}

	public BLineTeamPolicyInfoSet query(int pageSize, int pageIndex) {
		return query(null, pageSize, pageIndex);
	}

	public BLineTeamPolicyInfoSet query(QueryBuilder qb , int pageSize, int pageIndex){
		return (BLineTeamPolicyInfoSet)querySet(qb , pageSize , pageIndex);
	}
