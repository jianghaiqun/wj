...
public class BZCSiteSchema extends Schema {
	...
	public static final SchemaColumn[] _Columns = new SchemaColumn[] {
		...
		new SchemaColumn("AddUser", DataColumn.STRING, 48, 200 , 0 , true , false),
		new SchemaColumn("AddTime", DataColumn.DATETIME, 49, 0 , 0 , true , false),
		new SchemaColumn("ModifyUser", DataColumn.STRING, 50, 200 , 0 , false , false),
		new SchemaColumn("ModifyTime", DataColumn.DATETIME, 51, 0 , 0 , false , false),
		new SchemaColumn("BackupNo", DataColumn.STRING, 52, 15 , 0 , true , true),
		new SchemaColumn("BackupOperator", DataColumn.STRING, 53, 200 , 0 , true , false),
		new SchemaColumn("BackupTime", DataColumn.DATETIME, 54, 0 , 0 , true , false),
		new SchemaColumn("BackupMemo", DataColumn.STRING, 55, 50 , 0 , false , false)
	};
	...
	protected static final String _UpdateAllSQL = "update BZCSite set ID=?,Name=?,Alias=?,WapAlias=?,Info=?,BranchInnerCode=?,URL=?,WapURL=?,WapResourceURL=?,RootPath=?,IndexTemplate=?,ListTemplate=?,DetailTemplate=?,EditorCss=?,Workflow=?,OrderFlag=?,LogoFileName=?,MessageBoardFlag=?,CommentAuditFlag=?,ChannelCount=?,MagzineCount=?,SpecialCount=?,ImageLibCount=?,VideoLibCount=?,AudioLibCount=?,AttachmentLibCount=?,ArticleCount=?,HitCount=?,ConfigXML=?,AutoIndexFlag=?,AutoStatFlag=?,HeaderTemplate=?,TopTemplate=?,BottomTemplate=?,AllowContribute=?,BBSEnableFlag=?,ShopEnableFlag=?,Meta_Keywords=?,Meta_Description=?,Prop1=?,Prop2=?,Prop3=?,Prop4=?,Prop5=?,Prop6=?,StaticResourcePath=?,ProductResourcePath=?,JsResourcePath=?,AddUser=?,AddTime=?,ModifyUser=?,ModifyTime=?,BackupNo=?,BackupOperator=?,BackupTime=?,BackupMemo=? where ID=? and BackupNo=?";
	...
	public void setV(int i, Object v) {
		...
		if (i == 48){AddUser = (String)v;return;}
		if (i == 49){AddTime = (Date)v;return;}
		if (i == 50){ModifyUser = (String)v;return;}
		if (i == 51){ModifyTime = (Date)v;return;}
		if (i == 52){BackupNo = (String)v;return;}
		if (i == 53){BackupOperator = (String)v;return;}
		if (i == 54){BackupTime = (Date)v;return;}
		if (i == 55){BackupMemo = (String)v;return;}
	}
	...
	public Object getV(int i) {
		...
		if (i == 48){return AddUser;}
		if (i == 49){return AddTime;}
		if (i == 50){return ModifyUser;}
		if (i == 51){return ModifyTime;}
		if (i == 52){return BackupNo;}
		if (i == 53){return BackupOperator;}
		if (i == 54){return BackupTime;}
		if (i == 55){return BackupMemo;}
		return null;
	}
