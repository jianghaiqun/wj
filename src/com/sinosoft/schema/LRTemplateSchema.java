 ...
 * 表主键：Id<br>
 */
public class LRTemplateSchema extends Schema {
	private String Id;
	...
	public static final String _TableCode = "LRTemplate";

	public static final String _NameSpace = "com.sinosoft.schema";

	protected static final String _InsertAllSQL = "insert into LRTemplate values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";

	protected static final String _UpdateAllSQL = "update LRTemplate set Id=?,Version=?,RuleName=?,RuleCh=?,SQLStatement=?,BOMs=?,SQLParameter=?,ViewParameter=?,Valid=?,State=?,TemplateLevel=?,Business=?,Type=?,TableName=?,StartDate=?,EndDate=?,Description=?,Creator=?,Modifier=?,Approver=?,Deployer=?,MakeTime=?,MakeDate=?,ModifyTime=?,ModifyDate=?,AuthorTime=?,AuthorDate=?,DeclTime=?,DeclDate=?,MarketingNum=?,DRLPath=? where Id=?";

	protected static final String _DeleteSQL = "delete from LRTemplate  where Id=?";

	protected static final String _FillAllSQL = "select * from LRTemplate  where Id=?";

	public LRTemplateSchema(){
	...
	public LRTemplateSet query() {
		return query(null, -1, -1);
	}

	public LRTemplateSet query(QueryBuilder qb) {
		return query(qb, -1, -1);
	}

	public LRTemplateSet query(int pageSize, int pageIndex) {
		return query(null, pageSize, pageIndex);
	}

	public LRTemplateSet query(QueryBuilder qb , int pageSize, int pageIndex){
		return (LRTemplateSet)querySet(qb , pageSize , pageIndex);
	}
