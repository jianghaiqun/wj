 ...
 * 表主键：ID, BackupNo<br>
 */
public class BZCVideoSchema extends Schema {
	private Long ID;
	...
	public static final String _TableCode = "BZCVideo";

	public static final String _NameSpace = "com.sinosoft.schema";

	protected static final String _InsertAllSQL = "insert into BZCVideo values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";

	protected static final String _UpdateAllSQL = "update BZCVideo set ID=?,Name=?,OldName=?,SiteID=?,Tag=?,CatalogID=?,CatalogInnerCode=?,BranchInnerCode=?,Path=?,FileName=?,SrcFileName=?,Suffix=?,IsOriginal=?,Info=?,System=?,FileSize=?,PublishDate=?,ImageName=?,Count=?,Width=?,Height=?,Duration=?,ProduceTime=?,Author=?,Integral=?,OrderFlag=?,HitCount=?,StickTime=?,SourceURL=?,Status=?,Prop1=?,Prop2=?,Prop3=?,Prop4=?,AddUser=?,AddTime=?,ModifyUser=?,ModifyTime=?,BackupNo=?,BackupOperator=?,BackupTime=?,BackupMemo=? where ID=? and BackupNo=?";

	protected static final String _DeleteSQL = "delete from BZCVideo  where ID=? and BackupNo=?";

	protected static final String _FillAllSQL = "select * from BZCVideo  where ID=? and BackupNo=?";

	public BZCVideoSchema(){
	...
	public BZCVideoSet query() {
		return query(null, -1, -1);
	}

	public BZCVideoSet query(QueryBuilder qb) {
		return query(qb, -1, -1);
	}

	public BZCVideoSet query(int pageSize, int pageIndex) {
		return query(null, pageSize, pageIndex);
	}

	public BZCVideoSet query(QueryBuilder qb , int pageSize, int pageIndex){
		return (BZCVideoSet)querySet(qb , pageSize , pageIndex);
	}
