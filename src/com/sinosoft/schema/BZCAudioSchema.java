 ...
 * 表代码：BZCAudio<br>
 * 表备注：<br>
一个音频可以没有所属ID（OwnerID），即从音频库直接上传的音频，此时OwnerID=-1，Type=-1； 另一种情况是在文章中中上传是音频，此时OwnerID为文章的ID； 一张图片可以有多个OwnerID，即一个ID对应多个OwnerID，例如，这个音频同时是两篇文章中的音频； 一张音频可以属于多个分类，即多个CatalogID；一个音频可以没有所属ID（OwnerID），即从音频库直接上传的音频，此时OwnerID=-1，Type=-1； 另一种情况是在文章中中上传是音频，此时OwnerID为文章的ID； 一张图片可以有多个OwnerID，即一个ID对应多个OwnerID，例如，这个音频同时是两篇文章中的音频； 一张音频可以属于多个分类，即多个CatalogID；<br>
<br>
 * 表主键：ID, BackupNo<br>
 */
public class BZCAudioSchema extends Schema {
	...
	public static final SchemaColumn[] _Columns = new SchemaColumn[] {
		...
		new SchemaColumn("BackupNo", DataColumn.STRING, 32, 15 , 0 , true , true),
		new SchemaColumn("BackupOperator", DataColumn.STRING, 33, 200 , 0 , true , false),
		new SchemaColumn("BackupTime", DataColumn.DATETIME, 34, 0 , 0 , true , false),
		new SchemaColumn("BackupMemo", DataColumn.STRING, 35, 50 , 0 , false , false)
	};
	...
	protected static final String _UpdateAllSQL = "update BZCAudio set ID=?,Name=?,OldName=?,SiteID=?,Tag=?,CatalogID=?,CatalogInnerCode=?,BranchInnerCode=?,Path=?,FileName=?,SrcFileName=?,Suffix=?,IsOriginal=?,Info=?,System=?,FileSize=?,PublishDate=?,Duration=?,ProduceTime=?,Author=?,Integral=?,SourceURL=?,Status=?,OrderFlag=?,Prop1=?,Prop2=?,Prop3=?,Prop4=?,AddUser=?,AddTime=?,ModifyUser=?,ModifyTime=?,BackupNo=?,BackupOperator=?,BackupTime=?,BackupMemo=? where ID=? and BackupNo=?";
	...
	public void setV(int i, Object v) {
		...
		if (i == 32){BackupNo = (String)v;return;}
		if (i == 33){BackupOperator = (String)v;return;}
		if (i == 34){BackupTime = (Date)v;return;}
		if (i == 35){BackupMemo = (String)v;return;}
	}
	...
	public Object getV(int i) {
		...
		if (i == 32){return BackupNo;}
		if (i == 33){return BackupOperator;}
		if (i == 34){return BackupTime;}
		if (i == 35){return BackupMemo;}
		return null;
	}
