...
/**
 * 表名称：期刊期数表<br>
 * 表代码：ZCMagazineIssue<br>
 * 表主键：ID<br>
 */
public class ZCMagazineIssueSchema extends Schema {
	private Long ID;
	...
	public static final SchemaColumn[] _Columns = new SchemaColumn[] {
		...
		new SchemaColumn("MagazineID", DataColumn.LONG, 1, 0 , 0 , true , false),
		...
		new SchemaColumn("PublishDate", DataColumn.DATETIME, 6, 0 , 0 , false , false),
	...
	public static final String _TableCode = "ZCMagazineIssue";

	public static final String _NameSpace = "com.sinosoft.schema";

	protected static final String _InsertAllSQL = "insert into ZCMagazineIssue values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";

	protected static final String _UpdateAllSQL = "update ZCMagazineIssue set ID=?,MagazineID=?,Year=?,PeriodNum=?,CoverImage=?,CoverTemplate=?,PublishDate=?,Status=?,Memo=?,Prop1=?,Prop2=?,Prop3=?,Prop4=?,AddUser=?,AddTime=?,ModifyUser=?,ModifyTime=? where ID=?";

	protected static final String _DeleteSQL = "delete from ZCMagazineIssue  where ID=?";

	protected static final String _FillAllSQL = "select * from ZCMagazineIssue  where ID=?";

	public ZCMagazineIssueSchema(){
	...
	protected Schema newInstance(){
		return new ZCMagazineIssueSchema();
	}
	...
	public ZCMagazineIssueSet query() {
		return query(null, -1, -1);
	}

	public ZCMagazineIssueSet query(QueryBuilder qb) {
		return query(qb, -1, -1);
	}

	public ZCMagazineIssueSet query(int pageSize, int pageIndex) {
		return query(null, pageSize, pageIndex);
	}

	public ZCMagazineIssueSet query(QueryBuilder qb , int pageSize, int pageIndex){
		return (ZCMagazineIssueSet)querySet(qb , pageSize , pageIndex);
	}

	public void setV(int i, Object v) {
		if (i == 0){if(v==null){ID = null;}else{ID = new Long(v.toString());}return;}
		if (i == 1){if(v==null){MagazineID = null;}else{MagazineID = new Long(v.toString());}return;}
