...
public class CRUDGenerator {
	...
	private void generateOneTable(Element table) {
		...
		for (int i = 0; i < scs.length; i++) {
			...
			if (dataType == null || dataType.equals("")) {
				logger.error("Error:表{}的字段{}的数据类型未定义!", tableCode, code);
				return;
			}
	...
	public static void main(String[] args) {
		...
		String pdmprefix = prefix +"DB/"+"ZvingCMS";//Config.getValue("App.Code");
		String javapath = prefix + "Java/com/zving/schema";

		/*
		BackupTableGenerator btg = new BackupTableGenerator();
		btg.setFileName(pdmprefix + ".pdm");
		try {
			btg.toBackupTable();
		} catch (Exception e) {
			logger.error(e.getMessage(), e);
		...
		*/
		...
		og.setNamespace("com.sinosoft.schema");
		try {
			og.generate();
		} catch (Exception e) {
			logger.error(e.getMessage(), e);
		...
		/*
		og.setFileName(pdmprefix + "_B.pdm");
		og.setOutputDir(javapath);
		og.setNamespace("com.sinosoft.schema");
		try {
			og.generate();
