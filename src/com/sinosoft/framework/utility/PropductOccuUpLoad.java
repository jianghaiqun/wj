...
public class PropductOccuUpLoad extends Page {
	...
	public boolean resolvingExcel(String upLoadFileName) {
		try {
			...
			//产品编码
			String comCode = String.valueOf(getCellString(row1.getCell(1))).trim();
			...
			trans.add(new QueryBuilder(" DELETE  FROM `occupation` WHERE insurancecompany=? ",comCode));//先删除
			for (int i = 10; i <= sheet.getLastRowNum(); i++) {
				...
				if (row != null) {
					...
					if (row.getCell(1) != null
						...
						QueryBuilder qb  = new QueryBuilder(" INSERT INTO occupation VALUES(?,?,?,?,?,?,?,?,?,?) ");
						String code = String.valueOf(getCellString(row.getCell(1)));
						int tCount = new QueryBuilder(" SELECT COUNT(1) FROM occupation WHERE id=? ",code).executeInt();
						if(tCount>=2){
							addError("职业编码（“"+code+"）重复，！");
							return false;
						}
						//String occu_id = comCode+code;
						String occu_id = code;
						qb.add(occu_id.trim());
						qb.add(new Date());
						qb.add(new Date());
						
						String ComCodeCalue = String.valueOf(getCellString(row.getCell(5)));
						qb.add(ComCodeCalue.trim());
						
						String occu_name = String.valueOf(getCellString(row.getCell(2)));
						qb.add(occu_name.trim());
						qb.add("");
						//String occu_pid = comCode+String.valueOf(getCellString(row.getCell(3)));
						String occu_pid = String.valueOf(getCellString(row.getCell(3)));
						if(StringUtil.isNotEmpty(occu_pid)){
							qb.add(occu_pid.trim());
						}else{
							qb.add("");
						}
						qb.add(comCode.trim());
						String occu_grade = String.valueOf(getCellString(row.getCell(4)));
						if(StringUtil.isNotEmpty(occu_grade)){
							qb.add(occu_grade.trim());
						}else{
							qb.add("");
						}
						qb.add("");
