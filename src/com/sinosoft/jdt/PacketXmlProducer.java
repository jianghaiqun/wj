 ...
 * 
 */
public class PacketXmlProducer
{
	private static final Logger logger = LoggerFactory.getLogger(PacketXmlProducer.class);
	/**
	 ...
	 * @return
	 */
	public String generatePacket(String strFlag, String strManageCom, String strOrderSn, String[] encoding)
	{
		if (StringUtil.isEmpty(strFlag) || StringUtil.isEmpty(strManageCom) || StringUtil.isEmpty(strOrderSn))
		{
			...
			logger.warn("PacketXmlProducer: 核保承保退保标记、保险公司编码、订单号均不能为空！Flag={}, ManageCom={}, Order={}", argArr);
			return null;
		}
	...
	private boolean getChildrenAndDataForPXElement(PacketXmlElement ele)
	{
		if (null == ele)
		{
			logger.warn("PacketXmlProducer: 获取节点数据时，传入节点为空！Flag={}, ManageCom={}",
		...
		{
			Object[] argArr = {mapVarValue.get("Flag"), mapVarValue.get("ManageCom"), ele.getName(), ele.getElementNo(), ele.getElementNo()};
			logger.warn("PacketXmlProducer: 获取节点数据时，传入节点的名称或编号为空！Flag={}, ManageCom={}, name={}, No={}, OrderSn={}", argArr);
			return false;
		}
	...
	private boolean setAttributeForPXElement(PacketXmlElement ele)
	{
		...
		if ((leftNum < strAttributeArr.length) && (getFromParent < leftNum))
		{
			...
			logger.error("PacketXmlProducer: 设置节点属性时，未获取全！Flag=" +
	...
	private boolean setAttributeValueFromCfgValue(PacketXmlElement ele, String[] strAttributeArr)
	{
		...
		if (strAttributeArr.length == strAttributeValueArr.length)
		{
			if (strArributeValue.indexOf(this.strDefaultKeyValueSplit)>-1)
			{
				...
				logger.error("PacketXmlProducer: 节点属性值配置错误，节点的全部属性都有指定值时，" +
						"不需要配置属性名！name={}, No={}, Flag={}, ManageCom={}, AttributeValue={}", argArr);
				return false;
			}
	...
	private boolean getPrepareDataFromCfgAttributeSql(PacketXmlElement ele)
	{
		...
		if (null == parent)
		{
			Object[] argArr = {mapVarValue.get("Flag"), mapVarValue.get("ManageCom"),
					ele.getName(), ele.getElementNo(), mapVarValue.get("OrderNo")};
			logger.warn("PacketXmlProducer: 循环节点的父节点不能为空！" +
					"Flag={}, ManageCom={}, name={}, No={}, OrderSn={}", argArr);
			return false;
		}
	...
	private boolean getDataFromCfgSql(PacketXmlElement ele)
	{
		...
		if ((null == dt) || (0 == dt.getRowCount()))
		{
			...
			ele.setIsLeaf(true);
			Object[] argArr = { ele.getName(), ele.getElementNo(), mapVarValue.get("Flag"), mapVarValue.get("ManageCom")};
			logger.info("PacketXmlProducer: 叶子节点不需要配置ElementSql，顶多只需要配置AttributeSql！" +
					"name={}, No={}, Flag={}, ManageCom={}", argArr);
			return true;
		}
		...
		if (StringUtil.isNotEmpty(sqlVar))
		{
			...
			if(Config.getValue("groupTypes").indexOf(mapVarValue.get("ManageCom")) != -1){
				...
				if(dt != null && dt.getRowCount() > 0){
					...
					if(("Header".equals(element) || "insurantInfo".equals(element))){
	...
	private boolean getChildrenDataFromDT(DataTable dt, PacketXmlElement ele)
	{
		...
		if ((null == childDT) || (0 == childDT.getRowCount()))
		{
			...
			Object[] argArr = {mapVarValue.get("Flag"), mapVarValue.get("ManageCom"),
					ele.getName(), ele.getElementNo(), mapVarValue.get("OrderNo")};
			logger.error("PacketXmlProducer: 查找子节点失败！Flag={}, ManageCom={}, name={}, No={}, OrderSn={}", argArr);
			return false;
		}
	...
	private boolean getSelfDataFromDT(DataTable dt, PacketXmlElement ele)
	{
		...
		if (dt.getRowCount() > 1)
		{
			if (!"Y".equalsIgnoreCase(ele.getCfgForFlag()))
			{
				Object[] argArr = {mapVarValue.get("Flag"), mapVarValue.get("ManageCom"),
						ele.getName(), ele.getElementNo(), mapVarValue.get("OrderNo")};
				logger.info("PacketProducer: 非循环节点数据多余一条！" +
						"Flag={}, ManageCom={}, name={}, No={}, OrderSn={}", argArr);
				return false;
			}
	...
	public static void main(String[] args)
	{
		...
		String strFlag = "01";
		String strManageCom = "1015";
		String strOrderSn = "3013000011111152";
		/*
		...
		*/
//		String xml = null;
//		xml = new PacketXmlProducer().generatePacket(strFlag, strManageCom, strOrderSn, encoding);
