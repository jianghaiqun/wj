...
import com.sinosoft.framework.FDate;
 ...
 */
public class ExeSQL {
    private static final Logger logger = LoggerFactory.getLogger(ExeSQL.class);
    ...
    public String getOneValue(String sql) {
        ...
        try {
            pstmt = con.prepareStatement(sql.trim(), ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY);
            ...
            // @@错误处理
            logger.error("### Error ExeSQL at getOneValue: {}", sql);
            mErrors.clearErrors();
        ...
        if( t1-t0 > 500 && t1-t0 <= 1000 ){
            logger.warn("警告：执行时间超过500毫秒,执行时间为{}毫秒  ------  {}",(t1-t0),sql);
        }else if(t1-t0 > 1000 && t1-t0 <=5000){
            logger.warn("严重警告：执行时间超过1000毫秒,执行时间为{}毫秒  ------  {}",(t1-t0),sql);
        }else if(t1-t0 > 5000 && t1-t0 <=10000){
            logger.warn("严重警告：执行时间超过5000毫秒,执行时间为{}毫秒  ------  {}",(t1-t0),sql);
        }else if(t1-t0 > 10000 && t1-t0 <=50000){
            logger.warn("严重警告：执行时间超过10000毫秒,执行时间为{}毫秒  ------  {}",(t1-t0),sql);
        }else if(t1-t0 > 50000 && t1-t0 <=100000){
            logger.warn("严重警告：执行时间超过50000毫秒,执行时间为{}毫秒  ------  {}",(t1-t0),sql);
        }else if(t1-t0 > 100000){
            logger.warn("严重警告：执行时间超过100000毫秒,执行时间为{}毫秒  ------  {}",(t1-t0),sql);
        }
        return mValue.trim();
    }
    ...
    private int getResultCount(String sql, PreparedStatement pstmt, ResultSet rs) {
        ...
        logger.info("getResultCount : {}", sql);
        ...
        try {
            ...
            // @@错误处理
            logger.error("### Error ExeSQL at getResultCount: " + sql + e.getMessage(), e);
            CError.buildErr(this, e.toString());
            iCount = 0;
            try {
                if (rs != null) {
    ...
    public String getEncodedResult(String sql, int start) {
        ...
        try {
            ...
            // @@错误处理
            logger.error("##### Error Sql in ExeSQL at getEncodedResult(String sql, int start): " + sql + e.getMessage(), e);
            mErrors.clearErrors();
            CError.buildErr(this, e.toString());
            mResult.setLength(0);
            try {
        ...
        Object[] argArr = {rowCount, getCount, sql};
        if(rowCount>200 && rowCount<=500){
            logger.info("SQL结果条数超过200，结果条数为{}条,获取条数为{}条！--- {}", argArr);
        }else if(rowCount>500 && rowCount<=1000){
            logger.info("SQL结果条数超过500，结果条数为{}条,获取条数为{}条！--- {}", argArr);
        }else if(rowCount>1000 && rowCount<=5000){
            logger.info("SQL结果条数超过1000，结果条数为{}条,获取条数为{}条！--- {}", argArr);
        }else if(rowCount>5000){
            logger.info("SQL结果条数超过5000，结果条数为{}条,获取条数为{}条！--- {}", argArr);
        }
    ...
    public String getEncodedResultLarge(String sql, int start) {
        ...
        try {
            ...
            logger.info("getEncodedResultLarge : {}", tSBql.toString());

            pstmt = con.prepareStatement((tSBql.toString()).trim() , ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY);
            ...
            // @@错误处理
            logger.error("##### Error Sql in ExeSQL at getEncodedResultLarge(String sql, int start):  " + sql + e.getMessage(), e);
            mErrors.clearErrors();
            CError.buildErr(this, e.toString());
            mResult.setLength(0);
            try {
    ...
    public String getEncodedResult(String sql) {
        ...
        try {
            ...
            // @@错误处理
            logger.error("##### Error Sql in ExeSQL at getEncodedResult(String sql): " + sql + e.getMessage(), e);
            mErrors.clearErrors();
            CError.buildErr(this, e.toString());
            mResult.setLength(0);
            try {
    ...
    public String getEncodedResult(String sql, int start, int nCount) {
        ...
        try {
            ...
            // @@错误处理
            logger.error("##### Error Sql in ExeSQL at getEncodedResult(String sql, int start, int nCount): " + sql + e.getMessage(), e);
            mErrors.clearErrors();
            CError.buildErr(this, e.toString());
            mResult.setLength(0);
            try {
    ...
    public boolean execUpdateSQL(String sql) {
		...
		{
			// 这里是否可以修改，还需要测试一下
			pstmt = con.prepareStatement(sql.trim() , ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_UPDATABLE);
    ...
    public SSRS execSQL(String sql) {
        ...
        try {
            ...
            while (rs.next()) {
            	...
            	getCount++;
                for (int j = 1; j <= n; j++) {
                	logger.info("============={}", getDataValue(rsmd, rs, j));
    ...
    public SSRS execSQL(String sql, int start, int nCount) {
        ...
        try {
            ...
            // @@错误处理
            logger.error("##### Error Sql in ExeSQL at execSQL(String sql, int start, int nCount): " + sql + e.getMessage(), e);
            mErrors.clearErrors();
            CError.buildErr(this, e.toString());
